-- chunkname: @cocos.framework.audio
...
function audio.playMusic(filename, isLoop, force)
	...
	local volume = userDefault.getForeverLocalKey("musicVolume", 100, {
		rawKey = true
	})
...
function audio.playSound(filename, isLoop)
	...
	local volume = userDefault.getForeverLocalKey("effectVolume", 100, {
		rawKey = true
	}) / 100
