-- chunkname: @cocos.framework.components.event
...
function Event:on(eventName, listener, tag)
	...
	if DEBUG > 1 then
		printInfo("%s [Event] addEventListener() - event: %s, handle: %s, tag: \"%s\"", tostring(self.target_), eventName, handle, tostring(tag))
	end
