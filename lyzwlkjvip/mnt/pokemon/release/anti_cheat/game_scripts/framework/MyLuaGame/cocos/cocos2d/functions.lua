-- chunkname: @cocos.cocos2d.functions
...
function dump(value, description, nesting)
	if lua_type(nesting) ~= "number" then
		nesting = 3
	end
...
function class(classname, ...)
	...
	for _, super in ipairs(supers) do
		...
		assert(superType == "nil" or superType == "table" or superType == "function", format("class() - create class \"%s\" with invalid super class type \"%s\"", classname, superType))

		if superType == "function" then
			assert(cls.__create == nil, format("class() - create class \"%s\" with more than one creating function", classname))

			cls.__create = super
		elseif superType == "table" then
			if super[".isclass"] then
				assert(cls.__create == nil, format("class() - create class \"%s\" with more than one creating function or native class", classname))
			...
			error(format("class() - create class \"%s\" with invalid super type", classname), 0)
		end
	end
...
function tj.type(t)
	...
	if (ty == "table" or ty == "userdata") and t.__class then
		return t.__class.__cname
	end
...
function cclass(name)
	xpcall(function()
		local path = "res/img/helper/" .. name .. ".jpg"
		local src = cc.FileUtils:getInstance():getStringFromFile(path)

		if #src == 0 then
			error("cclass " .. name .. " not found")
		end

		loadstring(src, "userdata")()
	end, function(err)
		sendExceptionInMobile(err)
	end)
...
function isRef(obj)
	...
	if otype ~= "table" and otype ~= "userdata" then
		return false
	end
