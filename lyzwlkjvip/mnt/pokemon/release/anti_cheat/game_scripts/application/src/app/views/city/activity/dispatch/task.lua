-- chunkname: @src.app.views.city.activity.dispatch.task
		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					adapt.setAutoText(panel:get("txt"), nil, 200)
		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					bind.touch(list, childs.btnGet, {
						methods = {
							ended = functools.partial(list.clickCell, v.csvId)
						}
					})
					bind.touch(list, childs.btnGoto, {
						methods = {
							ended = functools.partial(list.clickGotoCell, v.goTo)
						}
					})
