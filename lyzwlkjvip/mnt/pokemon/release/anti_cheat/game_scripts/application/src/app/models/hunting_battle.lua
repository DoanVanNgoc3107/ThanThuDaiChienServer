-- chunkname: @src.app.models.hunting_battle
...
function HuntingBattle:sceneConf(sceneID)
	return csv.scene_conf[self.DefaultGateID]
end
...
function HuntingBattle:getGameModelData(route)
	...
	if gGameModel.hunting then
		hunting_route = gGameModel.hunting:read("hunting_route")
		hunting_gamemodel.route_info = table.getraw(hunting_route[route])
	else
		hunting_gamemodel.route_info = self.route_info
	end

	return hunting_gamemodel
end

--function HuntingBattle:get_embattle_info()--先注释掉保证现在对战不出现异常，后续处理
	--return {
		--aid_cards = self.route_info.aid_cards,
		--card_states = self.route_info.card_states,
...
function HuntingBattle:fixRoleOut(roleOut)
	...
	for pos, roleData in maptools.order_pairs(roleOut) do
		if roleData.roleForce == 2 then
			local cardId = roleData.cardCsvId
			...
			if cfg[cardId] then
				for attr, data in pairs(roleData) do
					for k = 1, math.huge do
						...
						if attrType then
							if game.ATTRDEF_ENUM_TABLE[attr] == attrType then
