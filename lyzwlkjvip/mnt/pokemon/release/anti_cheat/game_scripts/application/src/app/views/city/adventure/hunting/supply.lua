-- chunkname: @src.app.views.city.adventure.hunting.supply
		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					local richText = rich.createByStr("#C0x5B545B#" .. v.desc, 40, nil):xy(childs.text:width() / 2, childs.text:y() / 2):anchorPoint(0.5, 0.5):addTo(childs.text, 100, "richText"):formatText()
...
function HuntingSupplyView:onSure()
	...
	if cfg.type == SUPPLY_TYPE.single then
		if canSingle then
			gGameUI:stackUI("city.adventure.hunting.supply_detail", nil, nil, {
				route = self.params.route,
				node = self.params.node,
				cb = self:createHandler("onClose"),
				type = SUPPLY_TYPE.single,
				csvId = self.eventDatas:atproxy(self.selectNum:read()).id
			})
		else
