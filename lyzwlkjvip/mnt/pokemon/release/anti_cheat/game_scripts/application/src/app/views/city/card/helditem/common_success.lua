-- chunkname: @src.app.views.city.card.helditem.common_success
...
local function createAttrRichText(parent, csvId, advance)
	...
	local targetStr = table.concat({
		resultStr
	}, "\n")
	local richText = rich.createByStr("#C0x5B545B#" .. targetStr, 40, nil):anchorPoint(0, 0.5):xy(220, 23):addTo(parent, 10, "attrText")
		...
		binds = {
			...
			props = {
				...
				onCell = function(list, node, k, v)
					...
					adapt.oneLinePos(childs.note, childs.txt1, cc.p(13, 0))
					adapt.oneLinePos(childs.txt1, childs.iconArrow, cc.p(6, 0))
					adapt.oneLinePos(childs.iconArrow, childs.txt2, cc.p(6, 0))
					...
					uiEasy.setExecuteSequence(childs.txt1, {
						delayTime = baseDelay + BASE_TIME
					})
					uiEasy.setExecuteSequence(childs.iconArrow, {
						delayTime = baseDelay + BASE_TIME * 2
					})
					uiEasy.setExecuteSequence(childs.txt2, {
						delayTime = baseDelay + BASE_TIME * 3
					})
				end
			}
		}
...
function HeldItemCommonSuccessView:onCreate(params)
	...
	if cfg.effect2LevelAdvSeq[1] == self.newAdvance and params.typ == "advance" then
		...
		uiEasy.setExecuteSequence(self.descNote, {
			delayTime = BASE_TIME * 5
		})
	end
...
function HeldItemCommonSuccessView:setCenterPanel(cfg, level, advance, oldOrNew, typ)
	local childs = self.centerPanel:multiget("card1", "card2", "name1", "name2", "level1", "level2", "cardImg")
	local cardNode = childs["card" .. oldOrNew]
	local nameNode = childs["name" .. oldOrNew]
	local levelNode = childs["level" .. oldOrNew]
	local levelStr = gLanguageCsv.textLv .. level
	...
	text.addEffect(nameNode, {
		color = ui.COLORS.QUALITY[cfg.quality]
	})
	text.addEffect(levelNode, {
		color = ui.COLORS.QUALITY[cfg.quality]
	})
	adapt.oneLineCenterPos(cc.p(cardNode:x(), nameNode:y()), {
		nameNode,
		levelNode
	}, cc.p(8, 0))
	...
	uiEasy.setExecuteSequence(cardNode, {
		delayTime = baseDelay
	})
	uiEasy.setExecuteSequence(nameNode, {
		delayTime = baseDelay
	})
	uiEasy.setExecuteSequence(levelNode, {
		delayTime = baseDelay
	})
...
function HeldItemCommonSuccessView:setFight(oldFight, cardDbId)
	local childs = self.centerPanel:multiget("fight1", "fight2", "fightBg")
	...
	if cardDbId == nil then
		...
		uiEasy.setExecuteSequence(childs.fightBg, {
			delayTime = BASE_TIME * 3
		})
		uiEasy.setExecuteSequence(childs.fight1, {
			delayTime = BASE_TIME * 4
		})
		uiEasy.setExecuteSequence(childs.fight2, {
			delayTime = BASE_TIME * 5
		})
	end
end
