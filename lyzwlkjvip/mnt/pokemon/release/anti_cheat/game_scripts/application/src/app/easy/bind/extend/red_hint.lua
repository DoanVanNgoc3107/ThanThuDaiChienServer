-- chunkname: @src.app.easy.bind.extend.red_hint
...
function redHint:setNum(num)
	...
	local width = math.max(txt:width() + 51, spr:height())

	spr:size(width, spr:height())
end
...
function redHint:createPanel()
	...
	if self.showType ~= "num" then
		...
		txtLevel = label.create("", {
			fontSize = 45,
			color = ui.COLORS.NORMAL.WHITE
		}):setHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER)
	end

	local panel = ccui.Layout:create():size(spr:size()):alignCenter(self:size()):scale(1):addTo(self, 9999, "_redHint_"):xy(self:size().width, self:size().height)
...
function redHint:bindIdlers(k, v, tag, specialTag)
	if k == "unlockKey" then
		...
		if not self.unlockKeys[specialTag] then
			self.unlockKeys[specialTag] = {}
		end
...
function redHint:initExtend()
	...
	idlereasy.when(self.datas, function(_, datas)
		...
		performWithDelay(self, function()
			...
			local flag = func(datas, panel)

			if type(flag) == "number" then
				panel:visible(datas.state and flag > 0)
			else
				panel:visible(datas.state and flag ~= false)
			end
	...
	end):anonyOnly(self, string.format("%s/datas", self.selfID))
