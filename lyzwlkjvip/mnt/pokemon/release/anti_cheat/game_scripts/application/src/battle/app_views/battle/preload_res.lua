-- chunkname: @src.battle.app_views.battle.preload_res
...
local function loadBuffRes(resT, audioT, cfg)
	...
	for k, v in pairs(cfg.buffId) do
		if type(v) == "number" then
			...
			if csvBuff.effectRes then
				resT[csvBuff.effectRes] = (resT[csvBuff.effectRes] or 0) + 1
			end

			if csvBuff.textResPath then
				resT[csvBuff.textResPath] = (resT[csvBuff.textResPath] or 0) + 1
			end

			if csvBuff.iconResPath then
				resT[csvBuff.iconResPath] = (resT[csvBuff.iconResPath] or 0) + 1
			end

			if csvBuff.playEffect then
				resT[csvBuff.playEffect] = (resT[csvBuff.playEffect] or 0) + 1
			end

			if csvBuff.fixShow then
				resT[csvBuff.fixShow] = (resT[csvBuff.fixShow] or 0) + 1
			end
...
local function loadSkillProcess(resT, audioT, processId)
	...
	if cfg.effectRes then
		resT[cfg.effectRes] = (resT[cfg.effectRes] or 0) + 1
	end

	if cfg.shotEffect then
		resT[cfg.shotEffect] = (resT[cfg.shotEffect] or 0) + 1
	end
	...
	if cfg.callerId ~= nil and cfg.callerId > 0 then
		...
		if cfg.callerEffect then
			resT[cfg.callerEffect] = (resT[cfg.callerEffect] or 0) + 1
		end
	end
end
...
local function loadSkillEvent(resT, audioT, eventID)
	...
	if effectCfg.sound then
		audioT[effectCfg.sound.res] = true
	end

	if effectCfg.music then
		audioT[effectCfg.music.res] = true
	end

	if effectCfg.effectRes then
		resT[effectCfg.effectRes] = (resT[effectCfg.effectRes] or 0) + 1
	end
end
...
local function loadHeroUnit(resT, audioT, unitId)
	...
	if cfg.deathSound then
		audioT[cfg.deathSound] = true
	end
end
...
function globals.visitFightResources(resT, audioT, monsterCfg, data)
	...
	if data.aidRoleOut then
		for waveID, v in pairs(data.aidRoleOut[1]) do
			for seat, data in pairs(v) do
				loadHeroUnit(resT, audioT, data.roleId)

				count = count + 1
			end
		end

		for waveID, v in pairs(data.aidRoleOut[2]) do
			for seat, data in pairs(v) do
				loadHeroUnit(resT, audioT, data.roleId)
	...
	if data.skillProcess then
		for skillID, skill_process in pairs(data.skillProcess) do
			for k, v in ipairs(skill_process) do
				loadSkillProcess(resT, audioT, v)
			end
		end
	end
