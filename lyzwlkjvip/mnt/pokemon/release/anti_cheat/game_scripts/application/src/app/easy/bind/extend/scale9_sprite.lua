-- chunkname: @src.app.easy.bind.extend.scale9_sprite
...
function scale9Sprite:initExtend()
	...
	mask:size(size):alignCenter(size)
	cc.ClippingNode:create(mask):setAlphaThreshold(0.01):size(size):alignCenter(size):add(tile):addTo(self)
