-- chunkname: @src.app.views.city.adventure.clone_battle.view
...
function CloneBattleCityView:onCreate(data, baseView)
	...
	userDefault.setForeverLocalKey("cloneBattleBeKickedNum", self.kickNum, {
		new = true
	})

	if self.data.beKicked and self.kickNum < gCommonConfigCsv.cloneDailyBeKickedMax and beKickedNum < self.kickNum then
		gGameUI:showDialog({
			isRich = true,
			title = "",
			btnType = 1,
			content = string.format("#C0x5b545b#" .. gLanguageCsv.cloneBattleRecord8, gCommonConfigCsv.cloneDailyBeKickedMax - self.kickNum)
		})
	end
	...
	for idx, tb in pairs(data.nature) do
		...
		for i, id in pairs(tb[2]) do
			...
			local config = csv.unit[unitId] or csv.unit[1]
			local inBox = self.pokedexMarks[markId]
	...
	self:initCountDown()
	self:showKickTip()
	adapt.setTextScaleWithWidth(self.btnJoin:get("text"), nil, 240)
	adapt.setTextScaleWithWidth(self.btnRoom:get("text"), nil, 240)
end
...
function CloneBattleCityView:initNatureItem()
	...
	for i, pos in pairs(ITEM_POS) do
		...
		if natureTb then
			...
			local ani = widget.addAnimation(item, res, string.format(FMT2, natureId), 1):scale(scaleNum):xy(size.width / 2, 10)
			...
			text.addEffect(textItem, {
				color = ui.COLORS.ATTR[natureId]
			})
			bind.touch(self, item, {
				methods = {
					ended = functools.partial(self.onItemClick, self, item, i, natureTb)
				}
			})
		else
			item:hide()
