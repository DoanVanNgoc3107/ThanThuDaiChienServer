-- chunkname: @src.app.views.common.gain_sprite
...
function GainSpriteView:onCreate(data, hadSprite, showBtn, cb)
	local panel = ccui.Layout:create():addTo(self:getResourceNode(), 100, "mask"):setAnchorPoint(cc.p(0.5, 0.5)):size(cc.size(display.maxWidth, display.height)):alignCenter(self:getResourceNode():size()):setTouchEnabled(true):show()
	...
	local bgEffect = widget.addAnimation(self.effect, "effect/huodexinjinglingbg.skel", "effect", 1):alignCenter(self.effect:size()):scale(2)
	...
	performWithDelay(self.effect, function()
		...
		transition.executeSequence(self.leftPanel):scaleXTo(0.5, 1.1):scaleXTo(0.1, 1):done()
		transition.executeSequence(self.downPanel):scaleXTo(0.5, 1.1):scaleXTo(0.1, 1):done()
		transition.executeSequence(self.attrInfo):scaleTo(0.5, 1.1):scaleTo(0.1, 1):done()

		local pnode = self:getResourceNode()
		local spine1 = widget.addAnimation(pnode, "effect/huodexinjingling.skel", "effect", 103)

		spine1:align(cc.p(0.5, 1), self.titlePanel:xy()):scale(1.2)
		spine1:addPlay("effect_loop")

		local size = self.cardImg:size()

		widget.addAnimation(self.cardImg, "effect/jinglingchuxian.skel", "effect", 6):xy(size.width / 2, size.height * 0.05)
	...
	local cardData = card:read("card_id", "character", "nvalue", "name", "advance", "zawake_nvalue")
	...
	list:xy(self.skillContent:box()):z(self.skillContent:z()):addTo(self.skillContent:parent())
	...
	local size = self.cardImg:size()
	local cardSprite = widget.addAnimation(self.cardImg, unitCfg.unitRes, "standby_loop", 5):xy(size.width / 2, size.height * 0.05):scale(unitCfg.scaleU * 3)

	cardSprite:setSkin(unitCfg.skin)
	widget.addAnimationByKey(self.cardImg, "effect/jinhuajiemian.skel", "top", "effect_down2_loop", 4):xy(size.width / 2, -50):scale(1.2)
	widget.addAnimationByKey(self.cardImg, "effect/jinhuajiemian.skel", "down", "effect_up_loop", 6):xy(size.width / 2, -50):scale(1.2)
	...
	local params = {
		...
		position = {
			x = 0,
			y = math.floor(self.cardImg:height() / 1.8)
		}
	}

	uiEasy.setMaxStar(self.cardImg, params)
	uiEasy.setIconName("card", cardData.card_id, {
		space = true,
		node = self.txtName,
		name = cardData.name,
	...
	self.zawakeNvalue = cardData.zawake_nvalue
	...
	if data.new and not hadSprite[cardData.card_id] then
		...
		adapt.oneLineCenterPos(cc.p(self.pos:xy()), {
			self.cardName,
			self.cardNote
		}, cc.p(10, 0))
	else
	...
	if matchLanguageForce(cardCfg.languages) == false then
