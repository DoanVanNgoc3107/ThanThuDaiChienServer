-- chunkname: @src.app.views.city.adventure.gym_challenge.embattle2
...
function GymChallengeEmbattleView:initDefine(params)
	...
	self.panelNum = self.embattleMax
	...
	idlereasy.when(self.clientBattleCards, function(_, battle)
		...
		for i = 1, self.embattleMax do
			...
			if dbid then
				...
				childs.level:text(gLanguageCsv.textLv .. data:read("level"))
				...
				uiEasy.getStarPanel(data:read("star"), {
					align = "center",
					interval = -5,
					dbid = dbid
				}):scale(0.35):xy(170, 100):addTo(info, 2)
