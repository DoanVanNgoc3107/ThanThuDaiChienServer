-- chunkname: @src.app.views.city.adventure.hunting.view
...
function HuntingView:onCreate(data)
	...
	idlereasy.any({
		...
		self.eliteTimes,
		self.routeInfo
	}, function(_, normalTimes, eliteTimes, routeInfo)
		self:initPanel()

		local refreshHour = time.getRefreshHour()

		self.endTime1 = time.getNumTimestamp(self.routeInfo:read()[ROUTE_TYPE.normal].last_date, refreshHour, 0, 0) + 86400 * baseCfg[ROUTE_TYPE.normal].refreshDay

		if self.specialHuntingSign then
			self.endTime2 = time.getNumTimestamp(self.routeInfo:read()[ROUTE_TYPE.elite].last_date, refreshHour, 0, 0) + 86400 * baseCfg[ROUTE_TYPE.elite].refreshDay
		end
	end)
	self:initCountDown1()
...
function HuntingView:initPanel()
	...
	local richText = rich.createByStr(normalContent, 56, nil):xy(210, self.normalPanel:get("times"):y()):anchorPoint(0, 0.5):addTo(self.normalPanel, 100, "richText"):formatText()
	...
	if self.specialHuntingSign then
		...
		local richText = rich.createByStr(normalContent, 56, nil):xy(210, self.elitePanel:get("times"):y()):anchorPoint(0, 0.5):addTo(self.elitePanel, 100, "richText"):formatText()
...
function HuntingView:onEliteRouteClick()
	if not self.specialHuntingSign then
...
function HuntingView:initCountDown1()
	...
	local function setLabel()
		...
		local richText1 = rich.createByStr(str1, 40, nil):xy(330, 45):anchorPoint(0.5, 0.5):addTo(self.normalPanel, 100, "richTextTime"):formatText()

		if self.endTime1 - time.getTime() <= 0 and time.getTime() - lastRequestTime > 10 then
			lastRequestTime = time.getTime()

			gGameApp:requestServer("/game/hunting/main")
		end
...
function HuntingView:initCountDown2()
	...
	local function setLabel()
		...
		local richText1 = rich.createByStr(str2, 40, nil):xy(330, 45):anchorPoint(0.5, 0.5):addTo(self.elitePanel, 100, "richTextTime"):formatText()

		if self.endTime2 - time.getTime() <= 0 and time.getTime() - lastRequestTime > 10 then
			lastRequestTime = time.getTime()
