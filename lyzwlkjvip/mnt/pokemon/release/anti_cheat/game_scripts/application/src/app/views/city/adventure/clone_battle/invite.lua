-- chunkname: @src.app.views.city.adventure.clone_battle.invite
		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					adapt.oneLinePos(children.name, {
						children.lv,
						children.lvNumber
					}, cc.p(15, 0), "left")
					adapt.oneLinePos(children.text1, children.text2, cc.p(15, 0), "left")
					children.btn:tag(k)
					bind.click(list, children.btn, {
						method = functools.partial(list.clickCell, children.btn, k, v)
					})
					text.addEffect(children.btn:get("text"), {
						glow = {
...
function CloneBattleInviteView:getReunionType(roleID)
	...
	if self.reunion and self.reunion.role_type == 1 and self.reunion.info and self.reunion.info.end_time - time.getTime() > 0 and self.reunionBindRoleId and self.reunionBindRoleId == roleID then
		reunionStatue = SHOW_REUNION.SHOW_SENIRO
	elseif self.reunion and self.reunion.role_type == 2 and self.reunion.info and self.reunion.info.end_time - time.getTime() > 0 and self.reunion.info.role_id == roleID then
