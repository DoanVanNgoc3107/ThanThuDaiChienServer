-- chunkname: @src.app.views.city.card.mega.conversion

local rarityData = gLanguageTxtRarity
					...
					if itertools.size(v) == 0 then
						...
						if v.type == "card" then
							if not v.selectId then
								...
								list:xy(childs.name2:x() - childs.name2:width() / 2, childs.name2:y() - childs.name2:height() / 2):addTo(node, 5, "descList")
							else
								childs.icon:hide()
						...
						elseif v.type == "frag" then
							...
							if not v.selectId then
								...
								childs.icon:hide()
								childs.name2:hide()
								uiEasy.setIconName(v.selectId, v.num, {
									node = childs.name,
									width = node:width()
								})
							...
							childs.name2:hide()
							uiEasy.setIconName(v.key, v.num, {
								node = childs.name
							})
						end

						bind.touch(list, childs.icon, {
							methods = {
								ended = function()
									list.chooseCard(k, v)
								end
...
function MegaConversionView:onCreate(data)
	...
	idlereasy.any({
		self.megaConvertTimes,
		self.megaConvertBuyTimes
	}, function(_, megaConvertTimes, megaConvertBuyTimes)
		...
		adapt.oneLinePos(self.timesPanel:get("num2"), {
			self.timesPanel:get("num1"),
			self.timesPanel:get("txt1")
		}, cc.p(0, 0), "right")
	end)
	...
	idlereasy.any({
		...
		self.frags,
		self.items
	}, functools.partial(self.stateChange, self))
	self:debrisBtnFunc()
	Dialog.onCreate(self, {
		blackType = 2
	})
	adapt.setTextScaleWithWidth(self.timesPanel:get("txt1"), nil, 440)
	adapt.setTextScaleWithWidth(self.timesPanel:get("tip"), nil, 660)
	adapt.setTextScaleWithWidth(self.clickBtn:get("txt"), nil, 300)
end
...
function MegaConversionView:costGoldUpdate()
	...
	if gold == 0 then
		...
		text.addEffect(self.costPanel:get("num"), {
			color = gold <= roleGold and ui.COLORS.QUALITY_OUTLINE[1] or ui.COLORS.NORMAL.ALERT_ORANGE
		})
		adapt.oneLineCenterPos(cc.p(260, 40), {
			self.costPanel:get("txt1"),
			self.costPanel:get("num"),
			self.costPanel:get("icon")
		}, cc.p(8, 0))
	end
end
...
function MegaConversionView:stateUpdate()
	...
	if self.cardConvertFlag then
		...
		if self.cardConvertCfg.needCards1[1] then
			maxNum = not self.selectedData.selectId and 0 or math.min(maxNum, 1)

			table.insert(data, 1, {
				type = "card",
				cfg = self.cardConvertCfg,
				selectId = self.selectedData.selectId
			})
		end
	else
	...
	uiEasy.setIconName(self.data.id, nil, {
		node = self.debrisItem:get("name")
	})
	self.debrisItem:get("title.num"):text(ownNum)
	self.debrisItem:get("title.txt2"):text("/" .. self.data.num)
	text.addEffect(self.debrisItem:get("title.num"), {
		color = ownNum >= self.data.num and ui.COLORS.QUALITY_OUTLINE[1] or ui.COLORS.NORMAL.ALERT_ORANGE
	})
	adapt.oneLineCenterPos(cc.p(200, 30), {
		self.debrisItem:get("title.txt1"),
		self.debrisItem:get("title.num"),
		self.debrisItem:get("title.txt2")
	}, cc.p(5, 0))
...
function MegaConversionView:chooseCard(list, k, v)
	if v.type == "card" then
		gGameUI:stackUI("city.card.mega.choose_card", nil, {
			dialog = true
		}, self.selectedData, self:createHandler("stateChange"))
	elseif v.type == "frag" then
