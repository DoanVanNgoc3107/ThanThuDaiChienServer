-- chunkname: @src.app.views.city.card.helditem.advance
...
local function setCostTxt(panel, myCoin, needCoin)
	...
	local coinColor = needCoin <= myCoin and ui.COLORS.NORMAL.DEFAULT or ui.COLORS.NORMAL.RED
...
local function setEffect(parent, effectName, cb)
	...
	if not effect then
		effect = widget.addAnimationByKey(parent, "koudai_gonghuixunlian/gonghuixunlian.skel", effectName, "fangguang2", 10):xy(40, 20):scale(0.25)
		...
		binds = {
			...
			props = {
				...
				onCell = function(list, node, k, v)
					local childs = node:multiget("txtName", "txtNum", "txtNote1", "txtAdd", "icon", "txtNote2", "maxIcon", "imgIcon")
					...
					adapt.oneLinePos(childs.txtNote1, {
						childs.txtAdd,
						childs.icon,
						childs.txtNote2
					}, cc.p(5, 0))
					...
					itertools.invoke({
						...
						childs.icon,
						childs.txtNote2
					}, (v.isMax or v.valExtra <= 0) and "hide" or "show")
				end
			}
		}
	},
		...
		binds = {
			...
			props = {
				...
				onCell = function(list, node, k, v)
					...
					bind.touch(list, node:get("subIcon"), {
						methods = {
							ended = functools.partial(list.subClick, t, v)
						}
					})
				end,
...
function HeldItemAdvanceView:onCreate(dbId)
	...
	adapt.setTextAdaptWithSize(txtEmpty, {
		vertical = "center",
		horizontal = "center",
	...
	idlereasy.any({
		...
		for _, v in pairs(heldItems) do
			...
			if heldItem then
				...
				if not meteorHelditemsHash[v] and not t[itemData.held_item_id] then
					t[itemData.held_item_id] = {
						itemType = 2,
						num = 0
					}
				end

				if itemData.advance == 0 and itemData.sum_exp == 0 and not itemData.card_db_id and itemData.exist_flag and v ~= self.dbId and not meteorHelditemsHash[v] then
	...
	idlereasy.any({
		...
		self.selectItemNum,
		self.level
	}, function(_, selectItemIdx, selectItemNum, level)
	...
	idlereasy.any({
		self.gold,
		self.needStrengthenGold
	}, function(_, gold, needStrengthenGold)
	...
	idlereasy.any({
		...
		self.totalExp,
		self.showTab
	}, function(_, level, advance, sumExp, totalExp)
...
function HeldItemAdvanceView:setAdvancePanel(advance)
	...
	local childs = self.advancePanel:multiget("attrNote", "nextNote", "attrList", "nextList", "costNote", "textTip", "itemList", "btnAdvance", "maxIcon")
	...
	if not self.advanceMax then
		...
		self.levelNotEnough = advanceLvLimit > self.level:read()
	...
	itertools.invoke({
		...
		childs.itemList,
		childs.btnAdvance
	}, self.advanceMax and "hide" or "show")
...
function HeldItemAdvanceView:setLeftAdvancePanel(advance)
	local childs = self.leftPanel:get("advancePanel"):multiget("iconMax", "level1", "level2", "iconArrow")

	childs.iconMax:visible(self.advanceMax)
	childs.level1:text("+" .. advance)
	childs.iconMax:setPositionY(childs.level1:y() + (childs.iconMax:height() - childs.level1:height()) / 2)
...
function HeldItemAdvanceView:setLeftPanel(level, advance, sumExp, totalExp)
	local childs = self.leftPanel:multiget("itemName", "itemIcon", "itemIconBg", "strengthenPanel", "advancePanel")
	local csvItems = csv.held_item.items[self.heldItemId]
	...
	text.addEffect(childs.itemName, {
		color = quality == 1 and ui.COLORS.NORMAL.DEFAULT or ui.COLORS.QUALITY[quality]
	})
	...
	for i = 1, maxLevel do
		...
		if maxLevel <= i or sum_exp < needExp then
			clientLevel = i
	...
	local strengthenPanel = childs.strengthenPanel:multiget("iconMax", "level1", "level2", "iconArrow", "bar", "percent")

	strengthenPanel.iconMax:visible(maxLevel <= level or maxLevel <= clientLevel)
	strengthenPanel.level1:text(gLanguageCsv.textLv .. level)
	strengthenPanel.level2:text(clientLevel):visible(level < clientLevel)
	strengthenPanel.iconArrow:visible(level < clientLevel)
	strengthenPanel.iconMax:setPositionY(strengthenPanel.level2:y() + (strengthenPanel.iconMax:height() - strengthenPanel.level2:height()) / 2)
	adapt.oneLinePos(strengthenPanel.level1, {
		strengthenPanel.iconArrow,
		strengthenPanel.level2,
		strengthenPanel.iconMax
	}, {
		cc.p(20, 0),
		cc.p(20, 0),
		cc.p(20, 0)
	...
	if self.tmpShowTab ~= self.showTab:read() or self.clientLevel ~= clientLevel or self.tmpAdvance ~= advance or totalExp == 0 then
...
function HeldItemAdvanceView:setStrengthenPanel(itemData)
	local childs = self.strengthenPanel:multiget("itemName", "exp", "numNote1", "num", "numNote2", "btnSub", "btnAdd", "costPanel")

	if itemData then
		childs.itemName:text(itemData.cfg.name)
		text.addEffect(childs.itemName, {
			color = itemData.cfg.quality == 1 and ui.COLORS.NORMAL.DEFAULT or ui.COLORS.QUALITY[itemData.cfg.quality]
		})
	end
...
function HeldItemAdvanceView:playAction(parent, target)
	...
	transition.executeSequence(cloneItem, true):moveTo(0.4, pos.x, pos.y):func(function()
...
function HeldItemAdvanceView:onSelectItemClick(list, t, node, v, event)
			...
			if time > 0.5 then
				...
				if perSel ~= t.k and v.cfg.quality >= 4 and v.selectNum == 0 and not tipState then
					if not isSpecialItem(v.csvId) then
						...
						gGameUI:showDialog({
							btnType = 2,
							content = gLanguageCsv.advancedQualityCarryingProps,
							btnStr = gLanguageCsv.tips,
							cb = function()
								self.selectItemIdx:set(t.k, true)
							end
						})
	...
	elseif event.name == "moved" then
		beganTouchPos = beganTouchPos or node:getTouchBeganPosition()
...
function HeldItemAdvanceView:onStrengthenClick()
	...
	gGameApp:requestServerCustom("/game/helditem/strength"):params(csvIds, self.dbId):onResponse(function(tb)
		setEffect(self.leftPanel:get("itemIcon"), "strength", function()
			self.showOver[1] = true
		end)
	end):wait(self.showOver):doit(function(tb)
		if self.oldLevel < self.level:read() then
			gGameUI:stackUI("city.card.helditem.common_success", nil, {
				blackLayer = true
			}, {
				typ = "level",
				dbId = self.dbId,
...
function HeldItemAdvanceView:onAdvanceClick()
	...
	local costHeldItemIDs, costItemIDs
	...
	if csvSize(csv.held_item.items[self.heldItemId].universalItems) == 0 then
		costItemIDs = {}
		costHeldItemIDs = {}
	end
	...
	gGameApp:requestServerCustom("/game/helditem/advance"):params(self.dbId, costHeldItemIDs, costItemIDs):onResponse(function(tb)
...
function HeldItemAdvanceView:onLeftItemClick(list, k, v)
	if v.csvId <= game.HELD_ITEM_CSVID_LIMIT and v.csvId > game.FRAGMENT_CSVID_LIMIT or csvSize(csv.held_item.items[self.heldItemId].universalItems) == 0 then
