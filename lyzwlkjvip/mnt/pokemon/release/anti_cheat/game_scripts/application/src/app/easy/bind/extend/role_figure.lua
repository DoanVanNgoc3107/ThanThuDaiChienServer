-- chunkname: @src.app.easy.bind.extend.role_figure
...
function roleFigure:initExtend()
	local figure = ccui.ImageView:create():alignCenter(self:size()):addTo(self, 10, "figure"):visible(false)
	...
	helper.callOrWhen(self.data, function(id)
		...
		if self.spine and cfg and cfg.resSpine ~= "" then
			...
			local spine = widget.addAnimationByKey(self, cfg.resSpine, "spine", "standby_loop1"):xy(self:size().width / 2, 0)
