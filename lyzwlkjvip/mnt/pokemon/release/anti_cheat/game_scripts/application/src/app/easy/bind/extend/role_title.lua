-- chunkname: @src.app.easy.bind.extend.role_title
...
function roleTitle:showData(titleId)
	...
	if not panel then
		panel = ccui.Layout:create():addTo(self, 1, panelName)

		ccui.ImageView:create():addTo(panel, 2, "titleImgBg")
		ccui.ImageView:create():addTo(panel, 3, "titleImg")
		label.create("", {
			fontSize = 33,
	...
	elseif showType == "txt" then
		...
		size.width = math.max(size.width, titleTxt:width())
		...
		if self.isGray then
			...
			text.addEffect(titleTxt, {
				outline = {
					size = 4,
					color = cc.c4b(cfg.color[1], cfg.color[2], cfg.color[3], 255)
				}
			})
		end
