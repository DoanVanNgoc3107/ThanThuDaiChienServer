-- chunkname: @src.app.views.city.adventure.gym_challenge.view
		...
		binds = {
			{
				event = "touch",
				methods = {
					ended = bindHelper.self("onBtnBufClick")
				}
			},
			{
				class = "red_hint",
		...
		binds = {
			{
				event = "touch",
				methods = {
					ended = bindHelper.self("onBtnCrossClick")
				}
			},
			{
				action = "effect_loop",
		...
		binds = {
			{
				event = "touch",
				methods = {
					ended = bindHelper.self("onBtnLogClick")
				}
			},
			{
				class = "red_hint",
...
function GymChallengeView:initParticle()
	...
	particleNode:addTo(self.rightDownPanel):xy(self.btnCross:xy()):scale(2)
end
...
function GymChallengeView:onClose(t)
	if not self.notCross:read() then
		...
		local cloudSpine = widget.addAnimationByKey(self:getResourceNode(), "gym/kuafu_yun.skel", "cloudSpine", "kuafu_effect", 6):xy(cc.p(display.sizeInView.width / 2, display.sizeInView.height / 2)):scale(2)
	else
...
function GymChallengeView:initScrollView()
	...
	self.scrollView:size(display.sizeInViewRect):xy(display.sizeInViewRect)
...
function GymChallengeView:initGyms()
	for i = 1, 9 do
		...
		local spine = widget.addAnimationByKey(self.scrollView:get("bg"), "gym/haidao.skel", "spine" .. i, cfg.spine, 6):xy(cfg.pos):scale(cfg.scale)
	end

	for id, cfg in orderCsvPairs(csv.gym.gym) do
		local gym = self.gymItem:clone():addTo(self.scrollView):xy(cfg.pos)
		local spine = widget.addAnimationByKey(gym, "gym/haidao.skel", "spine" .. id, cfg.spine, 0):xy(cfg.posSkew):scale(2)
		...
		bind.touch(self, gym:get("imgHeadBg"), {
			methods = {
				ended = function()
					if self.notCross:read() == false then
						...
						if owners and owners[1] then
							...
							gGameApp:requestServer("/game/gym/role/info", function(tb)
								...
								local unlocked = self.gymsInfo:atproxy(id).unlocked and maxDegreeID < self.gymDatas:read().gym_fuben[id]
...
function GymChallengeView:refreshOpacity()
	...
	for id, gym in ipairs(self.gyms) do
		...
		local x, y = scrollViewPos.x + gymX * self.mapScale - 150, scrollViewPos.y + gymY * self.mapScale - 150
		local diff = 0

		if cx < math.abs(x - cx) or cy < math.abs(y - cy) then
			diff = math.max(math.abs(x - cx) - cx, math.abs(y - cy) - cy)
		end

		local opacity = math.max(76.5, 255 * (1 - math.min(1, diff / 300)))
	...
	for i = 1, 9 do
		...
		local x, y = scrollViewPos.x + imgX * self.mapScale - 150, scrollViewPos.y + imgY * self.mapScale - 150
		local diff = 0
...
function GymChallengeView:initModel()
	...
	self.curIndex = idler.new(0)
	self.gymsInfo = idlers.newWithMap({
		{},
		{},
		{},
		{},
	...
	idlereasy.any({
		...
		for id, info in ipairs(gymsInfo) do
			...
			self.btnCross:get("imgLock"):setVisible(notCross and gGameModel.gym:read("crossKey") == "" and itertools.size(lastCrossGymRoles) == 0 or openTime > time.getTime())
		end
	end)
	idlereasy.any({
		self.gymOwners,
		...
		self.notCross,
		self.round
	}, function(_, _gymOwners, _gymCrossOwners, notCross, round)
	...
	self.notCross:addListener(function(val, oldval)
		...
		if not val then
			local fontScale = matchLanguage({
				"kr"
			}) and 1 or 0.8

			self.mapScale = CROSS_MAP_SCALE

			self.btnCrossText:text(gLanguageCsv.gymReturn):scale(fontScale)

			id = self.maxCrossLockID
		else
			self.mapScale = MAP_SCALE

			self.btnCrossText:text(gLanguageCsv.gymCross):scale(1)

			id = self.maxLockID
		end

		adapt.setTextAdaptWithSize(self.btnCrossText, {
			vertical = "center",
			horizontal = "center",
			size = cc.size(300, 140)
		})
		...
		if self.first == true then
			...
			local ationMove = cc.MoveTo:create(1, {
				x = x,
				y = y
			})
	...
	idlereasy.any({
		gGameModel.daily_record:getIdler("gym_battle_times"),
		self.buyTimes
	}, function(_, times, buyTimes)
...
function GymChallengeView:getCountDownState()
	...
	elseif self.round:read() == "start" then
		self.endTime = time.getNumTimestamp(self.starDate, 21, 45) + 518400
...
function GymChallengeView:getOpenTime()
	...
	if wday == 1 and d.hour < time.getRefreshHour() then
		openTime = openStamps - (d.hour * 3600 + d.min * 60 + d.sec) + 18000
	else
		openTime = openStamps + (8 - wday) * 24 * 3600 - (d.hour * 3600 + d.min * 60 + d.sec) + 18000
	end
...
function GymChallengeView:onGymClick(id)
	if not self.notCross:read() then
		...
		if self.gymsInfo:atproxy(id).completed == true then
			gGameUI:stackUI("city.adventure.gym_challenge.cross_gate", nil, {
				full = true
			}, id, self.gymsInfo:atproxy(id).completed)
		else
			gGameUI:showTip(gLanguageCsv.gymCrossTips1)
		end
	elseif self.gymsInfo:atproxy(id).unlocked == true then
		gGameUI:stackUI("city.adventure.gym_challenge.gate", nil, {
			full = true
		}, id)
	else
	...
	if self.notCross:read() then
		...
		for k, v in orderCsvPairs(csv.gym.gym) do
			table.insert(context, c.clone(view.awardItem, function(item)
				...
				childs.text:text("")
				rich.createByStr("#C0x5b545b#" .. v.leaderAwardDesc, 40):anchorPoint(1, 0.5):xy(250, 100):addTo(childs.text, 6)
		...
		for k, v in orderCsvPairs(csv.gym.gym) do
			table.insert(context, c.clone(view.awardItem, function(item)
				...
				childs.text:text("")
				rich.createByStr("#C0x5b545b#" .. v.crossLeaderAwardDesc, 40):anchorPoint(1, 0.5):xy(260, 100):addTo(childs.text, 6)
			...
			table.insert(context, c.clone(view.awardItem, function(item)
				...
				childs.text:text("")
				rich.createByStr("#C0x5b545b#" .. v.crossSubAwardDesc, 40):anchorPoint(1, 0.5):xy(260, 100):addTo(childs.text, 6)
...
function GymChallengeView:onAddClick()
	...
	local times = math.min(self.buyTimes:read() + 1, csvSize(gCostCsv.gym_battle_buy_cost))
...
function GymChallengeView:onBtnLogClick()
	gGameApp:requestServer("/game/gym/main", function(tb)
		gGameUI:stackUI("city.adventure.gym_challenge.log", nil, {
			full = true
		})
