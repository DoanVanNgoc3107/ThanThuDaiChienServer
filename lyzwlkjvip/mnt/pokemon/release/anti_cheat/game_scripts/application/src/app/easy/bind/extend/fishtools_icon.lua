-- chunkname: @src.app.easy.bind.extend.fishtools_icon
...
function fishToolsIcon:initExtend()
	...
	local imgBG = ccui.ImageView:create():align(cc.p(0.5, 0.5), 100, 100):addTo(panel, 1)
	local icon = ccui.ImageView:create():align(cc.p(0.5, 0.5), 100, 100):scale(2):addTo(panel, 2, "icon")
	local num = panel:get("num")

	if not num then
		num = cc.Label:createWithTTF("", ui.FONT_PATH, 40):align(cc.p(1, 0), 170, 20):addTo(panel, 2, "num")
	...
	helper.callOrWhen(self.data, function(data)
		...
		elseif data.typ == 3 then
			local imgBG = ccui.ImageView:create("common/box/box_portrait.png"):align(cc.p(0.5, 0.5), 100, 100):scale(1.2):addTo(panel, 1)
			local cfg = csv.unit[data.key]
			local icon = ccui.ImageView:create(cfg.icon):align(cc.p(0.5, 0.5), 100, 100):scale(1.8):addTo(panel, 2, "icon")
		end
		...
		if self.lock then
			...
			elseif data.typ == 3 and data.lock == nil then
				self:setLock(panel, data.typ)
			end
		end
		...
		num = num or cc.Label:createWithTTF("", ui.FONT_PATH, 40):align(cc.p(1, 0), 170, 10):addTo(panel, 2, "num")
		...
		if self.num and data.typ == 2 then
			...
			text.addEffect(num, {
				outline = {
					color = ui.COLORS.QUALITY_OUTLINE[cfg.quality]
				}
			})
		end
...
function fishToolsIcon:setLock(panel, typ)
	...
	ccui.ImageView:create(mask):align(cc.p(0.5, 0.5), 100, 100):scale(scale):addTo(panel, 4, "lock")
	ccui.ImageView:create("common/btn/btn_bs.png"):align(cc.p(0.5, 0.5), 100, 100):addTo(panel, 5, "lock")
