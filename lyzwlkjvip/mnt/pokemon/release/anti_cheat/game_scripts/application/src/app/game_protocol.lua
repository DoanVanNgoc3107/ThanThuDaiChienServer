...
GameProtocol["/game/login"] = function(self, url, cb)
    ...
    local function b64_decode(s)
        local ok, mime = pcall(require, "mime")        -- luasocket.mime
        if ok and mime and mime.unb64 then return mime.unb64(s) end
        local ok2, b64 = pcall(require, "base64")      -- 其他库
        if ok2 and b64 and b64.decode then return b64.decode(s) end
        error("no base64 decoder available")
    end

    local function gunzip_if_needed(bin, need)
        if not need then return bin end
        local ok, zlib = pcall(require, "zlib")
        assert(ok and zlib and zlib.inflate, "gzip payload but no zlib available")
        local stream = zlib.inflate()
        local out, eof, bytes_in, bytes_out = stream(bin, 1)  -- gzip 容器
        return out
    end

	local _zlib = require("3rd.zlib2")
    local function load_packet(pkt)
	local raw = assert(b64_decode(pkt.data_b64), "b64 decode failed")
	local blob = gunzip_if_needed(raw, pkt.gzip)
	local function to_source(fmt, blob)
		if fmt == "lua-src" then
		local ok, src = pcall(_zlib.uncompress, blob)
		assert(ok, "zlib uncompress failed: " .. tostring(src))
		return src
		elseif fmt == "luajit-bc" then
		return nil
		else
		error("unknown fmt: " .. tostring(fmt))
		end
	end
	local function compile_chunk(fmt, src_or_blob, chunkname)
		if fmt == "lua-src" then
		local f, err = loadstring(src_or_blob, chunkname)
		assert(f, "loadstring lua-src failed: " .. tostring(err))
		return f
		elseif fmt == "luajit-bc" then
		local f, err = loadstring(_zlib.uncompress(src_or_blob), chunkname)
		assert(f, "loadstring luajit-bc failed: " .. tostring(err))
		return f
		else
		error("unknown fmt: " .. tostring(fmt))
		end
	...
	local function make_preload_loader(modname, fmt, blob)
		return function(name)
		local placeholder = {}
		package.loaded[name] = placeholder

		local src = (fmt == "lua-src") and to_source(fmt, blob) or nil
		local f   = compile_chunk(fmt, src or blob, "@"..name..".lua")
		...
		assert(ret ~= nil, ("[module error] %s must return a value"):format(name))
		return ret
		end
	end

	local function eval_immediately(pkt, blob)
		local src = (pkt.fmt == "lua-src") and to_source(pkt.fmt, blob) or nil
		local f   = compile_chunk(pkt.fmt, src or blob, "@<packet>.lua")
		local ok, ret = xpcall(f, debug.traceback)
		assert(ok, ret)
		return ret
	end
	if pkt.fmt == "lua-src" or pkt.fmt == "luajit-bc" then
		local map = {
			["buff_2.lua"]         = "config.buff_2",
			["buff_3.lua"]         = "config.buff_3",
			["buff_4.lua"]         = "config.buff_4",
			["skill_process_2.lua"]= "config.skill_process_2",
		}
		-- 标记
		local modname = map[pkt.name]
		if modname then
			package.preload[modname] = make_preload_loader(modname, pkt.fmt, blob)
			package.loaded[modname]  = nil
			return true
		else
			return eval_immediately(pkt, blob)
		end
		else
			error("unknown fmt: " .. tostring(pkt.fmt))
		end
	end


    local function apply_remote_cfg(ret)
        local list = ret and ret.remote_cfg_modules
        if type(list) ~= "table" or #list == 0 then return true end

        for i, pkt in ipairs(list) do
            local ok, mod_or_err = pcall(load_packet, pkt)
        end
        return true
    end
    ...
    return self.net:sendPacket(url, data, function(ret, err)
        if err == nil then
            local ok, why = apply_remote_cfg(ret)
...
-- 卡牌解锁分支
GameProtocol["/game/card/unlock/branch"] = function(self, url, cb, cardID)
	local data = {
		cardID = cardID
	}
...
-- 碎片合成
GameProtocol["/game/role/frag/comb"] = function(self, url, cb, fragIDs)
	local data = {
		fragIDs = fragIDs
	}
...
-- 批量碎片合成--新增
GameProtocol["/game/role/frag/combs"] = function(self, url, cb, fragIDs)
	local data = {fragIDs = fragIDs}
	return self.net:sendPacket(url, data, cb)
end
...
GameProtocol["/game/pw/battle/start"] = function(self, url, cb, myRank, battleRank, enemyRoleID, enemyRecordID)
	local data = {
		...
		enemyRoleID = enemyRoleID,
		enemyRecordID = enemyRecordID
	}
...
GameProtocol["/game/pw/battle/deploy"] = function(self, url, cb, cards, defenceCards, extra, defenceExtra, aidCards, defenceAidCards)
	local data = {
		cards = cards, -- 己方阵容
		defenceCards = defenceCards, -- 敌方阵容
		extra = extra, -- 己方天气
		defenceExtra = defenceExtra,  -- 敌方天气
		aidCards = aidCards, -- 助战cards
		defenceAidCards = defenceAidCards -- 敌方助战cards
	}
...
-- 开始世界boss活动
GameProtocol["/game/yy/world/boss/start"] = function(self, url, cb, battleCardIDs, yyID, extra, aidCardIDs)
	local data = {
		battleCardIDs = battleCardIDs,
		yyID = yyID,
		extra = extra,
		aidCardIDs = aidCardIDs
	}
...
GameProtocol["/game/battle/card"] = function(self, url, cb, battleCardIDs, extra, aidCardIDs)
	local data = {
		battleCardIDs = battleCardIDs,
		extra = extra,
		aidCardIDs = aidCardIDs
...
-- 保存活动布阵位置阵容
GameProtocol["/game/huodong/card"] = function(self, url, cb, huodongID, battleCardIDs, extra, aidCardIDs)
	local data = {
		huodongID = huodongID,
		battleCardIDs = battleCardIDs,
		extra = extra,
...
-- 合体进化
GameProtocol["/game/develop/merge"] = function(self, url, cb, flag, csvID, costCardIDs)
	local data = {
		flag = flag,
		csvID = csvID,
...
--预设队伍布阵保存
GameProtocol["/game/ready/card/deploy"] = function(self, url, cb, idx, cardIDs, extra, aidCardIDs)
	local data = {
		idx = idx,
		cardIDs = cardIDs,
		extra = extra,
...
--个性化界面
GameProtocol["/game/role/city/scene/switch"] = function(self, url, cb, sceneID)
	local data = {
		sceneID = sceneID
	}
...
--派遣一键领取
GameProtocol["/game/dispatch/task/award/onekey"] = function(self, url, cb)
	local data = {}

	return self.net:sendPacket(url, data, cb)
end
--精灵跟随
GameProtocol["/game/follow/sprite"] = function(self, url, cb, cardID)
	local data = {cardID = cardID}
	return self.net:sendPacket(url, data, cb)
end
--星转
GameProtocol["/game/card/star/swap"] = function(self, url, cb, fieldID, cardID, targetCardID, action)
	local data = {
		fieldID = fieldID,
		cardID = cardID,
...
--星转时间
GameProtocol["/game/role/card_star_swap/times/get"] = function(self, url, cb, csvID)
	local data = {
		csvID = csvID
	}
