-- chunkname: @src.app.views.common.gem_details
...
function GemDetailsView:onCreate(param)
	...
	self.panelName:text(cfg.name)
	text.addEffect(self.panelName, {
		color = ui.COLORS.QUALITY_DARK[quality]
	})
	...
	if data then
		...
		for i = 1, 9 do
			if data[i] then
				...
				local richText = rich.createWithWidth(color .. txt1 .. txt2 .. color, 40, cc.size(700, 90), 700, 0):anchorPoint(0, 1)

				itemTxt:height(richText:height())
				richText:addTo(itemTxt):y(itemTxt:height()):z(2)
