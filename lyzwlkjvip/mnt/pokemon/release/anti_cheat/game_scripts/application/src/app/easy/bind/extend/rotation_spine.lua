-- chunkname: @src.app.easy.bind.extend.rotation_spine
...
function rotationSpine:rotation(isClockWise, pos, scales)
	...
	for j = begin, endPos, step do
		...
		local diffX = j == judgePos and math.abs(2 * self.a - math.abs(pos[j].x - self.pos[j].x)) / 2 or math.abs(self.a - math.abs(pos[j].x - self.pos[j].x))
		...
		if diffX ~= 0 then
			...
			for i = 1, times do
				...
				try:spawnBegin():scaleTo(0.02, scale):moveTo(0.02, x, y):spawnEnd()
			end
			...
			self.spriteArr[j]:xy(pos[j]):scale(scales[j])
		end
	end
...
function rotationSpine:touchEventListener(panel, cb, index)
	...
	panel:addTouchEventListener(function(sender, eventType)
		...
		elseif eventType == ccui.TouchEventType.moved then
			...
			for i = 1, 3 do
				self.spriteArr[i]:xy(newPos[i]):scale(newScale[i]):z(isClockWise and 10 + i or 10 - i)
			end
		elseif eventType == ccui.TouchEventType.ended or eventType == ccui.TouchEventType.canceled then
			...
			if math.abs(endPos.x - beginPosX) < 5 then
				for i = 1, 3 do
					self.spriteArr[i]:xy(self.pos[i]):scale(self.scales[i])
				end
...
function rotationSpine:initExtend()
	...
	local panel = ccui.Layout:create():size(size):addTo(self, 10):alignCenter(self:size()):setTouchEnabled(true)

	self.spriteArr = {}
	self.pos = {
		cc.p(0, self.b),
		cc.p(self.a, 0),
		cc.p(2 * self.a, self.b)
	}
	self.scales = {
		self.minScale,
		self.maxScale,
	...
	local function _playAction(dt)
		self:rotation(self.isClockWise)
	end

	local dt = 5

	self:enableSchedule():schedule(_playAction, dt, 5, 1)

	for i, v in ipairs(self.data) do
		local unit = csv.unit[csv.cards[v].unitID]
		local unitRes = self.unitRes and self.unitRes[i] or unit.unitRes
		local spinePanel = ccui.Layout:create():setTag(v):addTo(panel, 10):anchorPoint(cc.p(0.5, 0.1)):xy(self.pos[i]):setTouchEnabled(true):scale(self.scales[i])
