-- chunkname: @src.app.views.city.adventure.hunting.route
...
local SPINE_CARDS = {
	charizard = {
		scale = 2.8,
		res = "koudai_penhuolong/hero_penhuolong.skel",
		x = 150,
		y = 0
	},
	bulbasaur = {
		scale = 2.4,
		res = "koudai_miaowazhongzi/hero_miaowazhongzi.skel",
		x = 430,
		y = -20
	},
	squirtle = {
		scale = 2.4,
		res = "koudai_jienigui/hero_jienigui.skel",
		x = 660,
		y = 0
	},
	pikachu = {
		scale = 2,
		res = "koudai_xiaozhibanpikaqiu/hero_xiaozhibanpikaqiu.skel",
		x = 890,
		y = 0
	}
}
local ICON_TYPE = {
	"city/adventure/hunting/icon_zd1.png",
	"city/adventure/hunting/icon_box.png",
	"city/adventure/hunting/icon_jiuyuan.png",
	"city/adventure/hunting/icon_zd2.png"
}
local GATE_TYPE = {
	"city/adventure/hunting/icon_zd1.png",
	"city/adventure/hunting/icon_zd2.png",
	"city/adventure/hunting/icon_boss.png"
}
local GATE_SPINE_TYPE = {
	{
		scale = 1,
		res = "city/adventure/hunting/icon_zd1_red.png"
	},
	{
		scale = 1,
		res = "city/adventure/hunting/icon_zd2_red.png"
	},
	{
		scale = 1.5,
		res = "city/adventure/endless_tower/icon_boss.png"
	}
}
local actionTime = 3
local EVENT_SPINE = {
	...
	{
		scale = 1.3,
		res = "effect/jiedianjiangli.skel",
		action = "effect_loop",
		y = 1850,
		x = 2700,
		spineName = "box"
	...
	{
		scale = 3,
		res = "koudai_jilidan/hero_jilidan.skel",
		action = "standby_loop",
		y = 1950,
		x = 2800,
		spineName = "supply"
	...
	{
		scale = 3,
		res = "koudai_kedaya/hero_kedaya.skel",
		action = "standby_loop",
		y = 1940,
		x = 2760,
		spineName = "mul"
...
HuntingRouteView.RESOURCE_BINDING = {
	...
	["buffPanel.buffBtn"] = {
		...
		binds = {
			event = "touch",
			methods = {
				ended = bindHelper.self("onBuffBtnClick")
			}
		}
	},
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					bind.touch(list, node, {
						methods = {
							ended = functools.partial(list.itemClick, list, node, v)
						}
					})
				end
			},
	...
	["rightBottomPanel.btnTask"] = {
		...
		binds = {
			event = "touch",
			methods = {
				ended = bindHelper.self("onEndClick")
			}
		}
	}
}
...
function HuntingRouteView:onCreate(route)
	...
	gGameUI.topuiManager:createView("hunting", self, {
...
function HuntingRouteView:initModel()
	...
	idlereasy.any({
		...
		if routeInfo[self.route].node == 0 then
			...
			return
		elseif routeInfo[self.route].node == self.minId + itertools.size(self.routeData) then
			self.curNode = self.minId + itertools.size(self.routeData)
		...
		elseif self.curNode ~= routeInfo[self.route].node or routeInfo[self.route].board_id and boardID ~= self.boardId then
			self.curNode = routeInfo[self.route].node
			self.boardId = boardID
		...
		if len > 0 then
			...
			performWithDelay(self, function()
				...
				self:onBuffBtnClick()
				gGameUI:stackUI("city.adventure.hunting.select_buff", nil, {
					blackLayer = true
				}, {
					route = self.route,
					node = self.curNode,
					cb = self:createHandler("playBuffSpine")
				})
...
function HuntingRouteView:playBuffSpine()
	...
	local effect = widget.addAnimationByKey(self.buffBtn, "random_tower/new_jiacheng.skel", "effect", "effect", 10):xy(70, 70):scale(2)
...
function HuntingRouteView:initBuffDatas()
	...
	local size = bg:size()
	local clip = cc.ClippingNode:create():setStencil(bg):setInverted(false):setAlphaThreshold(0.2):xy(cc.p(0, 0)):add(self.buffList, 100):addTo(self.buffActionNode, 3, "clipper")

	self.buffList:release()
end

function HuntingRouteView:initSpine(type)
	local effect = widget.addAnimationByKey(self.runSpine, "loading/loading_pikaqiu.skel", "effect", "effect_loop", 10):xy(0, 10):scale(1.3)

	self.spinePanel:removeAllChildren()

	if type then
		for id, v in pairs(SPINE_CARDS) do
			local cardSprite1 = widget.addAnimationByKey(self.spinePanel, v.res, id, ACTION_TYPE[type], 1000):scale(v.scale):xy(v.x, v.y)
		end
	end
end
...
function HuntingRouteView:initProgress()
	...
	for id, val in pairs(self.routeData) do
		if val.count == itertools.size(self.routeData) then
			local bar = self.prograssPanel:get("proPanel3"):clone():show():addTo(self.prograssPanel, 5, "bar" .. id):xy(self.prograssPanel:get("proPanel2"):x() + 252 * (val.count - 2) + 170, self.prograssPanel:get("proPanel2"):y())
			...
			local bar = self.prograssPanel:get("proPanel2"):clone():show():addTo(self.prograssPanel, 5, "bar" .. id):xy(self.prograssPanel:get("proPanel2"):x() + 253 * (val.count - 1) - 2, self.prograssPanel:get("proPanel2"):y())
	...
	for id = self.minId, self.minId + itertools.size(self.routeData) - 1 do
		if index == 0 then
			index = index + 1
			self.prograssData[index] = {
				percent = 0,
				prograssIndex = self.minId,
				round = id,
				panelX = panelSumX,
				spineX = spineSumX
			}
		...
		if self.routeData[id].count == itertools.size(self.routeData) then
			index = index + 1
			self.prograssData[index] = {
				percent = 100,
				prograssIndex = id + 1,
...
function HuntingRouteView:initEventSpine(flag)
	...
	if self.eventID == EVENT_TYPE.mul and self.boardId > 0 then
	...
	local function eventCb()
		...
		if self.eventID == EVENT_TYPE.gate then
			...
			performWithDelay(self, function()
				gGameApp:requestServer("/game/hunting/battle/info", function(tb)
					...
					if flag == 2 and panel then
						...
						local gatePanel = ccui.Layout:create():size(600, 800):addTo(self.circlePanel, 10, "gatePanel"):anchorPoint(0.5, 0.5):xy(x, y)

						gatePanel:setTouchEnabled(true)
						gatePanel:setRotation(rotation)

						local nodeSpine = widget.addAnimationByKey(self.circlePanel:get("gatePanel"), csv.unit[unitId].unitRes, EVENT_SPINE[self.eventID].spineName, EVENT_SPINE[self.eventID].action, 10):xy(300, 20):anchorPoint(0.5, 0):scale(EVENT_SPINE[self.eventID].scale):scaleX(EVENT_SPINE[self.eventID].scale * -1)

						nodeSpine:setSkin(csv.unit[unitId].skin)

						local iconBg = cc.Sprite:create("city/adventure/hunting/logo_sldd.png"):anchorPoint(0.5, 0.5):addTo(nodeSpine, 10, "iconBg"):xy(0, 40 + csv.unit[unitId].everyPos.headPos.y / 2):scale(1 / EVENT_SPINE[self.eventID].scale)
						local respurse = GATE_SPINE_TYPE[csv.cross.hunting.gate[self.routeData[self.curNode].gateIDs[1]].type]
						local iconPic = cc.Sprite:create(respurse.res):anchorPoint(0.5, 0.5):addTo(nodeSpine:get("iconBg"), 10, "icon"):xy(80, 100):scale(respurse.scale)

						if flag == 3 then
							local panel = self.circlePanel:get("gatePanel")
							local sequence = cc.Sequence:create(cc.MoveTo:create(actionTime - 1, cc.p(panel:x() - 700, panel:y() + 1530)))
			...
			local gatePanel = ccui.Layout:create():size(400, 500):addTo(self.circlePanel, 10, "eventPanel"):anchorPoint(0.5, 0):xy(x, y)
			...
			local nodeSpine = widget.addAnimationByKey(self.circlePanel:get("eventPanel"), EVENT_SPINE[self.eventID].res, EVENT_SPINE[self.eventID].spineName, EVENT_SPINE[self.eventID].action, 10):xy(200, 100):anchorPoint(0.5, 0):scale(EVENT_SPINE[self.eventID].scale):scaleX(EVENT_SPINE[self.eventID].scale * -1)

			if self.eventID == EVENT_TYPE.box then
				local iconBg = cc.Sprite:create("common/icon/icon_box_sldd.png"):anchorPoint(0.5, 0):addTo(nodeSpine, 10, "iconBg"):xy(0, 0):scale(1.6):scaleX(-1.6)
				...
				local iconBg = cc.Sprite:create("city/adventure/hunting/logo_sldd.png"):anchorPoint(0.5, 0.5):addTo(nodeSpine, 10, "iconBg"):xy(0, 120):scale(1 / EVENT_SPINE[self.eventID].scale)
			end

			if self.eventID == EVENT_TYPE.mul then
				local label = cc.Label:createWithTTF("!", "font/youmi1.ttf", 100):align(cc.p(1, 0), 40, 30):addTo(nodeSpine:get("iconBg"), 10, "icon"):xy(100, 50)

				text.addEffect(label, {
					color = cc.c4b(241, 59, 84, 255)
				})
			elseif self.eventID == EVENT_TYPE.HP then
				local iconPic = cc.Sprite:create("city/adventure/hunting/img_box_jy.png"):anchorPoint(0.5, 0.5):addTo(nodeSpine:get("iconBg"), 10, "icon"):xy(80, 110)

				if flag == 3 then
					local panel = self.circlePanel:get("eventPanel")
					local sequence = cc.Sequence:create(cc.MoveTo:create(actionTime - 1, cc.p(panel:x() - 750, panel:y() + 1400)))
	...
	if self.routeData[self.curNode].type == EVENT_TYPE.mul and panel then
		...
		local sequence = cc.Sequence:create(cc.MoveTo:create(self.delayTime, cc.p(panel:x() + 800, panel:y() - 1000)), cc.CallFunc:create(eventCb))
...
function HuntingRouteView:initPanel()
	if self.curNode > self.minId + itertools.size(self.routeData) - 1 then
		...
		local richText = rich.createByStr(normalContent, 36, nil):xy(self.rightBottomPanel:get("proText"):x(), self.rightBottomPanel:get("proText"):y()):anchorPoint(0.5, 0.5):addTo(self.rightBottomPanel, 100, "richText"):formatText()

		self.rightBottomPanel:get("clearance"):visible(false)
		self.rightBottomPanel:get("proBg"):visible(true)
		self.rightBottomPanel:get("proBg"):width(richText:width() + 20)
	end
end
...
function HuntingRouteView:checkCurIndex(type)
	...
	for k, v in ipairs(self.prograssData) do
		if self.curNode == self.minId + itertools.size(self.routeData) then
			...
			break
		elseif v.prograssIndex == self.curNode and v.round == self.curNode - 1 then
			curProIndex = k
...
function HuntingRouteView:updatePerformance()
	...
	local sequence = cc.Sequence:create(cc.MoveTo:create(actionTime, cc.p(self.prograssData[self.curProIndex + 1].spineX, self.runSpine:y())))

	self.runSpine:runAction(sequence)

	local sequence1 = cc.Sequence:create(cc.MoveTo:create(actionTime, cc.p(self.prograssData[self.curProIndex + 1].panelX, self.prograssPanel:y())))
	...
	performWithDelay(self, function()
		...
		if self.circlePanel:get("eventPanel") then
			bind.click(self, self.circlePanel:get("eventPanel"), {
				method = function()
					self:onNodeClick()
				end
			})
...
function HuntingRouteView:onNodeClick()
	...
	elseif self.eventID == EVENT_TYPE.box then
		gGameUI:stackUI("city.adventure.hunting.open_box", nil, {
			blackLayer = true,
			clickClose = true
		}, self.route, node, self:createHandler("onClientRouteRefresh"))
	elseif self.eventID == EVENT_TYPE.HP then
		local group = csv.cross.hunting.route[node].supplyGroup or 1

		gGameUI:stackUI("city.adventure.hunting.supply", nil, {
			blackLayer = true
		}, {
			route = self.route,
			node = node,
			group = group
		}, self:createHandler("onClientRouteRefresh"))
	elseif self.eventID == EVENT_TYPE.mul then
		gGameUI:stackUI("city.adventure.hunting.select_event", nil, {
			blackLayer = true
		}, node, self:createHandler("onClientRouteRefresh"))
	end
end
...
function HuntingRouteView:onEndClick()
	...
	local function cancelAndCloseCb()
		self.dialog = nil
...
function HuntingRouteView:onBuffBtnClick()
	...
	local sequence = cc.Sequence:create(cc.MoveTo:create(0.5, endPos), cc.CallFunc:create(function()
...
function HuntingRouteView:onItemClick(self, list, node, v)
	...
	gGameUI:showItemDetailCustom(node, v, name, {
		tipParams = tipParams
	})
end

function HuntingRouteView:oneKeyCheck()
	local routeInfo = gGameModel.hunting:read("hunting_route")
	local step = routeInfo[self.route].jump_step
	local info = routeInfo[self.route].jump_info

	if not dataEasy.isUnlock(gUnlockCsv.huntingJump) or not info or routeInfo[self.route].node ~= self.minId then
		return
	end

	if step == game.HUNTING_JUMP_STATE.OVER then
		return
	elseif step == game.HUNTING_JUMP_STATE.BEGIN then
		local function callbacks()
			gGameApp:requestServer("/game/hunting/jump/next", function(tb)
				gGameUI:stackUI("city.adventure.hunting.jump", nil, nil, {
					data = tb.view,
					route = self.route,
					cb = self:createHandler("onClientRouteRefresh")
				})
			end, self.route)
		end

		local function cancelAndCloseCb()
			gGameApp:requestServer("/game/hunting/jump/closed", function(tb)
				return
			end, self.route)
		end

		local lastMaxNode = routeInfo[self.route].last_max_node
		local hisMaxNode = routeInfo[self.route].history_max_node
		local lastCanPass = 0
		...
		local params = {
			...
			cb = callbacks,
			closeCb = cancelAndCloseCb,
			content = string.format(gLanguageCsv.huntingJumpTip, canPassNode, itertools.size(self.routeData)),
			dialogParams = {
				clickClose = false
			}
		}

		gGameUI:stackUI("common.prompt_box", nil, nil, params)
	elseif step == game.HUNTING_JUMP_STATE.POINT then
		gGameApp:requestServer("/game/hunting/jump/next", function(tb)
			gGameUI:stackUI("city.adventure.hunting.jump", nil, nil, {
				route = self.route,
				cb = self:createHandler("onClientRouteRefresh")
