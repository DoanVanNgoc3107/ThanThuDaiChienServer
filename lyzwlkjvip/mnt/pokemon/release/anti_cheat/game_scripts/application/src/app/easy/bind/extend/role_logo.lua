-- chunkname: @src.app.easy.bind.extend.role_logo
...
function roleLogo:showLogo(logoImg)
	...
	local mask = cc.Sprite:create("common/box/box_head_d.png"):alignCenter(size)
	local logoClipping = cc.ClippingNode:create(mask):setAlphaThreshold(0.1):size(size):alignCenter(size):addTo(self.panel, 3, "logoClipping")
	...
	helper.callOrWhen(self.logoId, function(logoId)
		...
		if logoId then
			...
			local logo = ccui.ImageView:create(res):alignCenter(size):scale(2):addTo(logoClipping, 3, "logo")
...
function roleLogo:showFrame()
	local size = self.panel:size()
	local frameImg = ccui.ImageView:create():alignCenter(size):addTo(self.panel, 4, "frame")
	...
	helper.callOrWhen(self.frameId, function(frameId)
		...
		if frameId == false then
			...
			if isSpine then
				...
				local frameSpine = widget.addAnimationByKey(self.panel, frameRes, "frameSpine", "effect_loop", 4):alignCenter(size)

				if self.isGray then
					frameSpine:setTimeScale(0)
					cache.setShader(frameSpine, false, "hsl_gray")
				end
			else
...
function roleLogo:showLevel()
	...
	levelImg:size(cc.size(72, 46)):align(cc.p(0.5, 0.5), 15, size.height - 5):addTo(self.panel, 5, "levelImg")

	local txtLevel = label.create(0, {
		fontSize = 40,
		color = ui.COLORS.NORMAL.WHITE
	}):xy(15, size.height - 5):setHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER):addTo(self.panel, 6, "level")
...
function roleLogo:showVip()
	local size = self.panel:size()
	local vipImg = ccui.ImageView:create():align(cc.p(0.5, 0.5), size.width / 2, -size.height * 0.1):addTo(self.panel, 7, "vip")
...
function roleLogo:initExtend()
	...
	local panel = ccui.Layout:create():size(size):alignCenter(self:size()):addTo(self, 1, "_roleLogo_")

	bottomImg:alignCenter(size):addTo(panel, 2, "bottom")
	...
	if self.onNodeClick then
		...
		bind.touch(self, panel, {
			methods = {
				ended = function(view, node, event)
