-- chunkname: @src.app.views.city.card.helditem.advance_detail
...
local function onInitItem(list, node, k, v)
	...
	local richText = rich.createWithWidth(color .. content, 40, nil, 920):anchorPoint(0, 1):x(25):addTo(node, 10, "attrText")
...
function HeldItemBreachView:onAfterBuild(list)
	...
	local offx = self.params.offx or 0
	local offx, offy = offx, self.params.offy or 0
