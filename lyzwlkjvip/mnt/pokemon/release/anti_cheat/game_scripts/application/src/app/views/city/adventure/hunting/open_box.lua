-- chunkname: @src.app.views.city.adventure.hunting.open_box
...
local function setEffect(parent, effectName, spineName, offy)
	...
	if not effect then
		widget.addAnimationByKey(parent, spineName, "effect", effectName, 0):xy(parent:width() / 2, parent:height() / 2 - offy):scale(2)
	else
...
function HuntingOpenBoxView:onCreate(route, node, cb)
	...
	idlereasy.any({
		...
		rich.createByStr(string.format(gLanguageCsv.canOpenTimes, timesColor .. subTimes .. "#C0xFFFCED#", self.boxLimit), 36):anchorPoint(0.5, 0.5):addTo(self.textOpenTimes, 6)
		...
		itertools.invoke({
			...
			self.textCost,
			self.imgCost
		}, (costIndex == 1 or count == self.boxLimit) and "hide" or "show")
		text.addEffect(self.textCost, {
			color = coinColor
		})
		adapt.oneLineCenterPos(cc.p(self.btnOpen:x(), self.textCost:y()), {
			self.textNote,
			self.textCost,
			self.imgCost
		}, cc.p(15, 0))
	end)
	adapt.setTextScaleWithWidth(self.textTip, nil, 700)
end
...
function HuntingOpenBoxView:onBtnOpen()
	...
	local function cb()
		...
		gGameApp:requestServerCustom("/game/hunting/box/open"):params(self.route, self.node):onResponse(function(tb)
