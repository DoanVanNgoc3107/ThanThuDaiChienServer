-- chunkname: @src.app.views.city.activity.world_boss.reward
		...
		binds = {
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					uiEasy.createItemsToList(list, childs.list, v.cfg.award, {
						margin = 40,
						scale = 0.9
					})
				end
...
function WorldBossRewardView:onCreate(activityID)
	...
	for k, v in orderCsvPairs(rankCsv) do
		if v.huodongID == yyCfg.huodongID then
			table.insert(roleRankAward, {
				cfg = v,
				csvId = k,
				preRank = rankCsv[k - 1] and rankCsv[k - 1].rank + 1 or 1
			})
		end
	end
