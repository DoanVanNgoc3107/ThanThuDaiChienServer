-- chunkname: @src.app.views.city.adventure.gym_challenge.cross_gate
...
function GymCrossGate:initMember()
	...
	idlereasy.any({
		self.crossAlGatesDatas,
		self.inCd,
		self.round
	}, function(_, _crossGatesDatas, inCd, round)
		...
		for i = 1, 8 do
			...
			if i == 1 then
				text.addEffect(panel:get("textTime"), {
					outline = {
						size = 2,
			...
			if data then
				...
				text.addEffect(childs.textFight, {
					outline = {
						size = 2,
						color = cc.c4b(255, 252, 237, 255)
					}
				})
				text.addEffect(childs.textLv, {
					outline = {
						size = 2,
						color = cc.c4b(79, 72, 79, 255)
					}
				})
				text.addEffect(childs.textName, {
					outline = {
						size = 2,
						color = cc.c4b(79, 72, 79, 255)
					}
				})
				text.addEffect(childs.textSever, {
					outline = {
						size = 2,
						color = cc.c4b(79, 72, 79, 255)
					}
				...
				childs.textLv:text(gLanguageCsv.textLv .. data.level)
				...
				if i == 1 then
					adapt.oneLineCenterPos(cc.p(150, childs.textLv:y()), {
						childs.textLv,
						childs.textName
					}, cc.p(6, 8))
					adapt.oneLineCenterPos(cc.p(150, childs.textSever:y()), {
						childs.txt,
						childs.textFight,
						childs.textSever
					}, {
				...
				if data.figure ~= "" then
					...
					widget.addAnimationByKey(childs.figurePanel, figureCfg.resSpine, "figure", "standby_loop1", 1):xy(size.width / 2, 0):scale(i == 1 and 1 or 0.9)
					bind.touch(self, childs.figurePanel, {
						methods = {
							ended = function()
...
function GymCrossGate:onBtnChallenge(k, data)
	...
	local function fightCb(view, battleCards, extra, aidCards)
		...
		battleEntrance.battleRequest("/game/cross/gym/battle/start", cardDatas, self.id, k, data.game_key, data.record_id, extra, aidCards:read()):onStartOK(function(data)
			view:onClose(false)
		end):run():show()
	end
...
function GymCrossGate:onBtnOccupy(k)
	...
	local function saveCb(view, clientbattleCards, battleCards, bCloseView, extra, aidCards)
		...
		end, cardDatas, self.id, k, extra, aidCards)
	end
