-- chunkname: @src.app.easy.bind.extend.chips_panel
	...
	helper.callOrWhen(self.selected, function(selected)
		for i = 1, 6 do
			...
			if itemSelected then
				...
				itemSelectedEffect = itemSelectedEffect or widget.addAnimation(item, "chip/xzk.skel", "effect_loop", 100):scale(itemSelected:scale()):xy(itemSelected:xy()):rotate(60 * (i - 1))
...
function chipsPanel:calcSuitAttr()
	...
	for idx, count in pairs(flags) do
		...
		effect = effect or widget.addAnimationByKey(item, "chip/fushi.skel", "effect_chacao", effectName, 100):scale(2):rotate(60 + 60 * idx):xy(item:width() / 2, item:height() / 2)
	...
	if newSuitActive then
		...
		suitImg = suitImg or cc.Sprite:create("city/card/chip/txt_tzjh.png"):xy(400, 300):addTo(self, 100, "suitImg_")

		suitImg:stopAllActions()
		suitImg:scale(0)
		suitImg:opacity(255)
		transition.executeSequence(suitImg):easeBegin("ELASTICOUT", 0.7):scaleTo(0.5, 1):easeEnd():delay(0.5):fadeOut(0.5):done()
	end
...
function chipsPanel:onItem_(k, dbId)
	...
	if not effect then
		...
		effect = widget.addAnimationByKey(item, "chip/fushi.skel", "effect_line", "effect_xian_di_loop", -2):scale(2 * scale):rotate(60 * (k - 2)):xy(pos)
	end
	...
	if dbId then
		...
		local lv = cc.Label:createWithTTF(gLanguageCsv.textLv2 .. chipData.level, ui.FONT_PATH, 30 - math.floor(10 * (1 - scale))):align(cc.p(0.5, 0), item:width() / 2, item:height() / 10):addTo(item, 6, "defaultLv")
