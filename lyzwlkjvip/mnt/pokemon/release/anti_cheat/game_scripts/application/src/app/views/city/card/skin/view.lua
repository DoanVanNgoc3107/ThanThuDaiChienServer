-- chunkname: @src.app.views.city.card.skin.view
...
function CardSkinView:onCreate(cardDbid)
	...
	uiEasy.setIconName("card", self.cardId:read(), {
		space = true,
		node = self.cardNameTxt,
		name = self.cardName:read(),
	...
	local id = userDefault.getForeverLocalKey("skinShowType", 0, {
		rawData = true,
		rawKey = true
	})
	...
	idlereasy.when(self.showTp, function(_, tpye)
		...
		self:setLeftPanelInfo(self.skinDatas[self.selectID:read() + 1].unitId)
	...
	idlereasy.when(self.cardActionState, function(_, cardState)
		...
		self.cardSprite:addTo(self.heroNode, 5):xy(size.width / 2, 0)
...
function CardSkinView:setSkinList(id, info)
	...
	local unitCfg = csv.unit[self.unitId:read()]
	...
	skinDatas[2] = {
		...
		id = 0,
		isHas = 0,
		icon = unitCfg.cardShow,
		scale = unitCfg.cardShowScale,
		posOffset = unitCfg.cardShowPosC,
...
function CardSkinView:initCell(cell, v, num)
	...
	local clippingNode = cc.ClippingNode:create(mask):setAlphaThreshold(0.1):size(size):alignCenter(cell:size()):add(sp):addTo(cell, 5, "clipping")
	...
	local function addCardSpine(spineRes, spineTag)
		return widget.addAnimationByKey(cell, spineRes, spineTag, "effect_loop", 5):setAnchorPoint(cc.p(0.5, 0.5)):xy(cell:width() / 2 + unitCfg.cardSpinePos.x, cell:height() / 2 + unitCfg.cardSpinePos.y):scale(unitCfg.cardSpineScale)
	end

	cell:removeChildByName("cardSkinListSpine")
	sp:show()

	if unitCfg.cardSpine and (v.skinType ~= 1 or self.star:read() >= unitCfg.cardSpineNeedStar) then
		sp:hide()
		addCardSpine(unitCfg.cardSpine, "cardSkinListSpine")
	end

	sp:removeChildByName("shinySpine")

	if unitCfg.shinyRes then
		widget.addAnimationByKey(sp, unitCfg.shinyRes, "shinySpine", "effect_loop", 4):setAnchorPoint(cc.p(0.5, 0.5)):xy(rect.width / 2 + unitCfg.shinyPos.x, ssize.height / 2 + unitCfg.shinyPos.y)
	end

	bind.touch(self, cell, {
		clicksafe = true,
		scaletype = 0,
		methods = {
			ended = function()
	...
	if v.extraItem and csvSize(v.extraItem) == 1 then
		...
		imgIcon:alignCenter(imgIcon:size()):scale(1.2):xy(cc.p(80, 80)):addTo(childs.imageAdd)
	end
end
...
function CardSkinView:setLeftPanelInfo(unitId)
	...
	self.cardSprite = widget.addAnimation(self.heroNode, unitCfg.unitRes, "standby_loop", 5):xy(size.width / 2, 0)
	...
	self.cardSpineNode:visible(not sign)
	self.imgIcon:texture(unitCfg.cardShow)
	self.cardActionState:set(3, true)

	local function addCardSpine(spineRes, spineTag)
		widget.addAnimationByKey(self.cardSpineNode, spineRes, spineTag, "effect_loop", 5):setAnchorPoint(cc.p(0.5, 0.5)):xy(self.imgIconBg:width() / 2 + unitCfg.cardSkinPos.x, self.imgIconBg:height() / 2 + unitCfg.cardSkinPos.y):scale(unitCfg.cardSkinScale)
	end

	local cardSkinPos = unitCfg.cardSkinPos

	self.cardSpineNode:removeChildByName("cardSpine")
	self.imgIcon:removeChildByName("shinySpine")

	local skinType = self.skinDatas[self.selectID:read() + 1].skinType

	if not sign then
		self.imgIcon:show()

		if unitCfg.cardSpine and (skinType ~= 1 or self.star:read() >= unitCfg.cardSpineNeedStar) then
			self.imgIcon:hide()
			addCardSpine(unitCfg.cardSpine, "cardSpine")
		end

		if unitCfg.shinyRes then
			self.imgIcon:show()
			widget.addAnimationByKey(self.imgIcon, unitCfg.shinyRes, "shinySpine", "effect_loop", 4):alignCenter(self.imgIcon:size())
		end
	end
end
...
function CardSkinView:setSkinInfo(id)
	...
	self.panelRight:removeChildByName("extraTip")
	self.panelRight:removeChildByName("sign")
	...
	local x, y = self.skinNoAdd:xy()

	if #attrDatas > 0 then
		self.skinNativeDatas:update(attrDatas)

		local y = self.infoList:y() + self.infoList:height() - self.infoList:size().height
		local descObj = self.panelNature:get("desc")
	...
	if skinCsv.extraTip ~= "" then
		rich.createByStr(skinCsv.extraTip, 40):anchorPoint(0.5, 0.5):xy(x, y):addTo(self.panelRight, 1, "extraTip")
	end

	if skinCsv.sign ~= "" then
		rich.createByStr("#C0x5B545B#" .. skinCsv.sign, 50):anchorPoint(0.5, 0.5):xy(x, 1140):addTo(self.panelRight, 1, "sign")
	end
end
...
function CardSkinView:setButtonInfo(val)
	...
	elseif csvSize(val.costMap) > 0 then
		...
		for index, value in csvMapPairs(val.costMap) do
			if value > dataEasy.getNumByKey(index) then
				sign = false
			end
		...
		local richText = rich.createByStr(str, 40):addTo(self.panelRight, 10, "privilege"):anchorPoint(1, 0.5):xy(cc.p(x - size.width / 2 - 10, y)):formatText()
	else
...
function CardSkinView:onButtonBuy()
	...
	if num > dataEasy.getNumByKey(key) then
		sign = false
	end
...
function CardSkinView:onClose()
	Dialog.onClose(self)
	userDefault.setForeverLocalKey("skinShowType", self.showTp:read(), {
		rawKey = true
	})
end
