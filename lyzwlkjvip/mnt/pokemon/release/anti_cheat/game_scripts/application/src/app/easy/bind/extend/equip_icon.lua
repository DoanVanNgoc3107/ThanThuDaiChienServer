-- chunkname: @src.app.easy.bind.extend.equip_icon
...
function equipIcon:initExtend()
	...
	local panel = ccui.Layout:create():size(198, 198):addTo(self, 1, "_equip_")
	local imgBG = ccui.ImageView:create(ui.QUALITY_BOX[1]):alignCenter(panelSize):addTo(panel, 1)
	local imgSel = ccui.ImageView:create("common/box/box_selected.png"):alignCenter(panelSize):visible(self.selected):addTo(panel, -1, "imgSel")
	local imgArrow = ccui.ImageView:create("common/icon/icon_up.png"):alignCenter(panelSize):xy(150, 150):visible(true):addTo(panel, 5, "imgArrow")

	self.panel = panel

	local size = panel:size()
	local frameNode = ccui.ImageView:create():xy(30, 99):addTo(panel, 3, "frame")
	local icon = ccui.ImageView:create():alignCenter(panelSize):scale(2):addTo(panel, 2, "icon")
	local labelLv = panel:get("txtLv")

	if not labelLv then
		labelLv = cc.Label:createWithTTF(gLanguageCsv.textLv1, ui.FONT_PATH, 24):align(cc.p(0, 0), 90, 30):addTo(panel, 2, "txtLv")
	...
	if not txtLvNum then
		txtLvNum = cc.Label:createWithTTF("", ui.FONT_PATH, 30):align(cc.p(0, 0), 25, 30):addTo(panel, 2, "txtLvNum")
	...
	helper.callOrWhen(self.data, function(data)
		...
		adapt.oneLinePos(txtLvNum, labelLv, cc.p(0, 0), "left")
...
function equipIcon:setStar(panel, star)
	...
	ccui.Layout:create():size(size.width, 70):align(cc.p(0, 0), 0, 0):addTo(panel, 4, "star")
	panel:get("star"):removeAllChildren()

	for i = 1, star do
		ccui.ImageView:create("city/card/equip/icon_star.png"):xy(99 - 12 * (star + 1 - 2 * i), 25):addTo(panel:get("star"), 4, "star"):scale(0.8)
	end
end
...
function equipIcon:setAbility(panel, ability)
	...
	ccui.Layout:create():size(size.width, 70):align(cc.p(0, 0), 0, 0):addTo(panel, 4, "star")
	panel:get("star"):removeAllChildren()
	ccui.ImageView:create("city/card/equip/icon_xx_d.png"):xy(100, 25):addTo(panel:get("star"), 4, "starbg"):scale(0.8)

	local abilityNum = cc.Label:createWithTTF(ability, "font/youmi1.ttf", 30):align(cc.p(1, 0), x, 5):addTo(panel:get("star"), 4, "starnum")

	text.addEffect(abilityNum, {
		outline = {
			size = 2,
...
function equipIcon:setAwakeInfo(panel, awakeLv)
	...
	if not awakePanel then
		awakePanel = ccui.Layout:create():size(128, 48):anchorPoint(0.5, 0.5):xy(50, size.height - 18):addTo(panel, 6, "_awakeInfo")

		ccui.ImageView:create("city/card/equip/logo_jxbs.png"):xy(80, 24):addTo(awakePanel, 1)
		cc.Label:createWithTTF("", "font/youmi1.ttf", 34):color(cc.c4b(252, 249, 203, 255)):xy(80, 24):addTo(awakePanel, 2, "_awakeStr")
	end
	...
	adapt.setTextScaleWithWidth(label, nil, 100)
end
...
function equipIcon:setAwakeAbilityInfo(panel, awakeAbilityLv)
	...
	if not awakeAbilityPanel then
		awakeAbilityPanel = ccui.Layout:create():size(128, 48):anchorPoint(0.5, 0.5):xy(50, size.height - 18):addTo(panel, 6, "_awakeAbilityInfo")

		local vertical = ccui.Scale9Sprite:create()

		vertical:initWithFile(cc.rect(36, 24, 1, 1), "city/card/equip/box_spjx_d.png")
		vertical:size(150, 50):anchorPoint(0.5, 0.5):xy(90, 24):addTo(awakeAbilityPanel)
		cc.Label:createWithTTF("", "font/youmi1.ttf", 34):color(cc.c4b(252, 249, 203, 255)):xy(85, 24):addTo(awakeAbilityPanel, 2, "_awakeAbilityStr")
	end
	...
	adapt.setTextScaleWithWidth(label, nil, 140)
end
