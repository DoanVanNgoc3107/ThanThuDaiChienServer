-- chunkname: @src.app.views.city.adventure.gym_challenge.buff_detail
...
function GymBuffDetail:onCreate(id, unlocked, preLv, needPerLv)
	...
	idlereasy.when(self.gymDatas, function(_, gymDatas)
		...
		adapt.setTextAdaptWithSize(self.textDesc, {
			vertical = "center",
			horizontal = "left",
			size = cc.size(740, 160)
		})
		adapt.oneLineCenterPos(cc.p(self.imgIcon:x(), 607), {
			self.textNoteLv,
			self.textLv
		})
		...
		elseif lv >= cfg.levelUp then
			...
			self.imgLockBg:hide()
			adapt.oneLineCenterPos(cc.p(self.btnAct:x(), self.textNoteCost:y()), {
				self.textNoteCost,
				self.textCost,
				self.imgCost
			}, {
		...
		adapt.setTextScaleWithWidth(self.lockPanel:get("textNode1"), nil, 220)
		adapt.setTextScaleWithWidth(self.lockPanel:get("textNote3"), nil, 660)
		adapt.setTextScaleWithWidth(self.btnAct:get("textNote"), nil, 240)
		adapt.oneLinePos(self.lockPanel:get("textNote3"), self.lockPanel:get("textLevel"))
	end)
end
...
function GymBuffDetail:onActClick()
	...
	if cost > self.gymDatas:read().gym_talent_point then
	...
	gGameApp:requestServer("/game/gym/talent/level/up", function(tb)
		self.imgTextBg:show():y(620):stopAllActions():setOpacity(255)
		self.imgTextBg:get("imgText"):stopAllActions():setOpacity(255)
		...
		transition.executeSequence(self.imgTextBg):moveBy(delay3, 0, 100):delay(delay1):moveBy(delay2, 0, 100):hide():done()
		transition.executeSequence(self.imgTextBg):delay(delay1 + delay3):fadeOut(delay2):done()
		transition.executeSequence(self.imgTextBg:get("imgText")):delay(delay1 + delay3):fadeOut(delay2):done()
	end, self.id)
end
...
function GymBuffDetail:onDetailClick(target)
	...
	gGameUI:showItemDetailCustom(target, {
		width = 420,
		content = content
	}, "common.text", {
