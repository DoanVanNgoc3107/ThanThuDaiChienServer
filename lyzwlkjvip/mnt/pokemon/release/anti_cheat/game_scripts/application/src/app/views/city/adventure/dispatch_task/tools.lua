-- chunkname: @src.app.views.city.adventure.dispatch_task.tools
...
local function createRichTxt(str, parent)
	...
	local richText = rich.createByStr(str, fontSize, nil, nil, cc.p(0, 0.5)):anchorPoint(0, 0.5):addTo(parent, 6)
...
local function setExtraCondition1(txtNode, condition1Num, csvTask, status)
	...
	if status ~= 2 then
		local color = num <= condition1Num and "#C0x60C456#" or "#C0xF76B45#"
...
local function setExtraCondition2(childs, cardNatures, attrItem, taskData, condition1Num, csvTask)
	...
	setConditionText(childs.extraCondition2, "", attrNum <= attrState, taskData.status)
	...
	if gainChance == 0 then
		...
		gainChance = attrState / attrNum * csvTask.cardNatureRate + 0.05 + (num <= condition1Num and csvTask.rate1 or 0)
	end
	...
	adapt.setTextScaleWithWidth(childs.gainChance, nil, 370)
end

function dispatchtaskTools.setItemCondition(conditionPanel, v, attrItem, typ)
	local childs = conditionPanel:multiget("condition1", "condition2", "extraCondition1", "extraCondition2", "attrList", "gainChance")
	local cardNums = 0
	local fight = 0
	local cardNatures = {}
	local csvTask = v.cfg
	...
	if typ ~= "main" then
		...
		for i, dbid in ipairs(v.cardIDs) do
			...
			if level <= cardData[CONDITION_TYPE[csvTask.condition1].name] then
		...
		local hash = itertools.map(v.taskData.reach_natures or {}, function(k, v)
			return v, k
		end)
		...
		bind.extend(list, panel, {
			...
			props = {
				...
				onNode = function(node)
					...
					local num = cc.Label:createWithTTF(v.num, ui.FONT_PATH, 32):align(cc.p(1, 0), size.width / 2 + box.width / 2 - 30 * scale, size.height - box.height + 14 * scale):addTo(panel, 10, "num")
