-- chunkname: @src.app.views.city.activity.dispatch.view
...
function DispatchView:onBtnRuleClick()
	gGameUI:stackUI("common.rule", nil, {}, self:createHandler("getRuleContext"), {
		width = 1000
	})
end
...
function DispatchView:onCreate(activityId, data)
	...
	gGameUI.topuiManager:createView("activity_dispatch", self, {
		onClose = self:createHandler("onClose")
	}):init({
		subTitle = "DISPATCH",
		title = gLanguageCsv.activityDispatch,
		actionPointKey = actionPointKey
	})
	self:initMap()
	self:initModel()
...
function DispatchView:initModel()
	...
	idlereasy.when(self.yyhuodongs, function(_, yyhuodongs)
		...
		for taskId, v in pairs(dispatch) do
			...
			if dispatch[taskId].times >= csv.yunying.dispatch[taskId].times then
			...
			elseif v.status == 1 then
				...
				if time.getTime() > dispatch[taskId].end_time then
					...
					widget.addAnimationByKey(item, "effect/guanqiabaoxiang.skel", "rewardEffect", "effect_loop", 2):xy(100, 200):scale(0.5)
				else
...
function DispatchView:initMap()
	...
	for k, v in pairs(self.gameCfg) do
		local item = self.item:clone():addTo(self.scrollView):show():xy(v.data.position):z(10)

		self.gameCfg[k].item = item

		item:get("lockPanel"):show()
		item:get("lockPanel.text"):text(v.data.numText)
		bind.touch(self.scrollView, item, {
			methods = {
				ended = functools.partial(self.clickGateItem, self, v.id)
			}
		})
...
function DispatchView:countDown(panel, endTime, taskId)
	local function setLabel()
		...
		if endTime - time.getTime() <= 0 then
			...
			self:setSelEffect(self.selEffectId)
