-- chunkname: @src.app.easy.bind.extend.multi_text_effect
...
function multiTextEffect:initExtend()
	...
	local panel = ccui.Layout:create():size(cc.size(0, 0)):addTo(self, 1)
	...
	for i = 1, count do
		local obj = label.create(" ", self.labelParams):addTo(panel, 100 - i, i)

		helper.callOrWhen(self.data, functools.partial(obj.text, obj))

		if self.effects then
			bind.effect(self, obj, {
				data = self.effects[i]
			})
		else
