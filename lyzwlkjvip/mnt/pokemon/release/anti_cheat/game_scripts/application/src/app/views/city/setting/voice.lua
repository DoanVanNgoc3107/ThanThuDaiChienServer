-- chunkname: @src.app.views.city.setting.voice
			...
			props = {
				...
				onItem = function(list, node, k, v)
					...
					bind.touch(list, node, {
						methods = {
							ended = functools.partial(list.playMusic, k, v)
						}
					})
...
function SettingVoiceView:onCreate()
	...
	setSlider(self.bgVoiceState, self.bgSlider, self.itemBgVoiceBtn, function(val)
		...
		userDefault.setForeverLocalKey("musicVolume", val, {
			rawKey = true
		})
	end, function()
	...
	setSlider(self.battleVoiceState, self.btSlider, self.itemBattleVoiceBtn, function(val)
		...
		userDefault.setForeverLocalKey("effectVolume", val, {
			rawKey = true
		})
	end, function()
	...
	for k, v in orderCsvPairs(csv.citysound) do
		table.insert(listData, {
			inTest = false,
			cfg = v,
			selected = curMusicIdx == k
		})
	end

	self.listData = idlers.newWithMap(listData)

	adapt.setTextAdaptWithSize(self.bottomText, {
		vertical = "center",
		horizontal = "center",
		size = cc.size(1300, 70)
	})
end
