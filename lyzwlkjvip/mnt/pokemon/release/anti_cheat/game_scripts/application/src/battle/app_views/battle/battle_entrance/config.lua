...
function battleEntrance.preloadConfig()
	...
	for _, name in ipairs(PreloadCsv) do
		if name == 'csv.skill_process' then
			local skill_process_2 = require "config.skill_process_2"
			local skill_process = require "config.skill_process"
			csvAppendNoConflict(skill_process, skill_process_2, true)
		elseif name == 'csv.buff' then
			local buff_2 = require "config.buff_2"
			local buff_3 = require "config.buff_3"
			local buff_4 = require "config.buff_4"
			local buff = require "config.buff"
			csvAppendNoConflict(buff, buff_2, true)
			csvAppendNoConflict(buff, buff_3, true)
			csvAppendNoConflict(buff, buff_4, true)
		end
	end
