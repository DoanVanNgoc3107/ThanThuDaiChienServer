-- chunkname: @src.app.views.city.setting.view
...
function SettingView:onCreate(params)
	...
	self.curBtn:addListener(function(val, oldval)
		for btn, tb in pairs(self.btns) do
			...
			local anchorPointX = state and 0.7 or 0.5
			...
			if not tb.view then
				tb.view = gGameUI:createView(tb.name, self:getResourceNode()):init(self.params):x(display.uiOrigin.x)
			end
