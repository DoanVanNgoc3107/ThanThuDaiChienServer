...
TestServerConf = {
	'pokemon01': servconf('1区', '/home/pokemon_01/pokemon_server', '/home/pokemon_01/deploy', ['game_server',], 'game.dev.1',),
	'pokemon02': servconf('2区', '/home/pokemon_02/server', '/home/pokemon_02/deploy', ['game_server',], 'game.dev.2',),
	'pokemon03': servconf('3区', '/home/pokemon_03/server', '/home/pokemon_03/deploy', ['game_server',], 'game.dev.3',),
	'pokemon04': servconf('4区', '/home/pokemon_04/server', '/home/pokemon_04/deploy', ['game_server',], 'game.dev.4',),
	'pokemon05': servconf('5区', '/home/pokemon_05/server', '/home/pokemon_05/deploy', ['game_server',], 'game.dev.5',),
	'pokemon06': servconf('6区', '/home/pokemon_06/server', '/home/pokemon_06/deploy', ['game_server',], 'game.dev.6',),
}
...
def getServerOpenDate():
	...
	for cfg in TestServerConf:
		...
		sys.path = oldPath
		# sys.path.append(os.path.join(TestServerConf[cfg].source, 'src')) # 防止defines模块中有导入其他模块
...
class TestYYHuoDongSocketHandler(tornado.websocket.WebSocketHandler):
	...
	def start_cache(cls, nowDT, days):
		...
		cls.firstStart = True
		TestYYHuoDongSocketHandler.send_updates({"test": ["重启完毕，开始测试"]})
		cls.timer.start()
	...
	def stop_cache(cls):
		...
		cls.yyCache = {}
		cls.timer.stop()
		TestYYHuoDongSocketHandler.send_updates({"test": ["测试完毕"]})
		...
		while cls.firstStart:
			for cfg in TestServerConfigs:
				...
				if log and cfg['name'] not in starts:
					TestYYHuoDongSocketHandler.send_updates({"test": [cfg['name'] + "初始化完毕"]})
					starts[cfg['name']] = True
			if len(starts) == len(TestServerConfigs):
				TestYYHuoDongSocketHandler.send_updates({"test": ["服务器初始化完毕"]})
				break
			else:
				yield sleep(1)
		if cls.nowDT.hour >= 5:
...
class TestFightSocketHandler(tornado.websocket.WebSocketHandler):
	...
	def start_cache(cls, servNode):
		...
		cls.timer = tornado.ioloop.PeriodicCallback(cls.update, 2*1000)
		cls.timer.start()
		TestFightSocketHandler.send_updates({"test": ["开始测试"]})
