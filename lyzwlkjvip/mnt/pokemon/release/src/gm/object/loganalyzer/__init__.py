...
def onLoginAdapterLine(mongo, logger, accountMongo, line):
	# login_server [login.cn.1 I 190820 16:53:04.289.059 enter_server.go:31] `tc_8252422` login in server 1 `game.cn.1` `皮卡丘`
	try:
		line = preHandle(line)
		head = handleLineHead(line)
		if head is None:
...
def onModifiedFile(mongo, logger, path):
	# 分析文件path： /var/log/192.168.1.98/game_server/2019-12-14.log
