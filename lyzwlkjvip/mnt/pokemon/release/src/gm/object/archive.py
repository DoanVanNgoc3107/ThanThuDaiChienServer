...
# unique
# date 日期 int 20170213
# channel 渠道 str tc
# sub_channel 子渠道 str ys
# area 区服 int 1
##########
# created 新增账号 list [account_id]
# login 登录账号 list [account_id]
# pay 付费账号 dict {account_id: {PayOrder.id: PayOrder.Model}}
# pay_orders list [{},]
	...
	def initSum(self):
		...
		try:
			self._sum = AttrsDict({
				...
				'pay_amount': sum([sumRecharges(d, self.language) for d in self.pay_orders.itervalues()]), # 结算货币按本地计算
