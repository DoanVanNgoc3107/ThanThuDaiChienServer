...
class Server(object):
	...
	RpcMinPermission = {
		...
		'gmStartCrossService': Admin,  # 跨服服务需要管理员权限
		...
		'gmRoleAbandon': P2,
		'gmKickPlayer': P2,
		'gmKickAllPlayers': Admin,  # 踢所有人需要管理员权限
		'gmSendMessage': P2,
		'gmSendMail': P2,
		'gmSetGameYYComfig': P2,
		# 内存数据查看器
		'gmGetPlayerMemoryData': P3,
		'gmGetPlayerDBData': P3,
		'gmComparePlayerData': P3,
		'gmForceSavePlayer': P2,
		'gmScanDataAnomalies': P2,  # 数据异常扫描
	...
	def __init__(self, name):
		...
		for dep in self.cfg['dependent']:
			if dep not in self.container.services:
				...
				logger.info('Added service to container: %s (type: %s)', dep, type(client).__name__)
		...
		# 跨服服务客户端字典 (按服务key存储)
		self.crossServices = {}
		# [拟态对战] 拟态通过 crossmatch 触发，所以需要收集 crossmatch
		crossPatterns = ['^crossarena', '^crosscraft', '^crossgym', '^crossmine', '^onlinefight', '^crossunionfight', '^crosschat', '^crossmatch']
		for pattern in crossPatterns:
			services = self.container.getservices(pattern)
			logger.info('Cross service pattern %s found %d services: %s', pattern, len(services), [s.name for s in services])
			for svc in services:
				self.crossServices[svc.name] = svc
		logger.info('Total cross services: %s', self.crossServices.keys())
