...
class ObjectItemsMap(ObjectBase):
	...
	def isExpire(self, itemID):
		items_cfg = getattr(csv, 'items', None)
		if items_cfg is None:
			logger.warning('items csv missing')
			return False
		try:
			cfg = items_cfg.get(itemID) if isinstance(items_cfg, dict) else getattr(items_cfg, 'dic', {}).get(itemID)
		except Exception:
			cfg = None
		if not cfg:
			logger.warning('item cfg missing for itemID %s', itemID)
			return False
	...
	def checkRegain(self, itemID, count):
		...
		items_cfg = csv.items
		stackMax = items_cfg[itemID].stackMax
...
#
# AutoChessDrawBoxItemEffect
#

class AutoChessDrawBoxItemEffect(GiftItemEffect):
	'''
	自走棋抽卡宝箱道具 (type=22)
	通过 drawItemType 字段指定抽卡配置：
	- 如果是字符串：通过 draw_version.csv 映射到 draw_item.csv 的 drawType
	- 如果是数字：直接使用 draw_items_lib.csv 的库ID
	'''
	Type = ItemDefs.autoChessDrawBoxType
	
	def __init__(self, game, itemID, count):
		ObjectItemEffect.__init__(self, game, itemID, count)
		
		csvItem = csv.items[itemID]
		drawItemType = getattr(csvItem, 'drawItemType', None)
		
		if not drawItemType:
			raise ServerError('AutoChessDrawBox item %s has no drawItemType' % itemID)
		
		argsD = {}
		
		# 如果是字符串，通过 draw_version 映射到 draw_item 的 drawType
		if isinstance(drawItemType, str):
			from game.object.game.lottery import ObjectDrawItemRandom
			
			# 使用 ObjectDrawItemRandom 的抽卡系统
			# drawItemType 作为 drawType，执行抽卡
			for _ in xrange(self._count):
				drawEffect = ObjectDrawItemRandom.getRandomItems(game, drawItemType, 1, None)
				
				if drawEffect:
					# ObjectDrawEffect.result 返回的是 {'items': [(id, count), ...]}
					# 我们需要从 _grids 中提取道具信息
					if hasattr(drawEffect, '_grids'):
						for grid in drawEffect._grids:
							if len(grid) >= 2:
								itemID, itemCount = grid[0], grid[1]
								argsD[itemID] = argsD.get(itemID, 0) + itemCount
					
					# 同时处理卡牌
					if hasattr(drawEffect, '_newCardsL') and drawEffect._newCardsL:
						if 'cards' not in argsD:
							argsD['cards'] = []
						argsD['cards'].extend([card['id'] for card in drawEffect._newCardsL])
		else:
			# 如果是数字，直接当作 draw_items_lib 库ID使用
			from game.object.game.lottery import ObjectDrawRandomItem
			
			for i in xrange(self._count):
				itemObj = ObjectDrawRandomItem.getObject(drawItemType)
				if itemObj is None:
					raise ServerError('AutoChessDrawBox libID %s not found in draw_items_lib.csv' % drawItemType)
				itemT = itemObj.getRandomItem()
				ObjectDrawRandomItem.packToDict(itemT, argsD)
...
class ObjectItemEffectFactory(ReloadHooker):
	...
	def classInit(cls):
		cls.ItemEffectMap = {
			...
			AutoChessDrawBoxItemEffect.Type: AutoChessDrawBoxItemEffect,
		}
