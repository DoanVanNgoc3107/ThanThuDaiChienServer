...
class ObjectMessageGlobal(ReloadHooker):
	...
	CrossQue = None # 跨服聊天消息
	OfficialQue = None # 官方消息
	...
	def __init__(self, servKey, onChat):
		...
		ObjectMessageGlobal.CrossQue = deque(maxlen=ChatMessageMax)
		ObjectMessageGlobal.OfficialQue = deque(maxlen=ChatMessageMax)
	...
	@classmethod
	def chatCrossMsg(cls, game, msg, type=MessageDefs.CrossChatType, args=None):
		'''跨服聊天消息'''
		cls.MsgID += 1
		role = game.role.chatRoleModel
		# 添加服务器标识
		role['game_key'] = cls.ServerKey
		model = Msg(cls.MsgID, nowtime_t(), msg, type, role, args)
		cls.CrossQue.append(model)
		if type == MessageDefs.CrossChatType:
			cls.OnChat('cross', model)
		return model

	@classmethod
	def officialMsg(cls, msg, type=MessageDefs.OfficialType, args=None):
		'''官方消息'''
		cls.MsgID += 1
		model = Msg(cls.MsgID, nowtime_t(), msg, type, None, args)
		cls.OfficialQue.append(model)
		return model
	...
	def modelSync(cls, msgID, game):
		...
		# 同步跨服聊天
		msgs = cls._listNewMsg(msgID, cls.CrossQue)
		if msgs:
			ret['cross'] = msgs

		# 同步官方消息
		msgs = cls._listNewMsg(msgID, cls.OfficialQue)
		if msgs:
			ret['official'] = msgs
	...
	@classmethod
	def crossCardShareMsg(cls, game, card):
		card.display()
		cls.chatCrossMsg(game, L10nDefs.NewsCardShare.format(name=game.role.name, card=card.name), MessageDefs.CrossCardShareType, args={'card1': card.id})
	...
	@classmethod
	def crossShareBattleMsg(cls, game, battleID, enemyName, src, crossKey):
		'''跨服战报分享（本地回退用）'''
		whereName = cls.whereNameShareBattleMsg(src)
		if crossKey:
			argsD = {'battleID': battleID, 'from': src, 'crossKey': crossKey}
			...
			argsD = {'battleID': battleID, 'from': src}
		cls.chatCrossMsg(game, L10nDefs.battleShare.format(where=whereName, enemy=enemyName), MessageDefs.CrossBattleShareType, args=argsD)
