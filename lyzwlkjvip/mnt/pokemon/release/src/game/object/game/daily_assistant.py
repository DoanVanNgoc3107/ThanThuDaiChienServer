...
from game.object import DailyAssistantDefs, FeatureDefs, DrawCardDefs, DrawEquipDefs, DrawItemDefs, UnionDefs, DrawGemDefs, CostDefs, HuoDongDefs, YYHuoDongDefs, ReunionDefs, DrawChipDefs, DrawEmeraDefs
...
class ObjectDailyAssistant(ObjectBase):
	...
	AllowMethod = {
		...
		DailyAssistantDefs.DrawTotem: 'getDrawTotemInput',
		DailyAssistantDefs.DrawEmera: 'getDrawEmeraInput',
		...
		DailyAssistantDefs.RandomTowerJump: 'getRandomTowerJumpInput',
	...
	def init(self):
		...
		self.game.role.daily_assistant.setdefault("draw_gem_gold10", 0)  # 符石金币10连抽开关
	...
	def getInputs(self, type, filterKeys):
		...
		for csvID in csvIDs:
			...
			if csvID not in self.AllowMethod:
				logger.warning("DailyAssistant Not Implement csvID=%s", csvID)
				continue
	...
	def getDrawGemInput(self):
		...
		# 金币10连抽（需要开启开关）
		if self.game.role.daily_assistant.get('draw_gem_gold10', 0):
			# 计算还能做几次10连抽
			remainLimit = self.game.role.goldDrawGemCountLimit - self.game.dailyRecord.draw_gem_gold
			times = remainLimit // 10  # 可做10连抽的次数
			for _ in xrange(times):
				handlerInput.append({'drawType': DrawGemDefs.Gold10, 'decompose': decompose})
		if not handlerInput:
			return
		return handlerInput
	...
	def getDrawTotemInput(self):
		...
		if self.game.dailyRecord.totem_rmb_dc1_free_count <= 0:
			handlerInput.append({'drawType': 'free1'})
		if not handlerInput:
			return
		return handlerInput

	def getDrawEmeraInput(self):
		'''
		抽琉石
		'''
		handlerInput = []
		if self.game.dailyRecord.emera_rmb_dc1_free_count <= 0:
			handlerInput.append({'drawType': DrawEmeraDefs.Free1})
		if not handlerInput:
			return
		return handlerInput
	...
	def getHuodongFubenInput(self):
		...
		'''
		handlerInput = []
		# 快速扫荡5个副本（金币、经验、碎片、礼物、契约）
		dailyFubens = [HuoDongDefs.TypeGold, HuoDongDefs.TypeExp, HuoDongDefs.TypeFrag, HuoDongDefs.TypeGift, HuoDongDefs.TypeContract]
		...
		for huodongID, flag in huoDongOpens.iteritems():
			if flag:
				...
				elif cfgHuodong.huodongType == HuoDongDefs.TypeContract:
	...
	def getRandomTowerJumpInput(self):
		'''
		以太乐园跳关 - 需要多步交互，不支持一键功能
		'''
		return None
