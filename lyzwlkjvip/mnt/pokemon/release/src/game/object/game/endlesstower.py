...
class ObjectEndlessTowerGlobal(ObjectNoGCDBase):
	...
	# 深渊无限塔 - 最近通关战报
	abyss_latest_plays = db_property('abyss_latest_plays')  # {gateID : [{'play_id': play_id, 'logo': logo, ... },{} ...]}
	...
	@classmethod
	def getAbyssLatestPlays(cls, gateID):
		self = cls.Singleton
		plays = self.abyss_latest_plays.get(gateID, [])
		return plays

	@classmethod
	@coroutine
	def recordAbyssPlay(cls, model, role):
		'''记录深渊无限塔首通战报'''
		self = cls.Singleton
		...
		# 深渊模式使用新的数据库模型 AbyssPVEBattlePlayRecord，支持多队格式
		# 不需要扁平化 cards 和 actions
		playRecordData = yield self._dbc.call_async('DBCreate', 'AbyssPVEBattlePlayRecord', model)
		if not playRecordData['ret']:
			raise ServerError('db create AbyssPVEBattlePlayRecord error')
		play_id = playRecordData['model']['id']

		simplePlay = {"play_id": play_id, "logo": role.logo, "name": role.name, "frame": role.frame, "level": role.level, "vip": role.vip_level_display, "fighting_point": fightingPoint, "round": round, "game_key": role.areaKey}
		# 最近战报
		latestPlays = self.abyss_latest_plays.get(gateID, [])
		deletes = latestPlays[:-2]  # 最多3个
		latestPlays = latestPlays[-2:]
		latestPlays.append(simplePlay)
		self.abyss_latest_plays[gateID] = latestPlays
		if deletes:
			self._dbc.call_async('DBMultipleDelete', 'AbyssPVEBattlePlayRecord', [x['play_id'] for x in deletes])
	...
	def sendToAntiCheatCheck(cls, uid, roleID, name, model, clientResult, rpc):
		...
		def callback(future):
			try:
				...
				# RPC 调用失败（如队列满、超时等），跳过本次检查
				logger.warning('role %d %s gate %d anti-cheat rpc failed: %s', uid, objectid2string(roleID), gateID, e)
				return
