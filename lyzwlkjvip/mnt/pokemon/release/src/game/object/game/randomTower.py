...
class ObjectRandomTower(ObjectDBase):
	...
	def refresh(self):
		...
		# 无论是否每日刷新，都确保当前房间有怪物数据
		if 'enemy' not in self.room_info:
			logger.info('randomTower room_info has no enemy, generating for room %d, roleUid %s', self.room, self.game.role.uid)
			self.setEnermyToRoom()
			self.setEventToRoom()
			self.setBuffToRoom()
	...
	def setEnermyToRoom(self):
		...
		将怪物塞入当前房间
		'''
		# 如果当前房间的 boards 不存在，先生成
		if self.room not in self.boards:
			self.setRoomBoards(self.room)
