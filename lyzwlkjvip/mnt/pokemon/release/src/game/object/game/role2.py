...
class ObjectRole(ObjectDBase):
	...
	def deployBattleExtra(self, weather, arms):
		"""
		保存天气/兵种扩展到 battle_extra
		:param weather: 天气 ID（可为 None/字符串/数字）
		:param arms: 兵种选择，期望 list[list[int]]，允许 None/空/混合类型
		...
		# 2) 规范化 weather -> int
		try:
			w = int(weather) if weather is not None else 0
		except Exception:
			w = 0
				...
				if isinstance(grp, (list, tuple)):
					...
					for x in grp:
						...
						# 字符串：允许负号，纯数字才转
							...
							sx = x.strip()
							s = sx[1:] if sx.startswith('-') else sx
							if s.isdigit():
								row.append(int(sx))
					norm_arms.append(row)

		# 4) 仅在有变化时回写（触发持久化跟踪）
		changed = (be.get('weather') != w) or (be.get('arms') != norm_arms)
		if changed:
			be['weather'] = w
			be['arms'] = norm_arms
