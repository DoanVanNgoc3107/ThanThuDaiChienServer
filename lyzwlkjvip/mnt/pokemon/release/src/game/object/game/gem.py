...
class ObjectGemsMap(ObjectDBaseMap):
	...
	def init(self):
		...
		# 清理 role.gems 中不存在的 gem ID
		roleGems = self.game.role.gems or []
		validGems = [gemID for gemID in roleGems if gemID in self._objs and self._objs[gemID].exist_flag]
		if len(validGems) != len(roleGems):
			invalidCount = len(roleGems) - len(validGems)
			logger.warning('role %s gems list has %d invalid gem IDs, cleaned' % (objectid2string(self.game.role.id), invalidCount))
			self.game.role.gems = validGems
		return ret
	...
	def getCardGemQualitySum(self, cardID):
		...
		card = self.game.cards.getCard(cardID)
		if card is None:
			logger.warning("card not found in getCardGemQualitySum: cardID=%s, role_cards_count=%d, cards_objs_count=%d",
				binascii.hexlify(cardID), len(self.game.role.cards), len(self.game.cards._objs))
			return 0
		ret = 0
		for pos, gemID in card.gems.iteritems():
			gem = self.getGem(gemID)
