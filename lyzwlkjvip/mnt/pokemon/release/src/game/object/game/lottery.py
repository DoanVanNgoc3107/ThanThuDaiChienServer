...
class ObjectDrawRandomItem(ReloadHooker):
	...
	def getRandomItem(self, game=None, selected=None, unselected=None, selectedCards=None, unselectedCards=None):
		if self._sum <= 0:
			return (None, 0)
		...
		ret = self._rand(rndObj, selected, unselected, selectedCards, unselectedCards)
		if ret is None or is_none(ret[0]):
			return (None, 0)
		return ret

	def _rand(self, rndObj, selected=None, unselected=None, selectedCards=None, unselectedCards=None):
		_lst = self._lst
		_cardLst = self._cardLst
		_sum = self._sum
		# 道具概率调整
		if selected and self._lst:
			_lst = [(x[0], x[1], int(x[2] * selected.get(x[0], unselected))) for x in self._lst]
			_sum = sum(x[-1] for x in _lst) + sum(x[-1] for x in _cardLst) + sum(x[-1] for x in self._specialLst)
		# 卡牌概率调整（许愿簿功能）
		# 注意：card 是字典 {'id': cardID}，需要用 card['id'] 来匹配
		if selectedCards and self._cardLst:
			_cardLst = [(card, int(weight * selectedCards.get(card.get('id') if isinstance(card, dict) else card, unselectedCards if unselectedCards else 1))) for card, weight in self._cardLst]
			_sum = sum(x[-1] for x in _lst) + sum(x[-1] for x in _cardLst) + sum(x[-1] for x in self._specialLst)
...
class ObjectDrawCardRandom(ReloadHooker):
	...
	# 许愿簿概率提升倍数
	CUSTOM_CARD_WEIGHT_MULTIPLIER = 5  # 选中卡牌概率提升为原来的5倍

	@classmethod
	def getRandomItems(cls, game, drawType, drawTimes, afterGain, customCardIDs=None):
		"""
		抽卡随机
		customCardIDs: 许愿簿选中的卡牌ID列表，这些卡牌概率会提升
		"""
		...
		selectedCards = None
		if customCardIDs:
			selectedCards = {cardID: cls.CUSTOM_CARD_WEIGHT_MULTIPLIER for cardID in customCardIDs}
					...
					for idx, count in libs.iteritems():
						...
						if itemObj is not None:
							for j in xrange(count):
								csvID, itemCount = itemObj.getRandomItem(game, selectedCards=selectedCards, unselectedCards=1)
								if csvID is None:
									continue
								# 处理可以直接打开的随机礼包
								if isinstance(csvID,int) and ItemDefs.isItemID(csvID) and csvID in csv.items and csv.items[csvID].type == ItemDefs.imOpenRandGiftType:
									...
									# 直接打开的随机礼包 数量就当一个处理
									if newitemObj:
										csvID, itemCount = newitemObj.getRandomItem(game, selectedCards=selectedCards, unselectedCards=1)
										if csvID is None:
											continue
