...
class ObjectHeldItem(ObjectDBase):
	...
	def _fixCorrupted(self):
		...
		if hasattr(self, 'meteorite_index') and self.meteorite_index and self.meteorite_index > 0:
			...
			if hasattr(self.game, 'role') and hasattr(self.game.role, 'meteorites'):
				...
				if meteorite_data:
					helditems = meteorite_data.get('helditems', {})
					meteorite_pos = getattr(self, 'meteorite_pos', 0)
					
					if meteorite_pos > 0:
						# 使用整数键（前端Lua期望整数索引）
						# 兼容旧数据中的字符串键
						actual_held_item_id = None
						
						# 先尝试整数键（标准方式）
						if meteorite_pos in helditems:
							actual_held_item_id = helditems[meteorite_pos]
						# 兼容旧数据的字符串键
						elif str(meteorite_pos) in helditems:
							actual_held_item_id = helditems[str(meteorite_pos)]
						
						if actual_held_item_id is not None:
							if actual_held_item_id != self.id:
								logger.warning('role %s held_item %s meteorite_index=%d pos=%d inconsistent, actual=%s, clearing' % 
											 (objectid2string(self.role_db_id), objectid2string(self.id), 
											  self.meteorite_index, meteorite_pos, actual_held_item_id))
								self.meteorite_index = 0
								self.meteorite_pos = 0
								need_fix = True
						else:
							logger.warning('role %s held_item %s meteorite_index=%d pos=%d not found in helditems, clearing' % 
										 (objectid2string(self.role_db_id), objectid2string(self.id), 
										  self.meteorite_index, meteorite_pos))
							self.meteorite_index = 0
							self.meteorite_pos = 0
							need_fix = True
					else:
						logger.warning('role %s held_item %s meteorite_index=%d pos=%d invalid, clearing' % 
									 (objectid2string(self.role_db_id), objectid2string(self.id), 
									  self.meteorite_index, meteorite_pos))
						self.meteorite_index = 0
						self.meteorite_pos = 0
					...
					logger.debug('role %s held_item %s meteorite_index=%d not exist, clearing' % 
							   (objectid2string(self.role_db_id), objectid2string(self.id), self.meteorite_index))
					self.meteorite_index = 0
					self.meteorite_pos = 0
					need_fix = True
			
			# 如果修复了数据，触发保存（直接标记为 dirty）
			if need_fix:
				# 直接修改 db 字典来确保触发保存
				self.db['meteorite_index'] = self.meteorite_index
				self.db['meteorite_pos'] = self.meteorite_pos
				logger.info('role %s held_item %s meteorite data fixed and marked dirty', 
						   objectid2string(self.role_db_id), objectid2string(self.id))
	...
	# 陨石索引（反向引用：持有物装备在哪个陨石上，0表示未装备）
	meteorite_index = db_property('meteorite_index')
	
	# 陨石位置（持有物在陨石上的槽位，1-6）
	meteorite_pos = db_property('meteorite_pos')
