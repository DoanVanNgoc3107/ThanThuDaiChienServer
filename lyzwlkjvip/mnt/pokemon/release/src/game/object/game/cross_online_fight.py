...
class ObjectCrossOnlineFightGameGlobal(ObjectNoGCDBase):
				...
				with guard:
					if game.dailyRecord.cross_online_fight_times <= self.MatchTime:
						...
						if result == 'win':
							...
							# 连胜次数（用于勋章统计）
							game.role.cross_online_fight_win_streak = (game.role.cross_online_fight_win_streak or 0) + 1
							# 公平模式胜利计数 (targetType=28, pattern=2是公平模式)
							pattern = view.get('pattern', 0)
							if pattern == 2:  # Limited = 2
								# 检查使用的精灵是否匹配勋章要求
								limitedCards = game.role.cross_online_fight_limited_cards or []
								usedMarkIDs = set()
								for cardID in limitedCards:
									if cardID in csv.cards:
										usedMarkIDs.add(csv.cards[cardID].cardMarkID)
								# 1231: 雷电云 (cardMarkID=3201)
								if 3201 in usedMarkIDs:
									game.medal.incrementMedalCounter(1231)
								# 1241: 三圣鸟 (cardMarkIDs=721,731,741 需要同时拥有)
								if {721, 731, 741} <= usedMarkIDs:
									game.medal.incrementMedalCounter(1241)
								# 1251: 三神柱 (cardMarkIDs=1911,1921,1931 需要同时拥有)
								if {1911, 1921, 1931} <= usedMarkIDs:
									game.medal.incrementMedalCounter(1251)
								# 1261: 路卡利欧 (cardMarkID=2241)
								if 2241 in usedMarkIDs:
									game.medal.incrementMedalCounter(1261)
						elif result in ('fail', 'giveup', 'offline'): # flee 逃跑不给奖励
							award = self.BattleFailAward
							# 失败重置连胜次数
							game.role.cross_online_fight_win_streak = 0
						if award:
	...
	def onFinalAwardRanks(self, ranks):
		...
		for roleID, rank in ranks.iteritems():
			...
			MailJoinableQueue.send(mail)
			# 勋章计数：对战竞技场排名 (Type 22, medalID 1171, 前10名)
			updateMedalCounterForRank(self._dbc, ObjectGame.getByRoleID, roleID, 1171, 10, rank)
