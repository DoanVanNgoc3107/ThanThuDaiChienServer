	...
	def onRankAward(self, roleRanks, type):
		...
		if type == 2:
			for roleID, rank in roleRanks.iteritems():
				if 'rbt' in roleID:  # robot
					continue
				# 勋章计数：跨服竞技场王者段位排名 (Type 25, medalID 1201, 王者段位前10名)
				# 王者段位stageID=19 (来自csv cross/arena/stage.csv)
				stageID, stageRank = self.getStageRank(rank)
				if stageID == 19:  # 王者段位
					updateMedalCounterForRank(self._dbc, ObjectGame.getByRoleID, roleID, 1201, 10, stageRank)
