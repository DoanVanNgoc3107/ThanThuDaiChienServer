...
class ObjectChipsMap(ObjectDBaseMap):
	...
	def _fixCorrupted(self):
		for _, chip in self._objs.iteritems():
			cardID = chip.card_db_id
			if cardID and not self.game.cards.getCard(chip.card_db_id):
				chip.card_db_id = None
				logger.warning('role %s chip %s %s card not exist!' % (objectid2string(self.game.role.id), chip.chip_id, objectid2string(chip.id)))
	...
	def deleteChips(self, objs):
		...
		for obj in objs:
			...
			if obj.card_db_id:
				...
				if card:
					for pos, chipID in list(card.chip.iteritems()):
						if chipID == obj.id:
