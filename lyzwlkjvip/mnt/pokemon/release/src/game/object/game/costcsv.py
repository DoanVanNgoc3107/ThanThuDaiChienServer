...
class ObjectCostCSV(ReloadHooker):
	...
	@classmethod
	def _initPlayPassportBuyCost5(cls):
		cfg = cls.CostMap[CostDefs.PlayPassportBuyCost5]
		cls.CostMap[CostDefs.PlayPassportBuyCost5] = cfg.seqParam

	@classmethod
	def getPlayPassportBuyCost5(cls, times):
		lst = cls.CostMap[CostDefs.PlayPassportBuyCost5]
		maxT = min(len(lst) - 1, times)
		return lst[maxT]
	...
	# 启示之塔抽奖消耗
	@classmethod
	def _initRoundDrawCost(cls):
		cfg = cls.CostMap[CostDefs.RoundDrawCost]
		cls.CostMap[CostDefs.RoundDrawCost] = cfg.seqParam

	@classmethod
	def getRoundDrawCost(cls, times):
		lst = cls.CostMap[CostDefs.RoundDrawCost]
		maxT = min(len(lst) - 1, times)
		return lst[maxT]