...
from game.object.game.emera import ObjectEmera, ObjectEmerasMap
from game.object.game.contract import ObjectContract, ObjectContractsMap
from game.object.game.totem import ObjectTotem
...
from game.object.game.mimicry import ObjectMimicry
from game.object.game.auto_chess import ObjectAutoChess
# 注意：ObjectAutoChessPVPShop 在 handler 中按需导入使用
...
from game.object.game.record import ObjectDailyRecord, ObjectMonthlyRecord, ObjectLotteryRecord, ObjectWeeklyRecord
from game.object.game.rank import ObjectPWAwardRange, ObjectRandomTowerAwardRange, ObjectArenaFlopAwardRandom
from game.object.game.shop import ObjectUnionShop, ObjectFixShop, ObjectMysteryShopRandom, ObjectUnionShopRandom, ObjectFixShopRandom, ObjectMysteryShop, ObjectExplorerShopRandom, ObjectExplorerShop, ObjectFragShop,	ObjectFragShopRandom, ObjectRandomTowerShop, ObjectEquipShopRandom, ObjectEquipShop, ObjectFishingShop, ObjectFishingShopRandom, ObjectTotemShop, ObjectTotemShopRandom
...
from game.object.game.medal import ObjectMedal
from game.object.game.meteorite import ObjectMeteorite
from game.object.game.town import ObjectTown, ObjectTownShop
...
class ObjectGame(GCObject, Copyable, ReloadHooker):
	...
	def initAllClass(cls):
		...
		ObjectTotemShopRandom.classInit()
		...
		ObjectMimicry.classInit()
		ObjectHellRandomTower.classInit()
		...
		ObjectEmera.classInit()
		ObjectContract.classInit()
		ObjectTotem.classInit()
		ObjectCrossArenaGameGlobal.classInit()
		ObjectCrossSupremacyGameGlobal.classInit()
	...
	def popByRoleID(cls, roleID):
		...
		if game:
			...
			for emeraID in (game.role.emeras or []):
				ObjectEmera.EmeraObjsMap.pop(emeraID, None)
			...
			ObjectTotemShop.ShopsObjsMap.pop(game.role.totem_shop_db_id, None)
	...
	def __init__(self, dbcGame, setSafeGuard):
		...
		self.weeklyRecord = ObjectWeeklyRecord(self, dbcGame)
		...
		self.medal = ObjectMedal(self)
		self.meteorite = ObjectMeteorite(self)
		...
		self.mimicry = ObjectMimicry(self, dbcGame)
		self.auto_chess = ObjectAutoChess(self, dbcGame)
		# 注意：autoChessShop 不预初始化，在 handler 中按需创建（同 ObjectCrossSupremacyShop）
		self.hellRandomTower = ObjectHellRandomTower(self, dbcGame)
		...
		self.emeras = ObjectEmerasMap(self)
		...
		self.contracts = ObjectContractsMap(self)
		self.totem = ObjectTotem(self, dbcGame)
		self.totemShop = ObjectTotemShop(self, dbcGame)
		self.town = ObjectTown(self, dbcGame)
		self.townShop = ObjectTownShop(self, dbcGame)
		self.meteorite = ObjectMeteorite(self)
	...
	def init(self):
		...
		# 拟态对战：如果没有 db_id，不初始化（保持 _db 为 None）
		if self.role.mimicry_db_id:
			self.mimicry.init()
		# 卡牌冒险：如果没有 db_id，不初始化（保持 _db 为 None）
		# 首次进入功能时在 handler 中创建记录
		if self.role.auto_chess_db_id:
			self.auto_chess.init()
		# 没有 db_id 时，_db 为 None，save_async 会自动跳过
		# 无尽随机塔：无论是否有 db_id，都需要调用 set() 初始化默认数据
		# 这样前端才能获取到 room_info 等字段的默认值
		self.hellRandomTower.set({})
		if self.role.hell_random_tower_db_id:
			self.hellRandomTower.init()
		self.meteorite.set().init()
		self.cards.init()
		self.medal.set().init()
		self.society.init()
	...
	def save_async(self, forget=False):
		fus = [
			...
			self.mimicry.save_async(forget),
			...
			self.totemShop.save_async(forget),
			self.totem.save_async(forget),
			...
			self.emeras.save_async(forget),
			self.contracts.save_async(forget),
			self.reunionRecord.save_async(forget),
			self.town.save_async(forget),
			self.townShop.save_async(forget)
		]
		future = Future()
		closure = [len(fus), 0]
	...
	def model(self):
		...
		return {
			...
			'weekly_record': self.weeklyRecord.model,
			...
			'mimicry': self.mimicry.model,
			'hell_random_tower': self.hellRandomTower.model,
			'equip_shop': self.equipShop.model,
			'fishing_shop': self.fishingShop.model,
			'totem_shop': self.totemShop.model,
			...
			'emeras': {o.id: o.model for o in self.emeras},
			'reunion_record': self.reunionRecord.model,
			'contracts': {o.id: o.model for o in self.contracts},
			'totem': self.totem.model,
			'town': self.town.model,
			'town_shop': self.townShop.model,
			'auto_chess': self.auto_chess.model,
			# 注意：auto_chess_shop 不单独同步，前端没有这个模型
		}
	...
	def modelSync(self):
		...
		weeklyRecordSync = self.weeklyRecord.modelSync
		...
		mimicrySync = self.mimicry.modelSync
		hellRandomTowerSync = self.hellRandomTower.modelSync
		equipShopSync = self.equipShop.modelSync
		fishingShopSync = self.fishingShop.modelSync
		totemShopSync = self.totemShop.modelSync
		...
		emerasSync = self.emeras.dirtyMapSync
		reunionRecordSync = self.reunionRecord.modelSync
		contractsSync = self.contracts.dirtyMapSync
		totemSync = self.totem.modelSync
		townSync = self.town.modelSync
		townShopSync = self.townShop.modelSync
		autoChessSync = self.auto_chess.modelSync
		# 注意：auto_chess_shop 不单独同步，前端没有这个模型
		...
		kvs = (
			...
			('weekly_record', weeklyRecordSync),
			...
			('mimicry', mimicrySync),
			('hell_random_tower', hellRandomTowerSync),
			('equip_shop', equipShopSync),
			('fishing_shop', fishingShopSync),
			('totem_shop', totemShopSync),
			('capture', captureSync),
			('fishing', fishingSync),
			('gems', gemsSync),
			('chips', chipsSync),
			('emeras', emerasSync),
			('contracts', contractsSync),
			('reunion_record', reunionRecordSync),
			('totem', totemSync),
			('town', townSync),
			('town_shop', townShopSync),
			('auto_chess', autoChessSync),
		)
		...
		kvs = (
			...
			('emeras', self.emeras.modelSync),
			('contracts', self.contracts.modelSync),
		)
