...
from game.object.game.lottery import ObjectDrawCardRandom, ObjectDrawEquipRandom, ObjectDrawItemRandom, ObjectDrawEffect, ObjectDrawGemRandom, ObjectDrawRandomItem
...
class ObjectYYTimeLimitDraw(ObjectYYBase):
	...
	def getEffect(cls, yyID, drawType, game):
		...
		eff = ObjectGainAux(game, {})
		# 许愿簿：获取限时自选卡牌ID，提升概率
		customCardIDs = game.lotteryRecord._getLimitCustomCardID(yyID)
		for _ in xrange(drawCount):
			drawTimes += 1
			award = ObjectDrawCardRandom.getRandomItems(game, DrawCardDefs.LimitDrawRandomKey(DrawCardDefs.LimitRMB1, yyID), drawTimes, None, customCardIDs)
			award = award.to_dict()
...
class ObjectYYTimeLimitBox(ObjectYYBase):
	...
	def acquireQualify(cls, yyID, game):
		...
		# 优先使用活动配置中的自定义战力限制
		if 'FightLimit' in cfg.paramMap:
			fightPointLimit = cfg.paramMap['FightLimit']
			vipLimit = cfg.paramMap.get('VipLimit', None)  # 可选的VIP限制，None表示不检查VIP
		else:
		...
		# 检查资格：仅检查战力
		qualify = game.role.top6_fighting_point >= fightPointLimit
		if qualify:
...
class ObjectYYItemExchange(ObjectYYBase):
	...
	def getEffect(cls, yyID, csvID, game, count=1):
		...
		# 消耗判定
		cost = ObjectCostAux(game, cfg.costMap, yy_id=yyID)
		cost *= count
		if not cost.isEnough():
		...
		cost.cost(src='yy_%d' % yyID, yy_id=yyID)
...
class ObjectYYGeneralTask(ObjectYYBase):
	...
	def active(cls, yyObj, game, type, val):
		...
		for i, tpred in enumerate(hd.typePreds):
			...
			# 每日充值任务：taskSpecialParam 中有 daily=1 时，使用 dailyRecord.recharge_rmb_sum
			taskCfg = cls.csv(csvID)
			if taskType == TargetDefs.RechargeRmb and taskCfg.taskSpecialParam.get('daily', 0) == 1:
				valsum = game.dailyRecord.recharge_rmb_sum
				valsumsD[csvID] = valsum
				# 检查今日是否已领取
				lastday = valinfoD.setdefault(csvID, {}).get('lastday', None)
				nowday = perioddate2int(globaldata.DailyRecordRefreshTime)
				if lastday != nowday:
					# 新的一天，重置领取状态
					if csvID in stamps:
						del stamps[csvID]
					valinfoD[csvID]['lastday'] = nowday
				# 判断是否可领取
				if csvID not in stamps:
...
class ObjectYYRegainStamina(ObjectYYBase):
	...
	@classmethod
	def classInit(cls):
		yyconf = getattr(csv.yunying, 'yyhuodong', None)
		if not yyconf:
			return
		for yID in yyconf:
			if yyconf[yID].type == YYHuoDongDefs.RegainStamina:
				cls.rmb = yyconf[yID].paramMap['rmb']
				break
...
class ObjectYYDirectBuyGift(ObjectYYBase):
	...
	def buyRecharge(cls, game, rechargeID, yyID, csvID):
		...
		if not cfg:
			return False, None
		yyCfg = csv.yunying.yyhuodong[yyID] if yyID in csv.yunying.yyhuodong else None
		if not yyCfg or cfg.huodongID != yyCfg.huodongID:
			return False, None
...
class ObjectYYPlayPassport(ObjectYYBase):
	...
	def refreshRecord(cls, yyObj, game):
		...
		if not record:
			...
			# 开放 level=1 的奖励
			cls.levelAwardOpen(huodongID, record, 0, 1)
			if not cls.isLoginPassPort(yyObj):
				cls.onTaskChange(game, yyObj, yyObj.paramMap['type'], 0)
		else:
			stamps = record.setdefault('stamps', {})  # 普通奖励
			record.setdefault('stamps1', {})  # 进阶奖励
			# 老数据兼容：确保 info 中所有字段存在
			info = record.setdefault('info', {})
			info.setdefault('level', 1)
			info.setdefault('exp', 0)
			info.setdefault('buy_times', 0)
			info.setdefault('buy_level', 0)
			info.setdefault('elite_buy', 0)
			info.setdefault('last_time', 0)
			# 老数据修复：level=0 改为 1
			if info['level'] == 0:
				info['level'] = 1
			# 老数据修复：如果 stamps 为空但 level >= 1，需要开放奖励
			if not stamps and info['level'] >= 1:
				cls.levelAwardOpen(huodongID, record, 0, info['level'])
			if ndi != info.get('last_time', None):
				info['buy_times'] = 0
		# 对于登录类型活动，不在这里更新 lastday，在 onTaskChange 中更新
		if not cls.isLoginPassPort(yyObj):
			if ndi != record.get('lastday', None):
				record['lastday'] = ndi
		return record
	...
	def getEffect(cls, yyID, csvID, game):
		...
		elite_buy = record['info'].get('elite_buy', 0)
	...
	def canBuy(cls, yyID, csvID, game):
		...
		if csvID:
			...
			if cfg.huodongID != ObjectYYHuoDongFactory.getConfig(yyID).huodongID:
				raise ClientError('huodongID error')
			if record['info'].get('elite_buy', 0):
				# 重复购买
				return False
		else:
			# 购买通行证经验
			...
			info = record['info']
			# 购买次数检查
			if info.get('buy_times', 0) >= yyObj.paramMap['dailyBuyTimes']:
				return False
			# 等级超出上限
			huodongID = csv.yunying.yyhuodong[yyID].huodongID
			levelAward = cls.LevelAwardMap[huodongID]
			if info.get('level', 0) >= yyObj.paramMap.get('levelLimit', levelAward['levelMax']):
				return False
		return True
	...
	def getCost(cls, playPassportType, info, level):
		buy_level = info.get('buy_level', 0)
		if playPassportType == PlayPassportDefs.DailyTask:
			rmb = sum(
				[ObjectCostCSV.getPlayPassportBuyCost2(buy_level + num) for num in xrange(level)])
		elif playPassportType == PlayPassportDefs.RandomTower:
			rmb = sum(
				[ObjectCostCSV.getPlayPassportBuyCost3(buy_level + num) for num in xrange(level)])
		elif playPassportType == PlayPassportDefs.Gym:
			rmb = sum(
				[ObjectCostCSV.getPlayPassportBuyCost4(buy_level + num) for num in xrange(level)])
		elif playPassportType == PlayPassportDefs.Stamina:
			rmb = sum(
				[ObjectCostCSV.getPlayPassportBuyCost5(buy_level + num) for num in xrange(level)])
		else:
			raise ClientError(ErrDefs.yyHuoDongNoActive)
		return rmb
	...
	def onTaskChange(cls, game, yyObj, playtype, val):
		...
		info = record['info']
		huodongID = csv.yunying.yyhuodong[yyObj.id].huodongID
		levelAward = cls.LevelAwardMap.get(huodongID)
		if levelAward is None:
			return
		...
		if 'exp' not in info:
			info['exp'] = 0
		if 'elite_buy' not in info:
			info['elite_buy'] = 0
		if 'buy_times' not in info:
			info['buy_times'] = 0
		if 'buy_level' not in info:
			info['buy_level'] = 0
		# 老数据修复：level=0 改为 1
		if info['level'] == 0:
			info['level'] = 1
		...
		if cls.isLoginPassPort(yyObj):
			# 修复：如果 stamps 为空且等级是1且经验为0，说明需要重新计算
			needFix = not record.get('stamps') and info['level'] == 1 and info['exp'] == 0
			if ndi == record.get('lastday', None) and not needFix:
				return  # 同一天不重复计算
	...
	def levelAwardOpen(cls, huodongID, record, oldLevel, newLevel):
		...
		for level in xrange(oldLevel + 1, newLevel + 1):
			# 通过等级 获取 对应奖励
			awardCsvID = levelAward['award'].get(level)
			if awardCsvID is None:
				continue
			cfg = cls.awardCsv(awardCsvID)
			# 普通奖励打开 (设置为 1)
...
class ObjectYYReunion(ObjectYYBase):
	...
	def buyRecharge(cls, game, rechargeID, yyID, csvID):
		...
		cfg = csv.yunying.reunion_recharge[csvID] if csvID in csv.yunying.reunion_recharge else None
		if not cfg:
			return False, None
		yyCfg = csv.yunying.yyhuodong[yyID] if yyID in csv.yunying.yyhuodong else None
		if not yyCfg or cfg.huodongID != yyCfg.huodongID:
...
class ObjectYYSnowBall(ObjectYYBase):
	...
	def refreshRecord(cls, yyObj, game, init=None):
		...
		record.setdefault('stamps', {})
		# 老数据兼容：确保 info 中所有字段存在
		info.setdefault('isGuide', 0)
		info.setdefault('top_point', 0)
		info.setdefault('top_time', 0)
		info.setdefault('top_role', 0)
		info.setdefault('rank', 0)
		info.setdefault('days', 0)
...
class ObjectYYJifu(ObjectYYGeneralTask):
	...
	def getEffect(cls, yyID, csvID, game):
		...
		yyCfg = csv.yunying.yyhuodong[yyID] if yyID in csv.yunying.yyhuodong else None
		cfg = cls.csv(csvID)
		if not cfg or not yyCfg or yyCfg.huodongID != cfg.huodongID:
...
# ObjectYYRefloat
#
class ObjectYYRefloat(ObjectYYGeneralTask):
	"""
	惊喜捞一捞
	type = 87
	"""

	HuoDongMap = {}  # {huodongID: ObjectYYRefloat}
	TaskMap = {}  # {huodongID: [cfg, ...]}

	@classmethod
	def csv(cls, csvID=None):
		if csvID is None:
			return csv.yunying.refloat_task
		return csv.yunying.refloat_task[csvID]
	...
	def classInit(cls):
		...
		for csvID in cls.csv():
			cfg = cls.csv(csvID)
			cls.TaskMap.setdefault(cfg.huodongID, []).append(cfg)

	@classmethod
	def refreshRecord(cls, yyObj, game, init=None):
		record = cls.getRecord(yyObj.id, game)
		info = record.setdefault('info', {})
		info.setdefault('refloat_draw1_times', 0)  # 单抽次数
		info.setdefault('refloat_draw10_times', 0)  # 十连次数
		record.setdefault('stamps', {})  # 任务领取状态 {csvID: 0/1}
		return record
	...
	def getEffect(cls, yyID, drawType, game):
		"""
		抽奖接口或任务奖励领取
		drawType: 'refloat_rmb1' 或 'refloat_rmb10' (抽奖)
		         或 整数 csvID (任务奖励领取)
		"""
		# 如果是整数，则是任务奖励领取
		if isinstance(drawType, (int, long)):
			return cls.getTaskEffect(yyID, drawType, game)
		...
		# 确定抽奖次数
		if drawType == 'refloat_rmb1':
			drawCount = 1
			costNum = paramMap.get('draw1CostNum', 1)
		elif drawType == 'refloat_rmb10':
			drawCount = 10
			costNum = paramMap.get('draw10CostNum', 10)
		else:
			raise ClientError('drawType error')

		# 检查消耗道具
		costItem = paramMap.get('costItem', 0)
		if costItem <= 0:
			raise ClientError('costItem config error')

		cost = ObjectCostAux(game, {costItem: costNum})
		if not cost.isEnough():
			raise ClientError(ErrDefs.refloatCostNotEnough)

		# 获取奖池配置 - 使用 paramMap.drawPoolId 或默认使用 huodongID
		drawPoolId = paramMap.get('drawPoolId', yyObj.huodongID)
		if drawPoolId <= 0:
			raise ClientError('drawPoolId config error')

		# 使用 draw_items_lib 进行抽奖
		grids = []
		eff = ObjectGainAux(game, {})

		for _ in xrange(drawCount):
			rndObj = ObjectDrawRandomItem.getObject(drawPoolId)
			if rndObj is None:
				raise ClientError('draw pool config error')
			itemT = rndObj.getRandomItem(game)
			if itemT:
				csvID, count = itemT
				if csvID is not None and count > 0:
					grids.append((csvID, count, 0))
					eff += ObjectGainAux(game, {csvID: count})

		def _afterGain():
			cost.cost(src='yy_refloat_%d' % yyID)
			# 记录抽奖次数，前端需要 refloat_draw1_times 和 refloat_draw10_times
			if drawCount == 1:
				info['refloat_draw1_times'] = info.get('refloat_draw1_times', 0) + 1
			else:
				info['refloat_draw10_times'] = info.get('refloat_draw10_times', 0) + 1
			# 更新任务状态
			cls._updateTaskStamps(yyObj, game)

		return ObjectDrawEffect(game, eff.to_dict(), grids, _afterGain)

	@classmethod
	def _updateTaskStamps(cls, yyObj, game):
		"""更新任务完成状态"""
		record = cls.getRecord(yyObj.id, game)
		info = record.get('info', {})
		stamps = record.setdefault('stamps', {})
		# 计算总抽奖次数：单抽次数 + 十连次数*10
		drawTimes = info.get('refloat_draw1_times', 0) + info.get('refloat_draw10_times', 0) * 10

		taskList = cls.TaskMap.get(yyObj.huodongID, [])
		for cfg in taskList:
			csvID = cfg.id
			if csvID not in stamps:
				if drawTimes >= cfg.targetDrawCount:
					stamps[csvID] = 1  # 可领取

	@classmethod
	def getTaskEffect(cls, yyID, csvID, game):
		"""领取任务奖励"""
		yyObj = ObjectYYHuoDongFactory.getConfig(yyID)
		if yyObj is None:
			raise ClientError(ErrDefs.huodongNoOpen)
		...
		cfg = cls.csv(csvID)
		if cfg is None:
			raise ClientError('task config error')
	...
	def getOneKeyEffect(cls, yyID, game):
		...
		ids = [csvID for csvID in stamps if stamps[csvID] == 1]
		if not ids:
			return None
...
# ObjectYYRoundFlop
#
class ObjectYYRoundFlop(ObjectYYBase):
	"""
	惊喜连连活动
	type = 99

	玩法说明：
	1. 4x4翻牌面板（16个格子），翻开消耗道具
	2. 每轮有1个大奖、若干普通奖励和炸弹
	3. 翻到大奖进入下一轮，翻到炸弹翻开所有剩余格子
	4. 玩家需先选择想要的大奖才能开始翻牌

	数据结构：
	{
		info: {roundID: 当前轮数, grandPrizeID: 选择的大奖ID},
		stamps: {pos: awardID},  # 位置->奖励ID (-1=大奖, -2=炸弹, >0=普通奖励ID)
		stamps1: {csvID: 0/1},   # 轮数奖励领取状态
		valsums: {grandPrizeID: count}  # 大奖获得次数
	}
	"""
	...
	HuoDongMap = {}  # {huodongID: ObjectYYRoundFlop}
	GrandPrizeMap = {}  # {huodongID: {csvID: cfg}}
	NormalAwardMap = {}  # {huodongID: {csvID: cfg}}
	WeightsMap = {}  # {huodongID: {round: {normalWeights, boomWeight, grandPrizeWeight}}}
	RoundAwardMap = {}  # {huodongID: {csvID: cfg}}
	...
	def csv(cls, csvID=None):
		...
		return csv.yunying.round_flop_award[csvID]
	...
	def classInit(cls):
		...
		# 初始化大奖配置
		if hasattr(csv.yunying, 'round_flop_grand_prize'):
			for csvID in csv.yunying.round_flop_grand_prize:
				cfg = csv.yunying.round_flop_grand_prize[csvID]
				cls.GrandPrizeMap.setdefault(cfg.huodongID, {})[csvID] = cfg
			logger.info('RoundFlop GrandPrizeMap loaded: %s', cls.GrandPrizeMap.keys())
		else:
			logger.warning('RoundFlop round_flop_grand_prize CSV not found!')

		# 初始化普通奖励配置
		if hasattr(csv.yunying, 'round_flop_normal'):
			for csvID in csv.yunying.round_flop_normal:
				cfg = csv.yunying.round_flop_normal[csvID]
				cls.NormalAwardMap.setdefault(cfg.huodongID, {})[csvID] = cfg

		# 初始化权重配置
		if hasattr(csv.yunying, 'round_flop_weights'):
			for csvID in csv.yunying.round_flop_weights:
				cfg = csv.yunying.round_flop_weights[csvID]
				cls.WeightsMap.setdefault(cfg.huodongID, {})[cfg.round] = {
					'normalWeights': cfg.normalWeights if hasattr(cfg, 'normalWeights') else {},
					'boomWeight': cfg.boomWeight if hasattr(cfg, 'boomWeight') else 0,
					'grandPrizeWeight': cfg.grandPrizeWeight if hasattr(cfg, 'grandPrizeWeight') else 0,
				}

		# 初始化轮数奖励配置
		if hasattr(csv.yunying, 'round_flop_award'):
			for csvID in csv.yunying.round_flop_award:
				cfg = csv.yunying.round_flop_award[csvID]
				cls.RoundAwardMap.setdefault(cfg.huodongID, {})[csvID] = cfg
	...
	def refreshRecord(cls, yyObj, game, init=None):
		record = cls.getRecord(yyObj.id, game)
		info = record.setdefault('info', {})
		info.setdefault('roundID', 1)
		info.setdefault('grandPrizeID', 0)
		record.setdefault('stamps', {})  # 翻开的格子 {pos: awardID}
		record.setdefault('stamps1', {})  # 轮数奖励领取状态
		record.setdefault('valsums', {})  # 大奖获得次数
		return record

	@classmethod
	def getMaxRound(cls, huodongID):
		"""获取最大轮数"""
		weightsMap = cls.WeightsMap.get(huodongID, {})
		if not weightsMap:
			return 1
		return max(weightsMap.keys())

	@classmethod
	def switchGrandPrize(cls, yyID, csvID, game):
		"""切换/选择大奖"""
		yyObj = ObjectYYHuoDongFactory.getConfig(yyID)
		if yyObj is None:
			raise ClientError(ErrDefs.huodongNoOpen)
		...
		# 检查大奖是否存在
		logger.info('RoundFlop switchGrandPrize: yyID=%s, huodongID=%s, csvID=%s, GrandPrizeMap keys=%s', 
			yyID, yyObj.huodongID, csvID, cls.GrandPrizeMap.keys())
		grandPrizeMap = cls.GrandPrizeMap.get(yyObj.huodongID, {})
		logger.info('RoundFlop grandPrizeMap for huodongID %s: %s', yyObj.huodongID, grandPrizeMap.keys() if grandPrizeMap else 'empty')
		if csvID not in grandPrizeMap:
			raise ClientError('grand prize not found')

		cfg = grandPrizeMap[csvID]

		# 检查解锁轮数
		if info['roundID'] < cfg.unlockRound:
			raise ClientError('grand prize not unlocked')

		# 检查大奖获取次数限制
		if cfg.limit > 0:
			valsums = record.setdefault('valsums', {})
			if valsums.get(csvID, 0) >= cfg.limit:
				raise ClientError('grand prize limit reached')

		info['grandPrizeID'] = csvID

	@classmethod
	def getFlipEffect(cls, yyID, pos, game):
		"""翻牌获取奖励"""
		yyObj = ObjectYYHuoDongFactory.getConfig(yyID)
		if yyObj is None:
			raise ClientError(ErrDefs.huodongNoOpen)
		...
		# 检查是否选择了大奖
		grandPrizeID = info.get('grandPrizeID', 0)
		if not grandPrizeID:
			raise ClientError('please select grand prize first')

		# 检查是否已经完成所有轮数
		maxRound = cls.getMaxRound(huodongID)
		if info['roundID'] > maxRound:
			raise ClientError('activity completed')

		# 检查位置是否有效
		if pos < 1 or pos > cls.GRID_SIZE:
			raise ClientError('invalid position')

		# 检查位置是否已翻开
		if pos in stamps:
			raise ClientError('position already opened')

		# 检查消耗道具
		yyCfg = csv.yunying.yyhuodong[yyID]
		costItem = yyCfg.paramMap.get('costItem', 0)
		if costItem <= 0:
			raise ClientError('costItem config error')

		cost = ObjectCostAux(game, {costItem: 1})
		if not cost.isEnough():
			raise ClientError(ErrDefs.costNotEnough)

		# 获取本轮权重配置
		roundID = info['roundID']
		weightsConfig = cls.WeightsMap.get(huodongID, {}).get(roundID, {})
		normalWeights = weightsConfig.get('normalWeights', {})
		boomWeight = weightsConfig.get('boomWeight', 0)

		# 构建随机池
		pool = {}
		# 添加普通奖励权重
		normalAwardMap = cls.NormalAwardMap.get(huodongID, {})
		for csvID, weight in normalWeights.items():
			csvID = int(csvID) if isinstance(csvID, str) else csvID
			if csvID in normalAwardMap and csvID not in stamps.values():
				pool[csvID] = weight

		# 添加炸弹权重（如果还没翻到炸弹）
		if cls.BOOM not in stamps.values() and boomWeight > 0:
			pool[cls.BOOM] = boomWeight

		# 如果只剩一个格子未翻开，必定是大奖
		openedCount = len(stamps)
		if openedCount >= cls.GRID_SIZE - 1:
			awardID = cls.GRAND_PRIZE
		elif not pool:
			# 池子空了，给大奖
			awardID = cls.GRAND_PRIZE
		else:
			# 随机抽取
			awardID, _ = WeightRandomObject.onceRandom(pool)

		# 记录翻开结果
		stamps[pos] = awardID
		openResult = {pos: awardID}

		# 如果是炸弹，翻开所有剩余格子
		if awardID == cls.BOOM:
			for p in range(1, cls.GRID_SIZE + 1):
				if p not in stamps:
					# 剩余格子随机分配（除了大奖）
					remainPool = {}
					for csvID, weight in normalWeights.items():
						csvID = int(csvID) if isinstance(csvID, str) else csvID
						if csvID in normalAwardMap and csvID not in stamps.values():
							remainPool[csvID] = weight
					# 最后一个未分配的格子是大奖
					if len([x for x in range(1, cls.GRID_SIZE + 1) if x not in stamps]) == 1:
						stamps[p] = cls.GRAND_PRIZE
						openResult[p] = cls.GRAND_PRIZE
					elif remainPool:
						rndAward, _ = WeightRandomObject.onceRandom(remainPool)
						stamps[p] = rndAward
						openResult[p] = rndAward
					else:
						stamps[p] = cls.GRAND_PRIZE
						openResult[p] = cls.GRAND_PRIZE
		...
		# 计算奖励
		eff = ObjectGainAux(game, {})
		grandPrizeMap = cls.GrandPrizeMap.get(huodongID, {})
		normalAwardMap = cls.NormalAwardMap.get(huodongID, {})

		for p, aid in openResult.items():
			if aid == cls.GRAND_PRIZE:
				# 大奖
				if grandPrizeID in grandPrizeMap:
					eff += ObjectGainAux(game, grandPrizeMap[grandPrizeID].award)
			elif aid == cls.BOOM:
				# 炸弹无奖励
				pass
			elif aid in normalAwardMap:
				# 普通奖励
				eff += ObjectGainAux(game, normalAwardMap[aid].award)

		# 检查是否需要进入下一轮
		hasGrandPrize = cls.GRAND_PRIZE in stamps.values()

		def _afterGain():
			# 消耗道具
			cost.cost(src='yy_round_flop_%d' % yyID)

			# 如果获得了大奖，更新统计并进入下一轮
			if hasGrandPrize:
				valsums = record.setdefault('valsums', {})
				valsums[grandPrizeID] = valsums.get(grandPrizeID, 0) + 1

				# 更新轮数奖励状态
				cls._updateRoundAwardStamps(yyObj, game, info['roundID'])

				# 进入下一轮
				if info['roundID'] < maxRound:
					info['roundID'] += 1
					record['stamps'] = {}  # 重置格子
					info['grandPrizeID'] = 0  # 重置大奖选择

		# 返回 (effect, openResult) 元组，open 不能放在 eff 中否则会被当作道具处理
		return ObjectYYHuoDongEffect(game, eff.to_dict(), _afterGain), openResult

	@classmethod
	def _updateRoundAwardStamps(cls, yyObj, game, completedRound):
		"""更新轮数奖励领取状态"""
		record = cls.getRecord(yyObj.id, game)
		stamps1 = record.setdefault('stamps1', {})
		roundAwardMap = cls.RoundAwardMap.get(yyObj.huodongID, {})

		for csvID, cfg in roundAwardMap.items():
			if csvID not in stamps1 and completedRound >= cfg.round:
				stamps1[csvID] = 1  # 可领取
	...
	def getOneKeyEffect(cls, yyID, game):
		...
		ids = [csvID for csvID in stamps1 if stamps1[csvID] == 1]
		if not ids:
			return None
...
#
# ObjectYYDailyRandomGift
#
class ObjectYYDailyRandomGift(ObjectYYBase):
	"""
	每日随机礼包
	type = 85
	"""
	HuoDongMap = {}  # {huodongID: [csvIDs]}

	@classmethod
	def csv(cls, csvID=None):
		if csvID is None:
			return csv.yunying.daily_random_gift
		return csv.yunying.daily_random_gift[csvID]
	...
	def classInit(cls):
		...
		for csvID in cls.csv():
			cfg = cls.csv(csvID)
			cls.HuoDongMap.setdefault(cfg.huodongID, []).append(csvID)

	@classmethod
	def active(cls, yyObj, game):
		"""初始化 stamps 数据，前端需要 stamps[csvID] 存在才显示"""
		record = cls.refreshRecord(yyObj, game)
		stamps = record.setdefault('stamps', {})
		csvIDs = cls.HuoDongMap.get(yyObj.huodongID, [])
		for csvID in csvIDs:
			if csvID not in stamps:
				stamps[csvID] = 0  # 初始化购买次数为 0
	...
	def refreshRecord(cls, yyObj, game):
		...
		if ndi != record.get('lastday', 0):
			...
			for csvID in csvIDs:
				cfg = cls.csv(csvID)
				if cfg.refresh:
					stamps[csvID] = 0  # 重置购买次数
				elif csvID not in stamps:
					stamps[csvID] = 0  # 首次初始化
			record['lastday'] = ndi
		return record
	...
	def getEffect(cls, yyID, csvID, game):
		...
		if cfg.huodongID != yyObj.huodongID:
			raise ClientError('huodongID error')
		...
		costMap = cfg.costMap
		if costMap:
			cost = ObjectCostAux(game, costMap)
			if not cost.isEnough():
				raise ClientError(ErrDefs.dailyBuyRMBNotEnough)
			cost.cost(src='yy_daily_random_gift_%d' % yyID)

		def _afterGain():
			stamps[csvID] = stamps.get(csvID, 0) + 1

		return ObjectYYHuoDongEffect(game, cfg.items, _afterGain)
...
#
# ObjectYYRoundDraw - 启示之塔
#

class ObjectYYRoundDraw(ObjectYYBase):
	'''
	启示之塔 - 阶梯抽奖
	...
	数据结构:
	{
		...
		valsums: {posID: csvID},  # 选择的奖品
		stamps: {posID: 1},       # 已抽中的位置
		stamps1: {csvID: times}   # 一等奖获取次数
	}
	...
	HuoDongMap = {}  # {huodongID: {round: {floor: {csvID: weight}}}}
	AwardMap = {}    # {huodongID: {csvID: cfg}}

	@classmethod
	def csv(cls, csvID=None):
		if csvID is None:
			return csv.yunying.round_draw_award
		return csv.yunying.round_draw_award[csvID]
	...
	def classInit(cls):
		...
		for idx in csv.yunying.round_draw_award:
			cfg = csv.yunying.round_draw_award[idx]
			huodongID = cfg.huodongID
			cls.AwardMap.setdefault(huodongID, {})[idx] = cfg

			for roundID in cfg.rounds:
				cls.HuoDongMap.setdefault(huodongID, {})
				cls.HuoDongMap[huodongID].setdefault(roundID, {})
				cls.HuoDongMap[huodongID][roundID].setdefault(cfg.floor, {})
				cls.HuoDongMap[huodongID][roundID][cfg.floor][idx] = cfg.weight
	...
	def refreshRecord(cls, yyObj, game, init=None):
		...
		info = record.setdefault('info', {})
		# 确保 info 中有必要的字段
		info.setdefault('round', 1)
		info.setdefault('drawNum', 0)
		info.setdefault('next', 0)
		record.setdefault('valsums', {})
		record.setdefault('stamps', {})
		record.setdefault('stamps1', {})

		# 初始化当前轮的奖品池
		if not record['valsums']:
			cls._initRoundAwards(yyObj, game, record)

		return record

	@classmethod
	def _initRoundAwards(cls, yyObj, game, record):
		'''初始化当前轮的奖品'''
		huodongID = yyObj.huodongID
		roundID = record['info']['round']
		stamps1 = record.get('stamps1', {})

		if huodongID not in cls.HuoDongMap:
			return
		if roundID not in cls.HuoDongMap[huodongID]:
			return

		roundData = cls.HuoDongMap[huodongID][roundID]
		valsums = record['valsums']

		# 三等奖随机选择3个，四等奖随机选择4个
		floorCounts = {3: 3, 4: 4}
		for floor, count in floorCounts.items():
			if floor not in roundData:
				continue
			pool = copy.deepcopy(roundData[floor])
			pos = floor * 100 + 1
			for i in range(min(count, len(pool))):
				...
				csvID, _ = WeightRandomObject.onceRandom(pool)
				valsums[pos] = csvID
				del pool[csvID]  # 避免重复选择
				pos += 1

	@classmethod
	def choose(cls, yyID, game, posID, csvID):
		'''选择一等奖或二等奖位置的奖品'''
		yyObj = ObjectYYHuoDongFactory.getConfig(yyID)
		if yyObj is None:
			raise ClientError(ErrDefs.huodongNoOpen)
		...
		valsums = record['valsums']
		stamps = record['stamps']
		stamps1 = record['stamps1']

		# 检查是否已领取
		if posID in stamps:
			raise ClientError('already got')
		...
		# 检查csvID是否合法
		cfg = cls.csv(csvID)
		if cfg is None or cfg.huodongID != yyObj.huodongID:
			raise ClientError('invalid csvID')
		if cfg.floor != floor:
			raise ClientError('floor mismatch')

		# 一等奖检查轮数和次数限制
		if floor == 1:
			roundID = info['round']
			if roundID not in cfg.rounds:
				raise ClientError('round not match')
			# 检查次数限制
			if cfg.limit > 0 and stamps1.get(csvID, 0) >= cfg.limit:
				raise ClientError('limit reached')
		...
		if floor == 2:
			...
			if roundID not in cfg.rounds:
				raise ClientError('round not match')
			if cfg.position != (posID % 100):
				raise ClientError('position mismatch')

		valsums[posID] = csvID

	@classmethod
	def draw(cls, yyID, game):
		'''抽奖'''
		yyObj = ObjectYYHuoDongFactory.getConfig(yyID)
		if yyObj is None:
			raise ClientError(ErrDefs.huodongNoOpen)
		...
		paramMap = csv.yunying.yyhuodong[yyID].paramMap
		costItem = paramMap.get('costItem', 0)
		maxRound = paramMap.get('maxRound', 1)

		# 检查是否选择了一等奖和二等奖
		if 101 not in valsums:
			raise ClientError('first award not selected')
		if 201 not in valsums or 202 not in valsums:
			raise ClientError('second award not selected')

		# 检查是否已完成本轮
		if info['next'] == 1:
			raise ClientError('round completed')

		# 计算消耗
		drawNum = info['drawNum']
		costNum = ObjectCostCSV.getRoundDrawCost(drawNum)
		...
		# 构建抽奖池（未抽中的位置）
		pool = {}
		for posID, csvID in valsums.items():
			if posID not in stamps:
				cfg = cls.csv(csvID)
				pool[posID] = cfg.weight if cfg else 1

		if not pool:
			raise ClientError('no available award')

		# 随机抽取
		hitPosID, _ = WeightRandomObject.onceRandom(pool)
		hitCsvID = valsums[hitPosID]
		hitCfg = cls.csv(hitCsvID)

		# 发放奖励
		eff = ObjectGainAux(game, hitCfg.award)

		def _afterGain():
			cost.cost(src='yy_round_draw_%d' % yyID)
			stamps[hitPosID] = 1
			info['drawNum'] += 1

			# 一等奖记录次数
			hitFloor = hitPosID // 100
			if hitFloor == 1:
				stamps1[hitCsvID] = stamps1.get(hitCsvID, 0) + 1
				# 不设置 info['next'] = 1，让玩家可以继续抽当前轮

			# 检查是否全部抽完，自动进入下一轮
			allGot = all(posID in stamps for posID in valsums)
			if allGot:
				if info['round'] < maxRound:
					# 自动进入下一轮
					info['round'] += 1
					info['drawNum'] = 0
					info['next'] = 0
		...
		return ObjectYYHuoDongEffect(game, eff.to_dict(), _afterGain), hitCsvID

	@classmethod
	def nextRound(cls, yyID, game, confirm=True):
		'''进入下一轮'''
		yyObj = ObjectYYHuoDongFactory.getConfig(yyID)
		if yyObj is None:
			raise ClientError(ErrDefs.huodongNoOpen)
		...
		# 检查一等奖是否已抽中（一等奖位置固定为101）
		if 101 not in stamps:
			raise ClientError('cannot go next round')

		if info['round'] >= maxRound:
			raise ClientError('max round reached')
...
class ObjectYYHuoDongFactory(ReloadHooker):
	...
	# 兼容缺少枚举的旧版本
	if not hasattr(YYHuoDongDefs, 'LimitedSkinShop'):
		YYHuoDongDefs.LimitedSkinShop = 71
	...
	ClassMap = {
		...
		YYHuoDongDefs.LimitedSkinShop: ObjectYYDirectBuyGift,  # 限时皮肤商店，复用直购礼包
		YYHuoDongDefs.DailyRandomGift: ObjectYYDailyRandomGift,  # 每日随机礼包
		YYHuoDongDefs.RoundDraw: ObjectYYRoundDraw,  # 启示之塔
		YYHuoDongDefs.Refloat: ObjectYYRefloat,  # 惊喜捞一捞
		YYHuoDongDefs.RoundFlop: ObjectYYRoundFlop,  # 惊喜连连
	}
	...
	def classInit(cls):
		...
		ObjectYYDailyRandomGift.classInit()  # 每日随机礼包
		ObjectYYRoundDraw.classInit()  # 启示之塔
		ObjectYYRefloat.classInit()  # 惊喜捞一捞
		ObjectYYRoundFlop.classInit()  # 惊喜连连
	...
	def onLogin(cls, game):
		...
		cls._activeOpen(YYHuoDongDefs.DailyRandomGift, game)  # 每日随机礼包（登录惊喜）
		cls._activeOpen(YYHuoDongDefs.RoundDraw, game)  # 启示之塔
	...
	def onActiveGet(cls, game):
		...
		# 限时皮肤商店
		cls._activeOpen(YYHuoDongDefs.LimitedSkinShop, game)
		# 惊喜连连
		cls._activeOpen(YYHuoDongDefs.RoundFlop, game)
