...
class ObjectBadge(ObjectBase):
	...
	def getBadgeAttrs(self):
		...
		if self._effects == None:
			def _safe_level_value(values, level, badgeID, key):
				if not values:
					return None
				idx = level - 1
				if idx < 0:
					return None
				if idx >= len(values):
					logger.warning('[Badge getBadgeAttrs] level out of range badgeID=%s key=%s level=%s len=%s',
						badgeID, key, level, len(values))
					idx = len(values) - 1
				return values[idx]
			...
			for badgeID, badge in self._badges.iteritems():
				...
				if awakeLevel > 0:
					# 勋章觉醒对天赋属性的百分比加成
					awakeAttr = _safe_level_value(badgeCfg.awakeTalentAttrs, awakeLevel, badgeID, 'awakeTalentAttrs')
					if awakeAttr:
						awakePercent = str2num_t(awakeAttr)[1]
					...
					for i in xrange(1, 99):
						...
						attrNum = _safe_level_value(badgeCfg[attrNumKey], awakeLevel, badgeID, attrNumKey)
						if attrNum is None:
							continue
					...
					if level > 0:
						...
						for i in xrange(1, 99):
							...
							if attrKey not in cfg or not cfg[attrNumKey]:
								break
							attr = cfg[attrKey]
							attrNum = _safe_level_value(cfg[attrNumKey], level, badgeID, attrNumKey)
							if attrNum is None:
								continue
							num = str2num_t(attrNum)
	...
	def updateAllCardsAttrs(self):
		...
		try:
			...
			const, percent = self.getBadgeAttrs()
			...
			logger.error('[Badge updateAllCardsAttrs] 更新失败: %s', str(e))
