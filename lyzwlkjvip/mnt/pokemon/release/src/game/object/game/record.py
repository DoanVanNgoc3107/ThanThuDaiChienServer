...
class ObjectDailyRecord(ObjectDBase):
	...
	def init(self):
		...
		if self._db:
			if self._db.get('cross_supremacy_match_times', None) is None:
				self.cross_supremacy_match_times = 0
			if self._db.get('cross_supremacy_purchased_times', None) is None:
				self.cross_supremacy_purchased_times = 0
			if self._db.get('cross_supremacy_change_times', None) is None:
				self.cross_supremacy_change_times = 0
		return self
	...
	# 拟态对战次数
	mimicry_battle_times = db_property('mimicry_battle_times')
	...
	# 非好友挑战次数
	not_friend_fight_times = db_property('not_friend_fight_times')
	...
	# 跨服聊天次数
	cross_chat_times = db_property('cross_chat_times')
	...
	# 今日是否使用过日常助手（用于勋章统计）
	daily_assistant_used = db_property('daily_assistant_used')
	...
	# 图腾钻石免费单抽
	totem_rmb_dc1_free_count = db_property('totem_rmb_dc1_free_count')
	
	# 图腾抽卡总次数
	draw_totem = db_property('draw_totem')
	...
	# 琉石钻石单抽免费次数
	emera_rmb_dc1_free_count = db_property('emera_rmb_dc1_free_count')

	# 琉石钻石抽取次数
	draw_emera_rmb = db_property('draw_emera_rmb')

	# 琉石铸型次数
	comb_emera = db_property('comb_emera')
	...
	# 跨服冠军赛匹配次数
	def cross_supremacy_match_times():
		dbkey = 'cross_supremacy_match_times'
		def fget(self):
			return self._db.get(dbkey, 0)
		def fset(self, value):
			self._db[dbkey] = value
		return locals()
	cross_supremacy_match_times = db_property(**cross_supremacy_match_times())

	# 跨服冠军赛额外购买次数
	def cross_supremacy_purchased_times():
		dbkey = 'cross_supremacy_purchased_times'
		def fget(self):
			return self._db.get(dbkey, 0)
		def fset(self, value):
			self._db[dbkey] = value
		return locals()
	cross_supremacy_purchased_times = db_property(**cross_supremacy_purchased_times())

	# 跨服冠军赛换一批次数
	def cross_supremacy_change_times():
		dbkey = 'cross_supremacy_change_times'
		def fget(self):
			return self._db.get(dbkey, 0)
		def fset(self, value):
			self._db[dbkey] = value
		return locals()
	cross_supremacy_change_times = db_property(**cross_supremacy_change_times())
	...
	# 城镇商店刷新次数
	town_shop_refresh_times = db_property("town_shop_refresh_times")

	# 城镇许愿次数
	town_wish_times = db_property("town_wish_times")

	# 今日是否已进入家园（用于遗迹祝福每日首次抽取）
	into_town_daily = db_property("into_town_daily")

	# 家园社交 - 今日已点赞的玩家ID列表
	def town_home_role_liked():
		dbkey = 'town_home_role_liked'
		def fget(self):
			return self._db.get(dbkey, [])
		def fset(self, value):
			self._db[dbkey] = value
		return locals()
	town_home_role_liked = db_property(**town_home_role_liked())

	# 家园访问冷却记录 {townDBID: timestamp}
	def town_home_visit_times():
		dbkey = 'town_home_visit_times'
		def fget(self):
			return self._db.get(dbkey, {})
		def fset(self, value):
			self._db[dbkey] = value
		return locals()
	town_home_visit_times = db_property(**town_home_visit_times())

	# 跨服资源战21点游戏次数
	cross_mine_blackjack_play_times = db_property("cross_mine_blackjack_play_times")
	...
	def renew(self):
		...
		self.daily_assistant_used = 0  # 今日是否使用过日常助手
		...
		self.totem_rmb_dc1_free_count = 0
		self.draw_totem = 0
		self.draw_chip = 0
		self.emera_rmb_dc1_free_count = 0
		self.draw_emera_rmb = 0
		self.comb_emera = 0
		...
		self.cross_mine_blackjack_play_times = 0
		...
		self.town_shop_refresh_times = 0
		self.town_wish_times = 0
		self.into_town_daily = False
		self.town_home_role_liked = []
		self.town_home_visit_times = {}
...
class ObjectLotteryRecord(ObjectDBase):
	...
	def drawCard(self, drawType, half=False):
		...
		# 许愿簿：钻石抽卡时读取自选卡牌，提升概率
		customCardIDs = None
		if drawType in (DrawCardDefs.RMB1, DrawCardDefs.RMB10, DrawCardDefs.Free1):
			customCardIDs = self._getCustomCardIDs()

		return ObjectDrawCardRandom.getRandomItems(self.game, drawType, drawTimes + 1, _afterGain, customCardIDs)
	...
	# 图腾钻石单抽计数器
	totem_rmb_dc1_counter = db_property('totem_rmb_dc1_counter')
	
	# 图腾钻石6连抽计数器
	totem_rmb_dc6_counter = db_property('totem_rmb_dc6_counter')

	# 琉石钻石单抽计数器
	emera_rmb_dc1_counter = db_property('emera_rmb_dc1_counter')

	# 琉石钻石5连抽计数器
	emera_rmb_dc5_counter = db_property('emera_rmb_dc5_counter')
	...
	def drawTotem(self):
		"""
		图腾抽奖 - 从totem.shop配置的权重池中随机抽取图腾符号
		"""
		from framework.helper import WeightRandomObject
		
		# 检查是否有totem配置
		if not hasattr(csv, 'totem') or not hasattr(csv.totem, 'shop'):
			raise ServerError('totem config not found')
		
		# 从totem.shop获取抽奖池（ID 1000-1700）
		# 这些行的itemWeightMap配置了图腾符号ID和权重
		# ID 1000-1200: 1星符号池 (8301-8344)
		# ID 1300-1500: 2星符号池 (8401-8440)
		# ID 1600: 3星符号池 (8501-8514)
		# ID 1700: 4星符号池 (8601-8614)
		weight_pools = []
		for shop_id in [1000, 1100, 1200, 1300, 1400, 1500, 1600, 1700]:
			if shop_id in csv.totem.shop:
				cfg = csv.totem.shop[shop_id]
				if cfg.itemWeightMap:
					weight_pools.append(cfg.itemWeightMap)
		
		if not weight_pools:
			raise ServerError('totem draw pool not configured')
		
		# 随机选择一个池子
		import random
		pool = random.choice(weight_pools)
		
		# 从选中的池子中按权重随机抽取一个图腾符号
		rnd = WeightRandomObject(pool)
		symbol_id, _ = rnd.getRandom()
		
		# 返回抽奖结果
		eff = ObjectGainAux(self.game, {symbol_id: 1})
		return eff
	...
	# 许愿簿 - 钻石池自选 {index: [libID, ...], ...}
	draw_rmb_custom = db_property('draw_rmb_custom')

	# 许愿簿 - UP池自选 {index: [libID, ...], ...}
	draw_group_up_custom = db_property('draw_group_up_custom')

	# 许愿簿 - 限时自选 [yyID, cardID]
	draw_limit_custom = db_property('draw_limit_custom')

	# 许愿簿 - UP池自选单抽计数器
	group_up_custom_rmb1_counter = db_property('group_up_custom_rmb1_counter')

	# 许愿簿 - UP池自选十连计数器
	group_up_custom_rmb10_counter = db_property('group_up_custom_rmb10_counter')

	def init(self):
		"""初始化 - 确保数组字段不为 None（Go 端数组类型不能接收 nil）"""
		ObjectDBase.init(self)
		if self._db:
			# draw_limit_custom 是 []document.Integer，不能为 None
			if self._db.get('draw_limit_custom') is None:
				self._db['draw_limit_custom'] = []
		return self

	def _getCustomCardIDs(self):
		"""获取许愿簿选中的卡牌ID列表（用于钻石池概率提升）"""
		customData = self.draw_rmb_custom
		if not customData:
			return None
		cardIDs = []
		# customData 格式: {1: [libID, ...]}
		for libIDs in customData.itervalues():
			if not libIDs:
				continue
			for libID in libIDs:
				if libID in csv.tag_items_lib:
					cardID = csv.tag_items_lib[libID].cardID
					if cardID and cardID > 0:
						cardIDs.append(cardID)
		return cardIDs if cardIDs else None

	def _getLimitCustomCardID(self, yyID):
		"""获取许愿簿限时自选卡牌ID（用于神兽召唤概率提升）"""
		# draw_limit_custom 格式: [yyID, cardID]
		customData = self.draw_limit_custom
		if not customData or len(customData) != 2:
			return None
		storedYYID, cardID = customData[0], customData[1]
		# 验证 yyID 是否匹配
		if storedYYID != yyID:
			return None
		return [cardID] if cardID and cardID > 0 else None
	...
	def drawCardGroupUpCustom(self, drawType):
		"""自选抽卡 - 11% S级(选中的5张) + 36% A级 + 53% B级"""
		import random
		from game.object.game.lottery import ObjectDrawRandomItem, ObjectDrawEffect

		# 检查是否已选择卡牌
		customData = self.draw_group_up_custom
		if not customData:
			raise ClientError('please choose cards first')

		# 获取选中的卡牌ID列表
		selectedCardIDs = []
		for _, libIDs in customData.iteritems():
			for libID in libIDs:
				if libID in csv.tag_items_lib:
					cardID = csv.tag_items_lib[libID].cardID
					if cardID and cardID > 0:
		...
		if len(selectedCardIDs) < 5:
			raise ClientError('please choose 5 cards')

		# 确定抽取次数和消耗
		drawCount = 1
		if drawType == DrawCardDefs.GroupUpCustomRMB1:
			if self.game.items.getItemCount(DrawCardDefs.LimitUpDrawItem) < 1:
				if self.game.role.rmb < ConstDefs.drawCardUp1CostPrice:
					raise ClientError(ErrDefs.drawCardRMBNotEnough)

		elif drawType == DrawCardDefs.GroupUpCustomRMB10:
			drawCount = 10
			if self.game.items.getItemCount(DrawCardDefs.LimitUpDrawItem) < 10:
				if self.game.role.rmb < ConstDefs.drawCardUp10CostPrice:
					raise ClientError(ErrDefs.drawCardRMBNotEnough)
		...
		def _afterGain():
			if drawType == DrawCardDefs.GroupUpCustomRMB1:
				self.group_up_custom_rmb1_counter = (self.group_up_custom_rmb1_counter or 0) + 1
				cost = ObjectCostAux(self.game, {DrawCardDefs.LimitUpDrawItem: 1})
				if not cost.isEnough():
					cost = ObjectCostAux(self.game, {'rmb': ConstDefs.drawCardUp1CostPrice})
				cost.cost(src='draw_card_%s' % drawType)
				...
				self.group_up_custom_rmb10_counter = (self.group_up_custom_rmb10_counter or 0) + 1
				cost = ObjectCostAux(self.game, {DrawCardDefs.LimitUpDrawItem: 10})
				if not cost.isEnough():
					cost = ObjectCostAux(self.game, {'rmb': ConstDefs.drawCardUp10CostPrice})
				cost.cost(src='draw_card_%s' % drawType)
		...
		def _drawOneCard():
			...
			if roll <= 11:
				# 11% 概率抽选中的 S 级卡
				cardID = random.choice(selectedCardIDs)
			elif roll <= 47:  # 11 + 36 = 47
				# 36% 概率抽 A 级卡池 (205)
				libObj = ObjectDrawRandomItem.getObject(205)
				if libObj:
					ret = libObj.getRandomItem(self.game)
					if ret and ret[0]:
						cardID = ret[0].get('id') if isinstance(ret[0], dict) else ret[0]
			else:
				# 53% 概率抽 B 级卡池 (206)
				libObj = ObjectDrawRandomItem.getObject(206)
				if libObj:
					ret = libObj.getRandomItem(self.game)
					if ret and ret[0]:
						cardID = ret[0].get('id') if isinstance(ret[0], dict) else ret[0]
		...
		# 构建返回结果
		if drawCount == 1:
			# card 格式需要是 {'id': cardID} 而不是整数
			eff = ObjectDrawEffect(self.game, {'card': {'id': cardIDs[0]}}, grids, _afterGain)
		else:
			# cards 格式需要是 [{'id': cardID}, ...] 而不是 [cardID, ...]
			cards = [{'id': cid} for cid in cardIDs]
			eff = ObjectDrawEffect(self.game, {'cards': cards}, grids, _afterGain)
		return eff
	...
	def drawCounterSum(self, drawType):
		if drawType == DrawSumBoxDefs.RMBType:
			...
			for _, v in self.draw_card_up10_counters.iteritems():
				count = count + v*10
			# 自选抽卡也计入 UP 抽卡累计
			count += (self.group_up_custom_rmb1_counter or 0)
			count += (self.group_up_custom_rmb10_counter or 0) * 10
...
class ObjectWeeklyRecord(ObjectDBase):
	DBModel = 'WeeklyRecord'

	# 周标识 (年*100+周数)
	week = db_property('week')

	# 世界boss周最大伤害
	world_boss_damage_max = db_property('world_boss_damage_max')

	# 家园社交 - 本周点赞次数
	def town_home_likes():
		dbkey = 'town_home_likes'
		def fget(self):
			return self._db.get(dbkey, 0)
		def fset(self, value):
			self._db[dbkey] = value
		return locals()
	town_home_likes = db_property(**town_home_likes())

	# 家园社交 - 本周已领取的点赞奖励ID {csvID: 1}
	def town_home_like_award():
		dbkey = 'town_home_like_award'
		def fget(self):
			return self._db.get(dbkey, {})
		def fset(self, value):
			self._db[dbkey] = value
		return locals()
	town_home_like_award = db_property(**town_home_like_award())

	# 家园社交 - 本周评分记录 {townDBID: scoreValue}
	def town_home_score():
		dbkey = 'town_home_score'
		def fget(self):
			return self._db.get(dbkey, {})
		def fset(self, value):
			self._db[dbkey] = value
		return locals()
	town_home_score = db_property(**town_home_score())

	# 跨服冠军赛 周奖励领取记录 {csvID: 1(可领)/2(已领)}
	def cross_supremacy_week_award():
		dbkey = 'cross_supremacy_week_award'
		def fget(self):
			return self._db.get(dbkey, {})
		def fset(self, value):
			self._db[dbkey] = value
		return locals()
	cross_supremacy_week_award = db_property(**cross_supremacy_week_award())

	# 跨服冠军赛 本周战斗次数
	def cross_supremacy_week_fight():
		dbkey = 'cross_supremacy_week_fight'
		def fget(self):
			return self._db.get(dbkey, 0)
		def fset(self, value):
			self._db[dbkey] = value
		return locals()
	cross_supremacy_week_fight = db_property(**cross_supremacy_week_fight())

	def init(self):
		ObjectDBase.init(self)
		if self._db:
			if self._db.get('cross_supremacy_week_award', None) is None:
				self.cross_supremacy_week_award = {}
			if self._db.get('cross_supremacy_week_fight', None) is None:
				self.cross_supremacy_week_fight = 0
		self._initWeek()
		return self
	...
	def _initWeek(self):
		...
		if self.week != currentWeek:
			self.reset()
			self.week = currentWeek

	def reset(self):
		self.world_boss_damage_max = 0
		self.town_home_likes = 0
		self.town_home_like_award = {}
		self.town_home_score = {}
		self.cross_supremacy_week_award = {}
		self.cross_supremacy_week_fight = 0
