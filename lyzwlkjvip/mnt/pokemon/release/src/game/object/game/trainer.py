...
class ObjectTrainer(ObjectBase):
	...
	def updateAllCardsAttrs(self):
		...
		try:
			...
			logger.error('[Trainer updateAllCardsAttrs] 更新失败: %s', str(e))
	...
	# 契约副本次数增加
	@property
	def huodongTypeContractTimes(self):
		return self._privilege.get(TrainerDefs.HuodongTypeContractTimes, 0)

	# 契约副本产出增加(百分比)
	@property
	def huodongTypeContractDropRate(self):
		return self._privilege.get(TrainerDefs.HuodongTypeContractDropRate, 0)
