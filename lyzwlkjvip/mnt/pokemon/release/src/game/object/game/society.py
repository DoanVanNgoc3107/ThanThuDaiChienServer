...
class ObjectSociety(ObjectDBase):
	...
	# 跨服聊天黑名单列表 [[game_key, Role.id], ...]
	def cross_black_list():
		dbkey = 'cross_black_list'
		def fset(self, value):
			self.db[dbkey] = value
		return locals()
	cross_black_list = db_property(**cross_black_list())
...
class ObjectSocietyGlobal(ReloadHooker):
	...
	def onRoleInfo(cls, game):
		d = Dict({
			...
			'town_db_id': game.role.town_db_id,  # 家园数据库ID
			'town_home_visit': game.role.town_db_id,  # 家园拜访入口需要
			'town_home_decorativeness': game.role.town_home_decorativeness or 0,  # 家园装饰度
			'town_home_layout_version': game.role.town_home_layout_version or 0,  # 家园布局版本
			'area': game.role.area,  # 区服，用于计算 game_key
			'game_key': game.role.areaKey,  # 直接存储 game_key
		})
	...
	def onRoleInfoByModel(cls, d):
		...
		keys = ('id', 'uid', 'name', 'last_time', 'logo', 'frame', 'figure', 'level', 'vip_level', 'battle_fighting_point', 'pvp_record_db_id', 'top6_fighting_point', 'reunion_open', 'town_db_id', 'area', 'town_home_decorativeness', 'town_home_layout_version')
		model = Dict(copyKV(d, keys=keys))
		if d.get('vip_hide', False):
			model['vip_level'] = 0
		# 添加 town_home_visit 字段（家园拜访入口需要）
		model['town_home_visit'] = d.get('town_db_id', None)
		# 添加家园相关字段（前端 ctorHomeFriendData 需要）
		model['town_home_decorativeness'] = d.get('town_home_decorativeness', 0) or 0
		model['town_home_layout_version'] = d.get('town_home_layout_version', 0) or 0
		# 计算并存储 game_key
		area = d.get('area', 1)
		service, language, _ = service_key2domains(Server.Singleton.key)
		model['game_key'] = gamemerge2game(service_key(service, area, language))
