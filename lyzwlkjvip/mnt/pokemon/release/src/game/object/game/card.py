...
class CardSlim(object):
	...
	def __init__(self, slim):
		...
		self.starEffectIndex = csvCard.starEffectIndex
...
class ObjectCard(ObjectDBase):
	...
	CardAbilityExtraMap = {}  # {abilitySeqID: cfg}  潜能配置映射
	BattleCardHaloAttrMap = {} # 编队光环属性加成
	SkillPassiveAttrsCache = {} # {passiveSkillID: {attr: lambda}}
	StarSkillAttrCache = {} # {(skillID, level): {attr: (const, percent)}} 极限点技能属性缓存
	...
	def classInit(cls):
		...
		# 潜能配置映射
		cls.CardAbilityExtraMap = {}
		if hasattr(csv, 'card_ability_extra'):
			for csvID in csv.card_ability_extra:
				if csvID == '__default':
					continue
				cfg = csv.card_ability_extra[csvID]
				# abilitySeqIDGroup 是元组，每个 abilitySeqID 都映射到这个配置
				for abilitySeqID in cfg.abilitySeqIDGroup:
					cls.CardAbilityExtraMap[abilitySeqID] = cfg
		...
		for skillID in csv.skill:
			...
			for attrE, val in cfg.passiveSkillAttrs.iteritems():
				...
				attrs[attrE] = param
			# 只有当 attrs 非空时才添加（避免空配置覆盖 StarSkillAttrCache）
			if attrs:
				cls.SkillPassiveAttrsCache[skillID] = attrs

		# 极限点技能属性表 card_star_skill_attr.csv
		cls.StarSkillAttrCache = {}
		if hasattr(csv, 'card_star_skill_attr'):
			logger.info('[StarSkillAttr] Loading card_star_skill_attr.csv...')
			for csvID in csv.card_star_skill_attr:
				cfg = csv.card_star_skill_attr[csvID]
				skillID = cfg.skillID
				level = cfg.level
				if skillID == 0 or level == 0:
					continue
				attrs = {}
				# 遍历 attrType1-3 和 attrNum1-3
				for i in range(1, 4):
					attrType = getattr(cfg, 'attrType%d' % i, 0)
					attrNum = getattr(cfg, 'attrNum%d' % i, '')
					if attrType == 0 or not attrNum:
						continue
					# 解析属性值（支持百分比和固定值）
					if isinstance(attrNum, str):
						if attrNum.endswith('%'):
							# 百分比属性
							try:
								percent = float(attrNum[:-1]) / 100.0
								attrs[attrType] = (0, percent)
							except:
								pass
						else:
							# 固定值
							try:
								const = float(attrNum)
								attrs[attrType] = (const, 0)
							except:
								pass
					else:
						# 数值类型（固定值）
						attrs[attrType] = (float(attrNum), 0)
				if attrs:
					cls.StarSkillAttrCache[(skillID, level)] = attrs
			logger.info('[StarSkillAttr] Loaded %d entries into StarSkillAttrCache', len(cls.StarSkillAttrCache))
		else:
			logger.warning('[StarSkillAttr] csv.card_star_skill_attr not found!')
	...
	def init(self):
		...
		if self.db_attrs:
			...
			self._display = False
		if 'branch_switch_last_time' not in self.db:
				self.db['branch_switch_last_time'] = 0
		if self.held_item:
		...
		self._initExp()
		self._initAttrs()
		self._initContractSlots()  # 初始化契约槽位
	...
	def _fixCorrupted(self):
		...
		# 验证陨石索引的一致性（参考held_item的meteorite_index验证）
		if hasattr(self, 'meteorite_index') and self.meteorite_index and self.meteorite_index > 0:
			# 获取陨石数据
			if hasattr(self.game, 'role') and hasattr(self.game.role, 'meteorites'):
				meteorites = self.game.role.meteorites or {}
				meteorite_data = meteorites.get(self.meteorite_index)
				
				if meteorite_data:
					# 检查陨石槽位是否真的有这个卡牌
					actual_card_id = meteorite_data.get('card')
					if actual_card_id != self.id:
						# 数据不一致，清理卡牌的陨石引用
						logger.warning('role %s card %s meteorite_index=%d inconsistent, actual=%s, clearing' % 
									 (objectid2string(self.game.role.id), objectid2string(self.id), 
									  self.meteorite_index, actual_card_id))
						self.meteorite_index = 0
				else:
					# 陨石不存在，清理引用
					logger.debug('role %s card %s meteorite_index=%d not exist, clearing' % 
							   (objectid2string(self.game.role.id), objectid2string(self.id), self.meteorite_index))
					self.meteorite_index = 0
		
		# 清理无效的宝石引用（宝石可能已被删除但卡牌上的引用还在）
		# 同时修复宝石的 card_db_id 不一致问题
		if self.gems:
			valid_gems = {}
			for pos, gemID in self.gems.iteritems():
				gem = self.game.gems.getGem(gemID)
				if gem and gem.exist_flag:
					valid_gems[pos] = gemID
					# 修复宝石的 card_db_id 不一致（卡牌有引用但宝石没记录）
					if gem.card_db_id != self.id:
						logger.warning('role %s card %s gems[%s] card_db_id mismatch (gem.card_db_id=%s), fixed' % 
									 (objectid2string(self.game.role.id), objectid2string(self.id), pos, gem.card_db_id))
						gem.card_db_id = self.id
				else:
					logger.warning('role %s card %s gems[%s] invalid gemID, cleaned' % 
								 (objectid2string(self.game.role.id), objectid2string(self.id), pos))
			if len(valid_gems) != len(self.gems):
				self.gems = valid_gems
		
		# 清理无效的契约引用（契约可能已被删除但卡牌槽位上的引用还在）
		# 同时修复契约的 card_db_id 不一致问题
		if self.contracts:
			need_fix = False
			for pos, slot_info in self.contracts.iteritems():
				if slot_info and slot_info.get('contract_db_id'):
					contract_db_id = slot_info['contract_db_id']
					contract = self.game.contracts.getContract(contract_db_id)
					if not contract or not contract.exist_flag:
						logger.warning('role %s card %s contracts[%s] invalid contractID, cleaned' % 
									 (objectid2string(self.game.role.id), objectid2string(self.id), pos))
						self.contracts[pos]['contract_db_id'] = None
						need_fix = True
					elif contract.card_db_id != self.id:
						# 修复契约的 card_db_id 不一致
						logger.warning('role %s card %s contracts[%s] card_db_id mismatch (contract.card_db_id=%s), fixed' % 
									 (objectid2string(self.game.role.id), objectid2string(self.id), pos, contract.card_db_id))
						contract.card_db_id = self.id
						contract.position = int(pos) if isinstance(pos, str) else pos
			if need_fix:
				self.contracts = self.contracts  # 触发保存
		
		# 清理无效的琉石引用（琉石可能已被删除但卡牌上的引用还在）
		# 同时修复琉石的 card_db_id 不一致问题
		if self.emeras:
			valid_emeras = {}
			for pos, emeraID in self.emeras.iteritems():
				emera = self.game.emeras.getEmera(emeraID)
				if emera and emera.exist_flag:
					valid_emeras[pos] = emeraID
					# 修复琉石的 card_db_id 不一致
					if emera.card_db_id != self.id:
						logger.warning('role %s card %s emeras[%s] card_db_id mismatch (emera.card_db_id=%s), fixed' % 
									 (objectid2string(self.game.role.id), objectid2string(self.id), pos, emera.card_db_id))
						emera.card_db_id = self.id
				else:
					logger.warning('role %s card %s emeras[%s] invalid emeraID, cleaned' % 
								 (objectid2string(self.game.role.id), objectid2string(self.id), pos))
			if len(valid_emeras) != len(self.emeras):
				self.emeras = valid_emeras
	...
	def calcNValueAttrsAddition(card):
		...
		z8_active = ObjectCard._isZ8Stage(card)
		zawake_nvalue = getattr(card, 'zawake_nvalue', {}) or {}
		d = {}
		for attr, value in card.nvalue.iteritems():
			raw_value = value
			base_value = raw_value + (8 if z8_active else 0)
			value = base_value + zawake_nvalue.get(attr, 0)
			value = (value + 1) * 1.0 / 32 * 0.32 - 0.16
			if raw_value == 31: # 单项完美（仅看原始个体值）
				value += 0.05
			if flag: # 六项全完美
				value += 0.05
			d[attr] = value
	...
	def _isZ8Stage(card):
		...
		if not zawakeID:
			return False
		zawakeData = card.game.role.zawake.get(zawakeID, {})
		return zawakeData.get(8, 0) > 0

	def _getDisplayNvalue(self, base=None):
		# 仅返回原始个体值（0-31），Z觉醒的额外个体值单独走 zawake_nvalue
		return dict(base or self.nvalue or {})

	@property
	def modelSync(self):
		dbNew, dbUpd, dbDel = self.dbSync
		memNew, memUpd, memDel = self.memSync
		if dbUpd and ('nvalue' in dbUpd or 'zawake_nvalue' in dbUpd):
			dbUpd = dict(dbUpd)
			dbUpd['nvalue'] = self._getDisplayNvalue(dbUpd.get('nvalue', None))
	...
	@staticmethod
	def calcExtraEffortAttrsAddition(card):
		'''
		计算额外努力值属性加成（极限突破）
		'''
		from framework.csv import csv
		
		# CardSlim（随机塔敌人）没有 db 属性，跳过
		if not hasattr(card, 'db'):
			return
		
		extra_effort_level = card.db.get('extra_effort_level', None)
		if not extra_effort_level or extra_effort_level == 0:
			return
		
		extra_effort_values = card.db.get('extra_effort_values', None)
		if not extra_effort_values:
			extra_effort_values = {}
		
		# 从配置中读取该等级的属性加成
		if extra_effort_level not in csv.card_effort_extra:
			return
		
		cfg = csv.card_effort_extra[extra_effort_level]
		
		# 计算属性加成（基础努力值 + 额外努力值培养）
		constD = {}
		for attrType in EffortAttrs:
			baseValue = getattr(cfg, attrType, 0)
			extraValue = extra_effort_values.get(attrType, 0)
			constD[attrType] = baseValue + extraValue
		
		const = dict2attrs(constD)
		card.calc.const.set('extra_effort', const)
	...
	@staticmethod
	def calcZawakeNvalue(card, zawake):
		'''
		计算Z觉醒提供的个体值加成
		从 levels.csv 的 nvalue 字段读取并累加
		nvalue 格式：{attrEnum: value}，例如 {1: 8, 7: 8, 8: 8, 9: 8, 10: 8, 13: 8}
		'''
		zawake_nvalue = {}
		zawakeID = csv.cards[card.card_id].zawakeID
		if zawakeID == 0:
			return zawake_nvalue
		
		# 遍历该 zawakeID 已培养的所有阶段和等级
		zawakeData = zawake._zawake.get(zawakeID, {})
		for stage, maxLevel in zawakeData.iteritems():
			for level in xrange(1, maxLevel + 1):
				cfg = zawake.LevelMap.get((zawakeID, stage, level), None)
				if not cfg:
					continue
				
				# 读取 nvalue 字典 {attrEnum: value}
				nvalueDict = getattr(cfg, 'nvalue', None)
				if not nvalueDict:
					continue
				
				# 累加 nvalue 到对应属性
				for attrEnum, value in nvalueDict.iteritems():
					if value and attrEnum > 0 and attrEnum < len(AttrDefs.attrsEnum):
						# 将枚举转换为属性名
						attr = AttrDefs.attrsEnum[attrEnum]
						if attr in NValueAttrs:
							zawake_nvalue[attr] = zawake_nvalue.get(attr, 0) + value
	...
	def calcHeldItemAttrsAddition(card):
		# CardSlim（随机塔敌人）没有 held_item 属性
		if not hasattr(card, 'held_item'):
	...
	@staticmethod
	def calcArmsAttrsAddition(card):
		role = getattr(card.game, 'role', None)
		if not role:
			card.calc.const.set('arms', zeros())
			card.calc.percent.set('arms', zeros())
			return
		const, percent = role.getArmsAttrsAdd(card)
		card.calc.const.set('arms', const)
		card.calc.percent.set('arms', percent)
	...
	def calcGemAttrsAddition(card):
		# CardSlim（随机塔敌人）没有 gems 属性
		if not hasattr(card, 'gems'):
	...
	def calcChipAttrsAddition(card):
		# CardSlim（随机塔敌人）没有 chip 属性
		if not hasattr(card, 'chip'):
	...
	@staticmethod
	def calcEmerasAttrsAddition(card):
		# CardSlim（随机塔敌人）没有 emeras 属性
		if not hasattr(card, 'emeras'):
			card.calc.const.set('emeras', zeros())
			card.calc.percent.set('emeras', zeros())
			return
		
		if card.emeras:
			const, percent = card.calEmerasAttrsAdd()
			card.calc.const.set('emeras', const)
			card.calc.percent.set('emeras', percent)
		else:
			card.calc.const.set('emeras', zeros())
			card.calc.percent.set('emeras', zeros())
    
	@staticmethod
	def calcContractAttrsAddition(card):
		"""计算契约属性加成"""
		try:
			if not hasattr(card, 'contracts') or card.contracts is None:
				card._initContractSlots()
			
			if not card.contracts:
				card.calc.const.set('contracts', zeros())
				card.calc.percent.set('contracts', zeros())
				return
			
			const, percent = card.calContractsAttrsAdd()
			card.calc.const.set('contracts', const)
			card.calc.percent.set('contracts', percent)
			...
			logger.error('Unexpected error calculating contract attributes for card %s: %s', card.id, str(e))
			card.calc.const.set('contracts', zeros())
			card.calc.percent.set('contracts', zeros())
	
	@staticmethod
	def calcTotemAttrsAddition(card, inFront=False, inBack=False):
		"""计算图腾属性加成（全局+自然属性加成+能量阶段加成）
		...
		"""
		try:
			# 检查 game 是否存在（机器人卡牌的 game 为 None）
			if not card.game or not hasattr(card.game, 'totem'):
				card.calc.const.set('totem', zeros())
				card.calc.percent.set('totem', zeros())
				return
			
			# 获取图腾属性（传入card以获取针对该卡牌自然属性的加成）
			# 返回 (const, percent) 元组
			const, percent = card.game.totem.getAttrs(card=card, inFront=inFront, inBack=inBack)
			
			card.calc.const.set('totem', const)
			card.calc.percent.set('totem', percent)
		except Exception as e:
			logger.error('Unexpected error calculating totem attributes for card %s: %s', card.id, str(e))
			card.calc.const.set('totem', zeros())
			card.calc.percent.set('totem', zeros())
	
	@staticmethod
	def calcAidAttrsAddition(card):
		"""计算助战属性加成（全局加成，类似图鉴）"""
		try:
			# 检查 game 是否存在（机器人卡牌的 game 为 None）
			if not card.game or not hasattr(card.game, 'role'):
				card.calc.const.set('aid', zeros())
				card.calc.percent.set('aid', zeros())
				return
			
			# 获取玩家激活的所有助战
			active_aid = card.game.role.active_aid or {}
			...
			const, percent = AidHelper.getGlobalAidAttrs(card.game)
			
			card.calc.const.set('aid', const)
			card.calc.percent.set('aid', percent)
			
		except Exception as e:
			logger.error('Error calculating aid attributes for card %s: %s', card.id, str(e))
			card.calc.const.set('aid', zeros())
			card.calc.percent.set('aid', zeros())
	
	@staticmethod
	def calcMeteoriteAttrsAddition(card):
		"""计算陨石属性加成（全局加成，所有陨石提供属性给所有卡牌）"""
		try:
			# 检查 game 和陨石系统是否存在（机器人卡牌的 game 为 None）
			if not card.game or not hasattr(card.game, 'meteorite') or card.game.meteorite is None:
				card.calc.const.set('meteorite', zeros())
				card.calc.percent.set('meteorite', zeros())
				return
			
			# 获取所有陨石提供的属性加成
			const, percent = card.game.meteorite.getMeteoriteAttrs()
			
			card.calc.const.set('meteorite', const)
			card.calc.percent.set('meteorite', percent)
			
		except Exception as e:
			logger.error('Error calculating meteorite attributes for card %s: %s', card.id, str(e))
			card.calc.const.set('meteorite', zeros())
			card.calc.percent.set('meteorite', zeros())

	def _initContractSlots(self):
		"""初始化契约槽位 - 不预先创建，只维护已解锁的槽位"""
		try:
			if 'contracts' not in self.db:
				self.db['contracts'] = {}
			
			for slot_id in list(self.db['contracts'].keys()):
				if 'unlock' not in self.db['contracts'][slot_id]:
					self.db['contracts'][slot_id]['unlock'] = False
				if 'contract_db_id' not in self.db['contracts'][slot_id]:
					self.db['contracts'][slot_id]['contract_db_id'] = None
		except:
			pass

	@staticmethod
	def calcFishingLevelAttrsAddition(card, game):
		if game and game.fishing and ObjectFeatureUnlockCSV.isOpen(FeatureDefs.Fishing, game):
	...
	def calcGymBadgeAttrsAddition(card, game):
		if game and game.role.badges:
			const, percent = game.badge.getBadgeAttrs()
			card.calc.const.set('badge', const)
			card.calc.percent.set('badge', percent)

	@staticmethod
	def calcMedalAttrsAddition(card):
		"""计算勋章属性加成"""
		if not card.game or not card.game.medal:
			return
		
		# 防止初始化顺序问题：勋章对象可能还没有 set()
		if not hasattr(card.game.medal, '_medal'):
			return
		
		const, percent = card.game.medal.getMedalAttrs()
		card.calc.const.set('medal', const)
		card.calc.percent.set('medal', percent)

	@staticmethod
	def calcTownRelicAttrsAddition(card):
		"""计算遗迹属性加成
		
		遗迹建筑升级后，根据 relic_upgrade_award.csv 中的 attrMap 配置，
		为所有卡牌提供固定值属性加成。
		"""
		if not card.game or not card.game.town:
			return
		
		# 防止初始化顺序问题
		if not hasattr(card.game.town, '_db') or not card.game.town._db:
			return
		
		buildings = card.game.town.buildings or {}
		if not buildings:
			return
		
		relic_key = None
		if hasattr(card.game, '_town_relic_attrs_key') and hasattr(card.game, '_town_relic_attrs_cache'):
			relic_key = card.game._town_relic_attrs_key
		const_cache = getattr(card.game, '_town_relic_attrs_cache', None)
		
		# 遗迹建筑 ID: 14=大漠遗迹, 15=山城遗迹, 16=苍雪遗迹, 17=熔岩遗迹
		relicBuildingIDs = [14, 15, 16, 17]
		relic_levels = []
		for relicID in relicBuildingIDs:
			buildData = buildings.get(relicID, None)
			level = buildData.get('level', 0) if isinstance(buildData, dict) else 0
			relic_levels.append((relicID, level))
		relic_levels = tuple(relic_levels)
		if relic_key == relic_levels and const_cache is not None:
			card.calc.const.set('town_relic', const_cache)
			return

		const = zeros()
		for relicID, level in relic_levels:
			if level <= 0:
				continue
			for cfgId in csv.town.relic_upgrade_award:
				cfg = csv.town.relic_upgrade_award[cfgId]
				if cfg.relicID == relicID and cfg.level == level:
					attrMap = cfg.attrMap or {}
					for attrType, value in attrMap.iteritems():
						const[attrType] += value
					break

		card.game._town_relic_attrs_key = relic_levels
		card.game._town_relic_attrs_cache = const
		
		card.calc.const.set('town_relic', const)

	@staticmethod
	def calcTownHomeCollectionAttrsAddition(card):
		"""计算家园收藏属性加成
		
		根据收藏等级，从 home_collection.csv 中获取属性加成，
		为所有卡牌提供固定值属性加成。
		"""
		if not card.game or not card.game.role:
			return
		
		# 获取收藏等级
		townHome = card.game.role.town_home or {}
		collectionLevel = townHome.get('collection_level', 1)
		
		if collectionLevel <= 0:
			return
		
		if (hasattr(card.game, '_town_home_collection_attrs_key') and
				card.game._town_home_collection_attrs_key == collectionLevel and
				hasattr(card.game, '_town_home_collection_attrs_cache')):
			card.calc.const.set('town_home_collection', card.game._town_home_collection_attrs_cache)
			return

		const = zeros()
		
		# 从 home_collection.csv 读取属性加成
		if collectionLevel in csv.town.home_collection:
			cfg = csv.town.home_collection[collectionLevel]
			addAttrs = cfg.addAttrs or {}
			for attrType, value in addAttrs.iteritems():
				const[attrType] += value
		
		card.game._town_home_collection_attrs_key = collectionLevel
		card.game._town_home_collection_attrs_cache = const
		card.calc.const.set('town_home_collection', const)

	@staticmethod
	def calcTownHomeFurnitureSeriesAttrsAddition(card):
		"""计算家园套装属性加成
		
		根据已收集的套装，从 home_furniture_series.csv 中获取属性加成，
		为所有卡牌提供属性加成（支持固定值和百分比）。
		"""
		if not card.game or not card.game.role:
			return
		
		# 获取已收集的套装（字典格式 {seriesID: True}）
		townHome = card.game.role.town_home or {}
		furnitureSeries = townHome.get('furniture_series', {})
		
		if not furnitureSeries:
			return
		
		series_key = None
		if isinstance(furnitureSeries, dict):
			series_key = tuple(sorted(furnitureSeries.keys()))
		else:
			try:
				series_key = tuple(sorted(furnitureSeries))
			except Exception:
				series_key = None

		if (series_key is not None and
				hasattr(card.game, '_town_home_series_attrs_key') and
				card.game._town_home_series_attrs_key == series_key and
				hasattr(card.game, '_town_home_series_attrs_cache')):
			const, percent = card.game._town_home_series_attrs_cache
			card.calc.const.set('town_home_series', const)
			card.calc.percent.set('town_home_series', percent)
			return
		...
		# 遍历所有已收集的套装
		for seriesID in furnitureSeries:
			if seriesID not in csv.town.home_furniture_series:
				continue
			
			seriesCfg = csv.town.home_furniture_series[seriesID]
			addAttrs = seriesCfg.addAttrs or {}
			
			# 使用 str2num_t 正确解析百分比和固定值
			for attrType, attrValue in addAttrs.iteritems():
				num = str2num_t(attrValue)
				const[attrType] += num[0]
				percent[attrType] += num[1]
		
		if series_key is not None:
			card.game._town_home_series_attrs_key = series_key
			card.game._town_home_series_attrs_cache = (const, percent)
		card.calc.const.set('town_home_series', const)
		card.calc.percent.set('town_home_series', percent)
	...
	def calcAttrs(card):
		...
		ObjectCard.calcExtraEffortAttrsAddition(card)
		...
		if card.game:
			...
			# 计算Z觉醒个体值等效加成并保存
			zawake_nvalue = ObjectCard.calcZawakeNvalue(card, card.game.zawake)
			if zawake_nvalue:
				card.zawake_nvalue = zawake_nvalue
			elif hasattr(card, 'db') and 'zawake_nvalue' in card.db:
				card.zawake_nvalue = {}
			ObjectCard.calcFigureAttrsAddition(card)
			ObjectCard.calcTitleAttrsAddition(card)
			ObjectCard.calcArmsAttrsAddition(card)
			...
			ObjectCard.calcEmerasAttrsAddition(card)#琉石
			ObjectCard.calcContractAttrsAddition(card)#契约
			ObjectCard.calcTotemAttrsAddition(card)#图腾
			ObjectCard.calcAidAttrsAddition(card)#助战
			ObjectCard.calcMeteoriteAttrsAddition(card)#陨石
			ObjectCard.calcFishingLevelAttrsAddition(card, card.game)
			ObjectCard.calcGymBadgeAttrsAddition(card, card.game)
			ObjectCard.calcMedalAttrsAddition(card)#勋章
			ObjectCard.calcTownRelicAttrsAddition(card)#遗迹
			ObjectCard.calcTownHomeCollectionAttrsAddition(card)#家园收藏等级
			ObjectCard.calcTownHomeFurnitureSeriesAttrsAddition(card)#家园套装
	...
	def refreshFromCalc(self):
		"""基于当前 calc 节点值刷新属性与战力（不重算全链路）"""
		attrs = self.calc.evaluation()
		self._attrs, self._attrs2 = self.splitAttrs(attrs)
		if self._display:
			self.db_attrs = self._attrs
	...
	@property
	def mem(self):
		# 防护：服务器重启时卡牌可能还没完成 init()
		if not hasattr(self, '_attrs'):
			return {
				'attrs': None,
				'attrs2': None,
				'unit_id': getattr(self, 'unitID', 0),
				'next_level_exp': 0,
				'fetters': {},
			}
		return {
	...
	def calcBattleAttrs(self, inFront, inBack, scene):
		# 调整天赋, 好感度, 特性, 饰品刻印突破, 图腾 场景加成，计算完之后恢复
		with temporary(self.calc, 'talent', 'feel_effect', 'ability', 'equip_signet_advance', 'totem'):
			...
			if self.game:
				...
				self.calcTotemAttrsAddition(self, inFront, inBack)  # 图腾战斗加成（前排/后排）
			attrs = self.calc.evaluation()
		return attrs
	...
	def calcFilterAttrs(self, disables, inFront=False, inBack=False, scene=0):
		nodes = {
			...
			'arms': [self.calc.percent, self.calc.const],
			...
			'contracts': [self.calc.percent, self.calc.const],
			'totem': [self.calc.percent, self.calc.const],
			'aid': [self.calc.percent, self.calc.const],
			'meteorite': [self.calc.percent, self.calc.const],
			'badge': [self.calc.percent, self.calc.const],
			'medal': [self.calc.percent, self.calc.const],
			'town_relic': [self.calc.const],  # 遗迹属性加成（只有固定值）
			'town_home_collection': [self.calc.const],  # 家园收藏等级属性加成（只有固定值）
			'town_home_series': [self.calc.percent, self.calc.const],  # 家园套装属性加成
		}
	...
	def battleModel(self, inFront, inBack, scene, **kwargs):
		...
		if self.game:
			...
			emeraSkills = self.game.emeras.getPassiveSkills(self.id)
			passive_skills.update(emeraSkills)
			# 契约技能
			if hasattr(self, 'contracts') and self.contracts:
				# 1. 收集单个契约的技能
				for pos, contract_info in self.contracts.items():
					contract_db_id = contract_info.get('contract_db_id')
					if contract_db_id:
						contract = self.game.contracts.getContract(contract_db_id)
						if contract and contract.exist_flag:
							contractSkills = contract.getPassiveSkills()
							passive_skills.update(contractSkills)
				
				# 2. 检查契约羁绊技能
				fetterSkills = self.getContractFetterSkills()
				passive_skills.update(fetterSkills)
	...
	@property
	def zawakeID(self):
		# 直接从 csv 获取，不依赖可能过时的 _csvCard 缓存
		# 这对于合体卡切换形态后立即访问 zawakeID 很重要
		return csv.cards[self.card_id].zawakeID
	...
	def isSameCardSeries(self, otherCard):
		'''
		检查两个卡牌是否属于同一精灵系列（允许超进化/mega形态之间交换）
		'''
		# 1. 如果 markID 相同，肯定是同一系列
		if self.markID == otherCard.markID:
			return True
		
		# 2. 检查是否通过超进化（card_merge）关联
		# 遍历 card_merge 配置，查找 markID 关联
		try:
			if hasattr(csv, 'card_merge') and csv.card_merge:
				for mergeID, mergeCfg in csv.card_merge.items():
					# 获取合并卡牌的 markID
					mergeCardID = getattr(mergeCfg, 'mergeCardID', None)
					if mergeCardID and mergeCardID in csv.cards:
						mergeMarkID = csv.cards[mergeCardID].cardMarkID
						
						# 获取原始卡牌的 markID（从 costCards 中）
						costCards = getattr(mergeCfg, 'costCards', [])
						if costCards:
							# costCards 可能是列表或单个值
							if not isinstance(costCards, (list, tuple)):
								costCards = [costCards]
							
							originMarkIDs = set()
							for costCardGroup in costCards:
								if isinstance(costCardGroup, (list, tuple)):
									# 格式：[[card_id, count], ...]
									for cardInfo in costCardGroup:
										if isinstance(cardInfo, (list, tuple)) and len(cardInfo) >= 1:
											cardID = cardInfo[0]
											if cardID in csv.cards:
												originMarkIDs.add(csv.cards[cardID].cardMarkID)
								elif isinstance(costCardGroup, (int, long)):
									# 格式：card_id
									if costCardGroup in csv.cards:
										originMarkIDs.add(csv.cards[costCardGroup].cardMarkID)
							
							# 检查两个卡牌的 markID 是否在同一个 merge 关系中
							if (self.markID in originMarkIDs and otherCard.markID == mergeMarkID) or \
							   (otherCard.markID in originMarkIDs and self.markID == mergeMarkID):
								logger.info("PropertySwap: Cards are same series via card_merge - card1=%s (markID=%s), card2=%s (markID=%s)",
								           self.card_id, self.markID, otherCard.card_id, otherCard.markID)
								return True
		except Exception as e:
			logger.warning("PropertySwap: Error checking card_merge: %s", e)
		
		# 3. 简单的 markID 相邻检查（作为兜底方案）
		# 如果两个 markID 相差在3以内，且其中一个是 mega，可能是同一系列
		if abs(self.markID - otherCard.markID) <= 3:
			if self.isMega or otherCard.isMega:
				logger.info("PropertySwap: Cards might be same series (markID diff <= 3 and one is mega) - card1=%s (markID=%s, mega=%s), card2=%s (markID=%s, mega=%s)",
				           self.card_id, self.markID, self.isMega, otherCard.card_id, otherCard.markID, otherCard.isMega)
				return True
		
		logger.warning("PropertySwap: Cards are different series - card1=%s (markID=%s), card2=%s (markID=%s)",
		             self.card_id, self.markID, otherCard.card_id, otherCard.markID)
		return False
	...
	def getPassiveSkillAttrs(card, attrs=None):
		def _new(skillID):
			level = card.skills.get(skillID, 0)
			if level <= 0:
				return None
			
			# 优先从 SkillPassiveAttrsCache 读取（公式计算方式）
			if skillID in ObjectCard.SkillPassiveAttrsCache:
				ret = {}
			...
			# 从 StarSkillAttrCache 读取（极限点技能，按等级查表）
			cacheKey = (skillID, level)
			if cacheKey in ObjectCard.StarSkillAttrCache:
				return ObjectCard.StarSkillAttrCache[cacheKey]
	...
	def developNextChange(self, newCsvCard, nextDevelop, nextCardID):
		...
		for oldSkillID, newSkillID in zip(oldSkillList, newSkillList):
				skillLevel = self.skills.get(oldSkillID, None)
				if skillLevel is not None:
						newSkills[newSkillID] = skillLevel

		for oldSkillID, newSkillID in zip(
				self.starSkillList,
				csv.card_star_skill[newCsvCard.starSkillSeqID].starSkillList
		):
				skillLevel = self.skills.get(oldSkillID, None)
				if skillLevel is not None:
						newSkills[newSkillID] = skillLevel
	...
	def branch_unlock():
			...
			def fget(self):
					return self._db.get(dbkey, False)
	...
	branch_unlock = db_property(**branch_unlock())
	...
	def aid_fighting_point():
		"""助战战力（总是动态计算，确保实时更新）"""
		dbkey = 'aid_fighting_point'
		def fget(self):
			# 总是动态计算，不使用缓存
			try:
				if self.card_id not in csv.cards:
					return 0
				cardCfg = csv.cards[self.card_id]
				if not hasattr(cardCfg, 'aidID') or not cardCfg.aidID:
					return 0
				aidID = cardCfg.aidID
				active_aid = self.game.role.active_aid or {}
				# ObjectDict.get() 可能抛 KeyError，需要先检查
				aidData = active_aid[aidID] if aidID in active_aid else None
				if not aidData:
					return 0
				from game.object.game.aid import AidHelper
				return AidHelper.getAidFightingPoint(aidID, aidData)
			except:
				return 0
		def fset(self, value):
			# 允许设置，但前端读取时会忽略
			self.db[dbkey] = value
		return locals()
	aid_fighting_point = db_property(**aid_fighting_point())
	
	def updateAidFightingPoint(self):
		"""更新助战战力到数据库"""
		try:
			# 先清除缓存
			if 'aid_fighting_point' in self.db:
				del self.db['aid_fighting_point']
			
			if self.card_id not in csv.cards:
				self.aid_fighting_point = 0
				return
			cardCfg = csv.cards[self.card_id]
			if not hasattr(cardCfg, 'aidID') or not cardCfg.aidID:
				self.aid_fighting_point = 0
			...
			if not aidData:
				...
				return
			from game.object.game.aid import AidHelper
			self.aid_fighting_point = AidHelper.getAidFightingPoint(aidID, aidData)
		except:
			self.aid_fighting_point = 0
	...
	def cardAdvance(self, count=1, autoSwitch=False):
		'''
		卡牌进阶
		@param count: 进阶次数
		@param autoSwitch: 材料不足时自动转换同级材料
		@exception:
		'''
		if count <= 0:
			return self.advance
		...
		for i in xrange(count):
			...
			if autoSwitch:
				self._autoSwitchAdvanceItems(costD)
	...
	def _autoSwitchAdvanceItems(self, costD):
		'''
		自动转换进阶材料：同级材料1:1互换
		'''
		for itemID, needCount in costD.iteritems():
			if itemID == 'gold':
				continue
			haveCount = self.game.items.getItemCount(itemID)
			if haveCount >= needCount:
				continue
			lackCount = needCount - haveCount
			# 查找该物品所属的转换组
			switchGroup = None
			for cfgID in csv.item_switch:
				cfg = csv.item_switch[cfgID]
				if itemID in cfg.items:
					switchGroup = cfg.items
					break
			if not switchGroup:
				continue
			# 从同组其他物品转换
			for srcItemID in switchGroup:
				if srcItemID == itemID:
					continue
				srcCount = self.game.items.getItemCount(srcItemID)
				if srcCount <= 0:
					continue
				switchCount = min(srcCount, lackCount)
				# 消耗源物品
				costAux = ObjectCostAux(self.game, {srcItemID: switchCount})
				costAux.cost(src='advance_auto_switch')
				# 获得目标物品
				gainAux = ObjectGainAux(self.game, {itemID: switchCount})
				gainAux.gain(src='advance_auto_switch')
				lackCount -= switchCount
				if lackCount <= 0:
					break
	...
	# 额外努力值等级（极限突破）
	extra_effort_level = db_property('extra_effort_level')

	# 额外努力值属性
	extra_effort_values = db_property('extra_effort_values')
	...
	def saveEffortValue(self, effortIndexs):
		...
		# 判断是保存到普通努力值还是额外努力值
		extra_effort_level = self.db.get('extra_effort_level', 0)
		is_extra_effort = (self.effort_advance == self.advanceLimit and extra_effort_level > 0)
		
		# 修改前先获取引用
		if is_extra_effort:
			extra_values = self.extra_effort_values or {}
		
		for attr, val in grows.iteritems():
			if is_extra_effort:
				# 保存到额外努力值（extra_effort_values）
				# 额外努力值没有上限限制（或者有不同的上限）
				extra_values[attr] = extra_values.get(attr, 0) + val
				if attr == AttrDefs.damage:
					extra_values[AttrDefs.specialDamage] = extra_values[attr]
			else:
				# 保存到普通努力值（effort_values）
				curLimitArrt = self.calEffortArrtUpper(attr, self.effort_advance)
				lastLimitArrt = self.calEffortArrtUpper(attr, self.effort_advance-1)
				# 只处理当前阶段的
		...
		# 如果保存的是额外努力值，需要触发 db_property 保存
		if is_extra_effort:
			self.extra_effort_values = extra_values
			self.calcExtraEffortAttrsAddition(self)
	...
	def extraEffortAdvance(self):
		'''
		额外努力值突破（极限突破）
		'''
		from framework.csv import csv
		
		# 初始化 extra_effort_level
		if 'extra_effort_level' not in self.db or not self.extra_effort_level:
			self.extra_effort_level = 0
		
		# 初始化 extra_effort_values
		if 'extra_effort_values' not in self.db or not self.extra_effort_values:
			self.extra_effort_values = {}
		
		currentLevel = self.extra_effort_level
		nextLevel = currentLevel + 1
		
		# 检查配置是否存在
		if nextLevel not in csv.card_effort_extra:
			raise ClientError('extra effort level is max')
		
		cfg = csv.card_effort_extra[nextLevel]
		
		# 验证等级要求
		if self.level < cfg.needLevel:
			raise ClientError('level is not enough for extra effort advance')
		
		# 消耗道具和金币
		cost = ObjectCostAux(self.game, cfg.cost)
		if not cost.isEnough():
			raise ClientError('extra effort advance cost not enough')
		cost.cost(src='extra_effort_advance')
		
		# 升级
		self.extra_effort_level = nextLevel
		
		# 重新计算额外努力值属性加成，然后更新属性和战力
		self.calcExtraEffortAttrsAddition(self)
		self.onUpdateAttrs()
	...
	# Z觉醒个体值额外加成
	zawake_nvalue = db_property('zawake_nvalue')
	...
	def propertySwap(self, targetCard, swapType):
		...
		if swapType == PropertySwapDefs.EffortSwap:
			...
			# 极限努力值交换（extra_effort_level 和 extra_effort_values）
			tempExtraLevel = self.db.get('extra_effort_level', 0)
			tempExtraValues = self.db.get('extra_effort_values', {}).copy()
			
			self.extra_effort_level = targetCard.db.get('extra_effort_level', 0)
			targetCard.extra_effort_level = tempExtraLevel
			
			self.extra_effort_values = targetCard.db.get('extra_effort_values', {}).copy()
			targetCard.extra_effort_values = tempExtraValues
		else:
			# ✅ 检查是否为同一精灵系列（允许超进化/mega形态互相交换）
			if not self.isSameCardSeries(targetCard):
	...
	# 潜能 {level: int, branch: int}
	abilities_extra = db_property('abilities_extra')

	def abilityExtraStrength(self, branch, upLevel):
		"""潜能强化
		Args:
			branch: 分支 (1或2)
			upLevel: 提升等级数
		...
		# 获取配置
		if self.abilitySeqID not in self.CardAbilityExtraMap:
			raise ClientError('no ability extra config for abilitySeqID: %d' % self.abilitySeqID)

		cfg = self.CardAbilityExtraMap[self.abilitySeqID]

		# 初始化数据（如果不存在）
		if 'abilities_extra' not in self.db or not self.abilities_extra:
			self.abilities_extra = {'level': 0, 'branch': 0}

		currentLevel = self.abilities_extra.get('level', 0)
		currentBranch = self.abilities_extra.get('branch', 0)

		# 验证分支
		if branch not in [1, 2]:
			raise ClientError('invalid branch')

		# 如果是首次激活，设置分支
		if currentLevel == 0:
			self.abilities_extra['branch'] = branch
			currentBranch = branch
		else:
			# 已激活，验证分支是否匹配
			if currentBranch != branch:
				raise ClientError('branch not match, use switch to change branch')

		# 验证等级上限
		if currentLevel + upLevel > cfg.strengthMax:
			raise ClientError('Ability Extra is MaxLevel')

		# 计算消耗
		strengthSeqID = cfg.strengthSeqID
		cost = ObjectCostAux(self.game, {})
		for i in xrange(currentLevel + 1, currentLevel + upLevel + 1):
			costCfg = csv.card_ability_extra_cost[i]
			costItems = getattr(costCfg, 'costItemMap%d' % strengthSeqID)
			cost += ObjectCostAux(self.game, costItems)

		# 验证并消耗
		if not cost.isEnough():
			raise ClientError('ability extra strength cost not enough')
		cost.cost(src='ability_extra_strength')

		# 升级（直接修改字典，db_property 会自动标记为脏数据）
		self.abilities_extra['level'] = currentLevel + upLevel
		
		# 更新属性和战力
		self.onUpdateAbility()
		
		# 触发活动任务（如果需要）
		# ObjectYYHuoDongFactory.onGeneralTask(self.game, TargetDefs.CardAbilityExtraStrength, upLevel)

	def abilityExtraSwitch(self, branch):
		"""潜能分支切换（重置潜能）
		Args:
			branch: 新分支 (1或2)
		"""
		...
		# 未激活时不能切换
		if currentLevel == 0:
			raise ClientError('ability extra not activated')
		...
		cost = ObjectCostAux(self.game, {'rmb': ConstDefs.abilityExtraSwitchCost})
		if not cost.isEnough():
			raise ClientError('rmb not enough')
		cost.cost(src='ability_extra_switch')
	...
	def getAbilityAttrsAdd(self, scene):
		...
		# 潜能属性加成
		if self.abilitySeqID in self.CardAbilityExtraMap:
			abilities_extra = self.db.get('abilities_extra', None)
			if abilities_extra and abilities_extra.get('level', 0) > 0:
				extraCfg = self.CardAbilityExtraMap[self.abilitySeqID]
				extraLevel = abilities_extra['level']
				
				# 计算 5 个属性类型的加成
				for i in xrange(1, 6):  # attrType1-5, attrNum1-5
					attrType = getattr(extraCfg, 'attrType%d' % i, 0)
					if attrType > 0:
						attrNumList = getattr(extraCfg, 'attrNum%d' % i, ())
						if extraLevel <= len(attrNumList):
							attrNum = attrNumList[extraLevel - 1]
							num = str2num_t(attrNum)
							const[attrType] += num[0]
							percent[attrType] += num[1]
	...
	def calEmerasAttrsAdd(self):
		...
		for pos, emeraID in emeras.iteritems():
			...
			if emera is None:
				# 琉石不存在（可能是数据不一致）
				continue
			const1, percent1 = emera.getAttrs()
			const += const1
			percent += percent1
		
		# 印记套装属性
		const2, percent2 = self.game.emeras.getEmeraLinkageSuitAttrs(self)
		const += const2
		percent += percent2
		
		# 额外属性（品质、等级条件）
		const3, percent3 = self.game.emeras.getEmeraExtraAttrs(self)
		const += const3
		percent += percent3
    ...
    # 契约装备 {位置: {contract_db_id: contractDbID, unlock: bool}}
	contracts = db_property('contracts')
	
	# 陨石索引（反向引用：卡牌装备在哪个陨石上，0表示未装备）
	meteorite_index = db_property('meteorite_index')

	def getContractFetterSkills(self):
		"""获取契约羁绊技能
		
		逻辑：
		1. 获取当前卡牌装备的所有契约ID列表
		2. 遍历 csv.contract.group，检查每个羁绊配置的 items
		3. 如果当前装备的契约ID包含了 items 中的所有契约，触发羁绊
		4. 返回羁绊技能字典
		"""
		from framework.csv import csv
		...
		# 检查契约配置
		if not hasattr(self, 'contracts') or not self.contracts:
			return skills
		
		if not hasattr(csv.contract, 'group') or not csv.contract.group:
			return skills
		
		try:
			# 收集当前装备的所有契约ID
			equipped_contract_ids = set()
			for pos, contract_info in self.contracts.items():
				contract_db_id = contract_info.get('contract_db_id')
				if contract_db_id:
					contract = self.game.contracts.getContract(contract_db_id)
					if contract and contract.exist_flag:
						equipped_contract_ids.add(contract.contract_id)
			
			# 如果没有装备契约，直接返回
			if not equipped_contract_ids:
				return skills
			
			# 遍历所有羁绊配置
			for group_id in csv.contract.group:
				try:
					group_cfg = csv.contract.group[group_id]
					
					# 获取羁绊需要的契约列表
					items = None
					if hasattr(group_cfg, 'items'):
						items = group_cfg.items
					elif isinstance(group_cfg, dict):
						items = group_cfg.get('items', None)
					
					if not items:
						continue
					
					# 如果 items 是 callable，调用它
					if callable(items):
						try:
							items = items()
						except Exception:
					...
					# 检查是否满足羁绊条件（装备的契约包含所有 items）
					required_contracts = set(items) if isinstance(items, (list, tuple)) else set([items])
					
					if required_contracts.issubset(equipped_contract_ids):
						# 满足羁绊条件，获取羁绊技能
						skillID = getattr(group_cfg, 'skillID', 0) if hasattr(group_cfg, 'skillID') else 0
						
						if skillID and skillID > 0:
							# 羁绊技能等级固定为 1
							skills[skillID] = 1
							logger.info('Card %s activates fetter group %s (items=%s), provides skill %d', 
							           self.id, group_id, items, skillID)
				
				except (KeyError, IndexError, TypeError, AttributeError) as e:
					continue
		
		except Exception as e:
			logger.error('Error getting contract fetter skills for card %s: %s', self.id, str(e))
		
		return skills

	def calContractsAttrsAdd(self):
		'''
		计算契约的属性加成
		'''
		const = zeros()
		...
		# 确保contracts字段正确初始化
		if not hasattr(self, '_contracts_checked'):
			self._initContractSlots()
			self._contracts_checked = True
		
		logger.debug('=== CALCULATING CONTRACT ATTRIBUTES FOR CARD %s ===', self.id)
		logger.debug('Card %s contracts: %s', self.id, dict(self.contracts))
		
		# 镶嵌属性
		for pos, contract_info in self.contracts.iteritems():
			contract_db_id = contract_info.get('contract_db_id')
			if contract_db_id:
				logger.debug('Processing contract at position %d: %s', pos, contract_db_id)
				contract = self.game.contracts.getContract(contract_db_id)
				if contract:
					try:
						const1, percent1 = contract.getAttrs()
						const += const1
						percent += percent1
						import numpy as np
						logger.debug('Added contract %s attributes: const_sum=%.2f, percent_sum=%.2f', 
						           contract.contract_id, float(np.sum(const1)), float(np.sum(percent1)))
					except Exception as e:
						logger.error('Error getting attributes from contract %s: %s', contract.contract_id, str(e))
				else:
					logger.warning('Contract not found: %s', contract_db_id)
		
		# 羁绊属性
		if hasattr(self.game, "contracts"):
			try:
				const2, percent2 = self.game.contracts.getContractFetterAttrs(self)
				const += const2
				percent += percent2
				import numpy as np
				logger.debug('Added fetter attributes: const_sum=%.2f, percent_sum=%.2f', float(np.sum(const2)), float(np.sum(percent2)))
			except Exception as e:
				logger.error('Error calculating fetter attributes: %s', str(e))
		
		# 输出最终结果 - 修复NumPy数组错误
		try:
			import numpy as np
			total_const = float(np.sum(const)) if const is not None and len(const) > 0 else 0.0
			total_percent = float(np.sum(percent)) if percent is not None and len(percent) > 0 else 0.0
			logger.debug('Total contract attributes: const_total=%.2f, percent_total=%.2f', total_const, total_percent)
		except Exception as sum_e:
			logger.debug('Error calculating totals: %s', str(sum_e))
		
		logger.debug('=== CONTRACT ATTRIBUTES CALCULATION COMPLETE ===')
...
class ObjectCardsMap(ObjectDBaseMap):
	...
	def _logCardInUse(self, cardID, scene):
		logger.info(
			u'[卡牌占用检测] 角色%s的卡牌%s正在被%s占用',
			objectid2string(self.game.role.id),
			objectid2string(cardID),
			scene
		)
	...
	def init(self):
		...
		# 修复：同步 role.cards 和 _objs，清理因服务器重启导致的脏数据
		valid_card_ids = [o.id for o in self._objs.itervalues() if o.exist_flag]
		role_cards_set = set(self.game.role.cards)
		valid_cards_set = set(valid_card_ids)
		if role_cards_set != valid_cards_set:
			dirty_ids = role_cards_set - valid_cards_set
			if dirty_ids:
				logger.warning('role %s cards mismatch! role.cards has %d dirty ids: %s, fixing...',
					objectid2string(self.game.role.id), len(dirty_ids),
					[objectid2string(x) for x in list(dirty_ids)[:5]])
				self.game.role.cards = valid_card_ids
		
		# 修复：清理卡牌引用的不存在的芯片，同时修复 card_db_id 不一致
		for card in self._objs.itervalues():
			if card.chip:
				...
				for pos, chipID in card.chip.iteritems():
					chip = self.game.chips.getChip(chipID)
					if not chip:
						dirty_chip_pos.append(pos)
					elif chip.card_db_id != card.id:
						# 修复芯片的 card_db_id 不一致
						logger.warning('role %s card %s chip[%s] card_db_id mismatch (chip.card_db_id=%s), fixed',
							objectid2string(self.game.role.id), objectid2string(card.id), pos, chip.card_db_id)
						chip.card_db_id = card.id
				if dirty_chip_pos:
					for pos in dirty_chip_pos:
						card.chip.pop(pos, None)
					logger.warning('role %s card %s has dirty chip refs, cleaned %d',
						objectid2string(self.game.role.id), objectid2string(card.id), len(dirty_chip_pos))
	...
	def getCardsByZawakeID(self, zawakeID):
		cards = []
		# 直接遍历查找，不依赖 _zawakeS 缓存
		# 因为超进化等操作可能导致 _zawakeS 没有及时更新
		for _, o in self._objs.iteritems():
			if o.zawakeID == zawakeID:
				cards.append(o)
		# 如果找到了卡牌但 _zawakeS 中没有，更新缓存
		if cards and zawakeID not in self._zawakeS:
			self._zawakeS.add(zawakeID)
		return cards
	...
	def getCostCards(self, cardIDs, user=None):
		...
		for card in cards:
			...
			if card.locked:
				raise ClientError('卡牌被加锁，请先解锁后再操作')
			in_using, scene = self.inusing(card.id, return_scene=True)
			if in_using:
				if scene:
					scene_desc = scene.encode('utf8') if isinstance(scene, unicode) else scene
					raise ClientError('精灵正在[%s]中，请先下阵后再操作' % scene_desc)
				raise ClientError('精灵正在使用中，请先下阵后再操作')
		return cards
	...
	def makeBattleCardHalo(self, cardIDs, scene):
		...
		for i, cardID in enumerate(cardIDs):
			logger.info("!!!bug cardID:%s", cardID)
	...
	def makeBattleCardModel(self, cardIDs, scene=SceneDefs.City, dirty=None, is_aid=False, aidCardIDs=None):
		...
		for i, cardID in enumerate(cardIDs):
			...
			if not card:
				continue
			# 防护：服务器重启时卡牌可能还没完成 init()
			if not hasattr(card, 'calc'):
				logger.warning('card %s not initialized (no calc), skipping in makeBattleCardModel', objectid2string(cardID))
				continue
			inFront = i < 3
			inBack = i >= 3
			...
			# 如果是助战卡牌，添加助战战斗力和被动技能
			if is_aid:
				from game.object.game.aid import AidHelper
				# 从 CSV 配置获取 aidID（不是 card_id）
				if card.card_id not in csv.cards:
					model['aid_fighting_point'] = 0
					cardsD[cardID] = model
					continue
				...
				if aidData:
					# 计算助战战斗力
					aid_fp = AidHelper.getAidFightingPoint(aidID, aidData)
					model['aid_fighting_point'] = aid_fp
					
					# 添加助战被动技能（战斗需要）
					# aid_skills 是字典 {技能ID: 技能等级}
					aid_skills = AidHelper.getAidPassiveSkills(aidID, aidData)
					if aid_skills:
						if 'passive_skills' not in model:
							model['passive_skills'] = {}
						for skillID, skillLevel in aid_skills.iteritems():
							model['passive_skills'][skillID] = skillLevel
				else:
					model['aid_fighting_point'] = 0
			...
			if card.twinFlag:
				...
				model = appendCardAttrsAddition(model, const, percent)
				if is_aid and 'aid_fighting_point' in cardsD[cardID]:
					model['aid_fighting_point'] = cardsD[cardID]['aid_fighting_point']
	...
	def markBattlePassiveSkills(self, cardIDs, scene=SceneDefs.City):
		...
		for i, cardID in enumerate(cardIDs):
			...
			emeraSkills = self.game.emeras.getPassiveSkills(cardID)
			passive_skills.update(emeraSkills)
		...
		if scene == SceneDefs.Gym:
			...
			passive_skills.update(gymTalentSkills)
		if self.game and self.game.role:
			armSkills = self.game.role.getArmsPassiveSkills()
			passive_skills.update(armSkills)
	...
	def inusing(self, cardID, return_scene=False):
		def ret(res, scene=None):
			if return_scene:
				return res, scene
			return res
		# 1.battle_card
		if cardID in self.game.role.battle_cards:
				scene = u'主阵容'
				self._logCardInUse(cardID, scene)
				return ret(True, scene)
...
class ObjectCardRebirthBase(ObjectBase):
	...
	def rebirthCost(cls, returnItems, type):
		...
		elif type == CardResetDefs.emeraCostType:
			rmb = min(rmb, ConstDefs.emeraRebirthRMBCostLimit)
			rmb = max(rmb, ConstDefs.emeraRebirthRMBCostMin)
		else:
