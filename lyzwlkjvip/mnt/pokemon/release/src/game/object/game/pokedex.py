...
class ObjectPokedex(ObjectBase):
	...
	def addPokedex(self, cardIDs, refresh=True):
		...
		if flag:
			...
			if refresh:
				self.calcAttrs()
				self.updateAllCardsAttrs(update_pokedex=True, update_develop=True)
	...
	def advance(self, csvID):
		...
		cfg = csv.pokedex_advance[csvID]
		cards = self.game.cards.getCardsByNature(cfg.attrNatureType)
		for card in cards:
			if getattr(card, '_attrs', None) is None or getattr(card, '_attrs2', None) is None:
				card.onUpdateAttrs()
				continue
			try:
				card.calcPokedexAdvanceAttrsAddition(card, self)
	...
	def updateDevelopRelatedCards(self):
		self.calcDevelopAttrs()
		self.updateAllCardsAttrs(update_pokedex=False, update_develop=True)

	def updateAllCardsAttrs(self, update_pokedex=True, update_develop=True):
