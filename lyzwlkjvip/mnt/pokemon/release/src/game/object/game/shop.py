	...
	def getRandomShop(cls, game, vip, level):
		...
		for p, lst in position.iteritems():
			...
			if lst[0]._cfg.vipWeight == -1:
				...
				# 过滤掉权重为0的物品
				validLst = [o for o in lst if o._cfg.vipWeight > 0]
				if not validLst:
					obj = rndobj.choice(lst)  # 兜底：随机选一个
				else:
					obj = WeightRandomObject.onceRandom(validLst, lambda o: o._cfg.vipWeight)
...
#
# ObjectTotemShopRandom
#
class ObjectTotemShopRandom(ObjectShopRandom):
	ShopMap = {}
	VIPFixShop = []
	VIPRandomShop = []
	RefreshDate = 20140101
...
# ObjectCrossSupremacyShop
#
class ObjectCrossSupremacyShop(ObjectPVPShop):
	Key = 'cross_supremacy_shop'

	def __init__(self, game):
		ObjectBase.__init__(self, game)
		if game.role.cross_supremacy_shop is None:
			game.role.cross_supremacy_shop = {}
		self._shopDB = game.role.cross_supremacy_shop
		self.level = game.role.level
	...
	def csv(cls, csvID=None):
		...
		return csv.cross.supremacy.shop[csvID]
		...
		def afterGain():
			...
			if cfg.exchangeLimit != -1:
				if csvID in self._shopDB:
					...
					self._shopDB[csvID] = (count, nowtime_t())
			self.game.role.cross_supremacy_shop = self._shopDB
...
#
# ObjectAutoChessPVPShop - 卡牌冒险商店（存储在 role 中）
#
class ObjectAutoChessPVPShop(ObjectPVPShop):
	Key = 'auto_chess_shop'

	def __init__(self, game):
		ObjectBase.__init__(self, game)
		if game.role.auto_chess_shop is None:
			game.role.auto_chess_shop = {}
		self._shopDB = game.role.auto_chess_shop
		self.level = game.role.level
	...
	def csv(cls, csvID=None):
		...
		return csv.auto_chess.shop[csvID]
		...
		def afterGain():
			...
			if cfg.exchangeLimit != -1:
				if csvID in self._shopDB:
					...
					self._shopDB[csvID] = (count, nowtime_t())
			self.game.role.auto_chess_shop = self._shopDB
...
# ObjectTotemShop
#
class ObjectTotemShop(ObjectUnionShop):
	Key = 'totem_shop'
	DBModel = 'TotemShop'
	FreeList = deque()
	...
	def csv(cls, csvID=None):
		...
		return csv.totem.shop[csvID]

	@classmethod
	def makeShopItems(cls, game):
		return ObjectTotemShopRandom.getRandomShop(game, game.role.vip_level, game.role.level)

	def isPast(self):
		'''
		判断商店是否过期（按全局刷新时间，参考 ObjectEquipShop）
		'''
		# 如果对象还没有初始化，认为未过期
		if self._db is None:
			return False
		from game.object.game.servrecord import ObjectServerGlobalRecord
		return ObjectServerGlobalRecord.isTotemShopPass(self.last_time)
