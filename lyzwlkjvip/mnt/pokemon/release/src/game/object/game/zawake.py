...
class ObjectZawake(ObjectBase):
    ...
    def reset(self, zawakeID, auto=False):
        ...
        # 清除 _maxAttrMap 缓存，让后续的 reqCheck 重新计算
        # 这对于超进化后立即升级新 zawakeID 的情况很重要
        self._maxAttrMap = {}

        for card in self.game.cards.getCardsByZawakeID(zawakeID):
            card.zawake_skills = []
            # 清除 z觉醒个体值加成
            if 'zawake_nvalue' in card.db:
                card.zawake_nvalue = {}
    ...
    def onAttrChange(self, battleOnly=True):
        ...
        for card in affectedCards:
            ...
            zawake_nvalue = ObjectCard.calcZawakeNvalue(card, self)
            if zawake_nvalue:
                card.zawake_nvalue = zawake_nvalue
            elif 'zawake_nvalue' in card.db:
                card.zawake_nvalue = {}
