...
class AttrDefs(object):
	...
	#94 damageHit 伤害命中
	#95 damageDodge 伤害闪避
	#96 finalDamageAdd 最终伤害加成
	#97 finalDamageSub 最终伤害减免
	#98 finalDamageDeepen 最终伤害加深
	#99 finalDamageReduce 最终伤害降低
	
	# 琉石系统属性
	#100 shieldPower 护盾强度
	#101 mpBeAttackRecover 被攻击回能
	#102 mpAttackRecover 攻击回能
	#103 ignoreDamageSub 无视伤害减免
	#104 elementalDamageAdd 元素伤害加成
	#105 elementalDamageSub 元素伤害减免
	#106 elementalDamageDeepen 元素伤害加深
	#107 elementalDamageReduce 元素伤害降低
	...
	finalDamageAdd = 'finalDamageAdd'
	finalDamageSub = 'finalDamageSub'    
	finalDamageDeepen = 'finalDamageDeepen'  
	finalDamageReduce = 'finalDamageReduce'
	
	# 琉石系统属性 (100-107)
	shieldPower = 'shieldPower'                    # 100 护盾强度
	mpBeAttackRecover = 'mpBeAttackRecover'        # 101 被攻击回能
	mpAttackRecover = 'mpAttackRecover'            # 102 攻击回能
	ignoreDamageSub = 'ignoreDamageSub'            # 103 无视伤害减免
	elementalDamageAdd = 'elementalDamageAdd'      # 104 元素伤害加成
	elementalDamageSub = 'elementalDamageSub'      # 105 元素伤害减免
	elementalDamageDeepen = 'elementalDamageDeepen'  # 106 元素伤害加深
	elementalDamageReduce = 'elementalDamageReduce'  # 107 元素伤害降低
...
class ItemDefs(object):
	...
	# 18=性格道具
	# 19-限时皮肤道具
	# 22-自走棋专用：直接打开的抽卡宝箱（通过specialID触发抽卡）
	normalType = 0
	expType = 1
	staminaType = 2
	giftType = 3
	...
	roleDisplayType = 17
	characterType = 18
	autoChessDrawBoxType = 22  # 自走棋抽卡宝箱
...
class ContractDefs(object):
	...
	@classmethod
	def isContractID(cls, contractID):
		return cls.startID <= contractID and contractID <= cls.maxID

	attrsEnum = AttrDefs.attrsEnum

class TotemDefs(object):
	# 图腾符号起始id (items.csv中的道具ID)
	startID = 8301
	# 图腾符号ID最大值
	maxID = 8800

	@classmethod
	def isTotemSymbolID(cls, itemID):
		"""判断是否是图腾符号道具"""
		return cls.startID <= itemID <= cls.maxID

	attrsEnum = AttrDefs.attrsEnum

class FurnitureDefs(object):
	# 家具起始id
	startID = 70000
	# 家具ID最大值
	maxID = 80000
	...
	@classmethod
	def isFurnitureID(cls, furnitureID):
		return cls.startID < furnitureID <= cls.maxID


class AidMaterialDefs(object):
	# 助战材料起始id
	startID = 90000
	# 助战材料ID最大值
	maxID = 100000
	...
	@classmethod
	def isAidMaterialID(cls, materialID):
		return cls.startID < materialID <= cls.maxID

class EmeraDefs(object):
	# 琉石起始id
	startID = 100000
	# 琉石ID最大值
	maxID = 110000
	...
	# 镶嵌槽位
	MainSlot = 1       # 核心槽位（只能镶嵌核心琉石）
	MaxSlots = 8       # 最大槽位数量

	@classmethod
	def isEmeraID(cls, emeraID):
		return cls.startID < emeraID <= cls.maxID

	@classmethod
	def isMainSlot(cls, slot):
		return slot == cls.MainSlot

	attrsEnum = AttrDefs.attrsEnum

class MeteoriteDefs(object):
	# 陨石系统定义
	...
	# 槽位有效性检查
	@classmethod
	def isValidHeldItemPos(cls, pos):
		return pos in [cls.HeldItemPos1, cls.HeldItemPos2]
	
	# 陨石索引有效性检查
	@classmethod
	def isValidIndex(cls, index):
		return cls.MinIndex <= index <= cls.MaxIndex
...
class TargetDefs(object):
	...
	# 无尽随机塔任务类型
	HellRandomTowerTimes = 160 # 无尽随机塔通过房间数量
	HellRandomTowerPoint = 161 # 无尽随机塔积分
	HellRandomTowerFloorTimes = 162 # 无尽随机塔累计通过层数
	HellRandomTowerFloorSum = 163 # 无尽随机塔累计通过层数总和
	...
	# 琉石抽取任务类型
	DrawEmeraRMB = 119 # 钻石抽琉石次数
	DrawEmera = 120 # 抽琉石次数
	EmeraStrength = 121 # 琉石强化次数
	EmeraRecast = 122 # 琉石洗练次数
...
class AchievementDefs(object):
	...
	ContractHuodongPassCount = 102  # 契约副本累计通关次数
	ContractHuodongPassType = 103  # 契约副本累计通关难度类型数
	AbyssEndlessTowerPass = 106  # 无尽随机塔历史最高关卡
...
class CostDefs(object):
	...
	# 玩法通行证等级购买次数消耗 PlayPassportDefs.Stamina
	PlayPassportBuyCost5 = "play_passport_buy_cost5"
	...
	# 启示之塔抽奖消耗
	RoundDrawCost = "round_draw_cost"

	# 自走棋扫荡消耗
	AutoChessSaodangCost = "autochess_saodang_cost"
...
class FeatureDefs(object):
	...
	# 星转（永久兑换）
	CardStarSwap = 'cardStarSwap'

	# 星转（临时援助）
	CardStarTemporarySwap = 'cardStarTemporarySwap'
...
class DrawCardDefs(object):
	...
	# 自选抽卡钻石单抽
	GroupUpCustomRMB1 = 'group_up_custom_rmb1'
	# 自选抽卡钻石10连抽
	GroupUpCustomRMB10 = 'group_up_custom_rmb10'
...
class HuoDongDefs(object):
	...
	#活动关卡类型
	# 1=金币 2=经验 3=礼物 4=碎片 5=51活动本 7=契约
	TypeGold = 1
	TypeExp = 2
	TypeGift = 3
	TypeFrag = 4
...
class MessageDefs(object):
	...
	OfficialType = 20  # 官方消息 (和前端 official=20 对应)
	CrossChatType = 21  # 跨服聊天
	CrossCardShareType = 22  # 跨服精灵分享
	CrossBattleShareType = 23  # 跨服战报分享
	CrossOfficialType = 24  # 跨服官方消息
	YYHuoDongCrossRedPacketType = 25  # 运营活动跨服红包（预留）
...
class TitleDefs(object):
	...
	ResetTime = {
		...
		'crossSupremacy': datetime.time(hour=21, minute=59, second=58),
	...
	if framework.__language__ == "en":
		...
		ResetTime["crossSupremacy"] = datetime.time(hour=22, minute=59, second=58)
