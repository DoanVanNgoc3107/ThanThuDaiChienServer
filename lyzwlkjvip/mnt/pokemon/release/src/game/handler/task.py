...
class RequestHandlerTask(object):
	...
	def finish(self):
		...
		# 走马灯消息和聊天消息
		if self.game and getattr(self.game, 'role', None) and getattr(self.game.role, '_db', None):
	...
	def runInServer(self):
		...
		try:
			...
			cost = self.endRunTime - self.beginRunTime
			if cost >= self.SLOW_REQ_THRESHOLD:
				role_id = None
				try:
					role_id = self.game.role.id
				except:
					pass
				self.logger.warning('[SlowReq] %s url=%s cost=%.3fs role=%s',
									self.__class__.__name__, self.ntask.url, cost, role_id)
