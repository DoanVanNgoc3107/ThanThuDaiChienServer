...
import copy
# 卡牌解锁分支
class CardUnlockBranch(RequestHandlerTask):
    url = r'/game/card/unlock/branch'
    ...
    def run(self):
        ...
        if card.branch_unlock:
            raise ClientError('branch already unlock')

        branchCfg = None
        cardIdx = None

        # 仅用于找到与当前 card 相关的 branchCfg（不做任何条件限制）
        for cfgID in csv.card_mega:
            logger.info(cfgID)
            cfg = csv.card_mega[cfgID]

            # 过滤掉 type=0
            cfgType = getattr(cfg, 'type', 0)
            try:
                cfgType = int(cfgType)
            except Exception:
                cfgType = 0
            ...
            # 标准化 card 字段为二维列表 [[baseCardID, ...], ...]
            cardsCfg = getattr(cfg, 'card', None)
            if not cardsCfg:
                continue
            if isinstance(cardsCfg, (list, tuple)):
                ...
                cardInfos = [cardsCfg]
            if cardInfos and not isinstance(cardInfos[0], (list, tuple)):
                cardInfos = [cardInfos]

            # 只匹配 baseCardID == 当前卡 id，不再校验星级等条件
            matched = False
            for idx, info in enumerate(cardInfos):
                if not isinstance(info, (list, tuple)) or not info:
                    continue
                try:
                    baseCardID = int(info[0])
                except Exception:
                    continue
                if baseCardID != card.markID:
                    continue
        ...
        if branchCfg is None:
            raise ClientError('card unlock branch cfg error')

        # 不做 condition / req / mergeReq 的任何限制检查，直接消耗材料并解锁
        routeCost = getattr(branchCfg, 'routeCost', None) or {}
        costAux = ObjectCostAux(self.game, routeCost)
        if not costAux.isEnough():
            raise ClientError(ErrDefs.costNotEnough)
        costAux.cost(src='card_unlock_branch')

        card.branch_unlock = True
        self.write({"view": {"branch_unlock": True}})
...
class CardAdvance(RequestHandlerTask):
	...
	def run(self):
		...
		autoSwitch = self.input.get('autoSwitch', False)
		card = self.game.cards.getCard(cardID)
		if card is None:
			raise ClientError('cardID error')

		self.game.badge.resetBadgeCache(card)
		card.cardAdvance(count, autoSwitch)
...
class CardStar(RequestHandlerTask):
	...
	def run(self):
		...
		for costCard in costCards:
			# 使用 isSameCardSeries 判断是否同一精灵系列（支持 Mega 进化前后互通）
			if not card.isSameCardSeries(costCard) and costCard.card_id not in csvStar.universalCards:
	...
	def run(self):
		...
		if zawakeIDOld and zawakeIDOld != zawakeIDNew:
				...
				if eff:
						...
						if getattr(eff, 'result', None):
								view.update(eff.result)
...
class CardEffortTrain(RequestHandlerTask):
	...
	def run(self):
		...
		elif card.effort_advance == card.advanceLimit:
			# 已达到最大普通突破：检查额外努力值是否已满
			# 如果有额外努力值等级，可以继续培养额外努力值
			extra_effort_level = card.db.get('extra_effort_level', 0)
			if extra_effort_level > 0:
				# 有额外努力值等级，可以继续培养额外努力值
				pass
			else:
				# 没有额外努力值等级，无法继续培养
...
class CardEffortAdvance(RequestHandlerTask):
	...
	def run(self):
		...
		if card.effort_advance < card.advanceLimit:
			...
			# 额外努力值突破（极限突破）
			# 执行额外突破（ObjectFeatureUnlockCSV.isOpen 内部会检查解锁）
			card.extraEffortAdvance()
		else:
			raise ClientError('effort advance error')
...
class CardPropertySwap(RequestHandlerTask):
	...
	def run(self):
		...
		if swapType == PropertySwapDefs.EffortSwap:  # 努力值
			...
			# 极限努力值属性加成
			card.calcExtraEffortAttrsAddition(card)
			targetCard.calcExtraEffortAttrsAddition(targetCard)
...
class CardShare(RequestHandlerTask):
	...
	def run(self):
		...
		if card is None:
			raise ClientError('cardID error')
		if shareType not in ['world', 'union', 'cross']:
		...
		if shareType == 'world':
			...
			card.display()
			msg = L10nDefs.NewsCardShare.format(name=self.game.role.name, card=card.name)
			args = {'card1': objectid2string(card.id)}
			resp = yield ObjectServerGlobalRecord.sendCrossChatMsg(self.game, msg, MessageDefs.CrossCardShareType, args)
			if resp is None:
				# 没有跨服服务，回退到本服广播
				ObjectMessageGlobal.crossCardShareMsg(self.game, card)
...
# 潜能强化
class CardAbilityExtraStrength(RequestHandlerTask):
	url = r'/game/card/ability/extra/strength'
	...
	def run(self):
		...
		# 潜能强化（内部已调用 onUpdateAbility 更新属性和战力）
		card.abilityExtraStrength(branch, upLevel)
		
		ta.card(card, event='ability_extra', oldFightingPoint=oldFightingPoint, branch=branch)

# 潜能分支切换
class CardAbilityExtraSwitch(RequestHandlerTask):
	url = r'/game/card/ability/extra/switch'
	...
	def run(self):
		...
		# 潜能切换（内部已调用 onUpdateAbility 更新属性和战力）
		card.abilityExtraSwitch(branch)
		
		ta.card(card, event='ability_extra_switch', oldFightingPoint=oldFightingPoint, branch=branch)
...
class CardStarSkillCardExchange(RequestHandlerTask):
	...
	def run(self):
		...
		for costCard in costCards:
			# 使用 isSameCardSeries 判断是否同一精灵系列（支持 Mega 进化前后互通）
			if not card.isSameCardSeries(costCard) and costCard.card_id not in cfg.universalCards:
				raise ClientError('costCards error')
		yield battleCardsAutoDeployment(costCardIDs, self.game, **self.rpcs)
...
# 卡牌从自动布阵中移除
class CardDeployAutoDrop(RequestHandlerTask):
	url = r'/game/card/deploy/auto/drop'
	...
	def run(self):
		...
		for cardID in cardIDs:
			card = self.game.cards.getCard(cardID)
			if card:
				validCardIDs.append(cardID)

		if validCardIDs:
			yield battleCardsAutoDeployment(validCardIDs, self.game, **self.rpcs)