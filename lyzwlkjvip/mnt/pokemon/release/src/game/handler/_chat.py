...
from game.object import FeatureDefs, AchievementDefs, MessageDefs
...
class Chat(RequestHandlerTask):
	...
	def run(self):
		...
		if msgType == 'world':
			...
			# 跨服聊天
			if not ObjectFeatureUnlockCSV.isOpen(FeatureDefs.CrossChat, self.game):
				raise ClientError(ErrDefs.chatNoOpen)
			if self.game.dailyRecord.cross_chat_times >= self.game.role.crossChatTimes:
				raise ClientError(ErrDefs.chatTooMuch)
		...
		if msgType == 'world':
			...
			# 检查跨服聊天是否开启
			if not ObjectServerGlobalRecord.isCrossChatStarted():
				raise ClientError(ErrDefs.chatNoOpen)

			self.game.dailyRecord.cross_chat_times += 1

			# 尝试通过跨服服务广播
			resp = yield ObjectServerGlobalRecord.sendCrossChatMsg(self.game, msg, MessageDefs.CrossChatType)
			if resp is None:
				# 没有跨服服务，回退到本服广播
				msgModel = ObjectMessageGlobal.chatCrossMsg(self.game, msg)
				self.broadcast('/game/push', {
					'msg': {'msgs': [msgModel]},
				})
