...
from game.handler import _emera as emera
...
HandlersMap = {
	...
	DailyAssistantDefs.DrawTotem: lottery.LotteryTotemDraw,
	DailyAssistantDefs.DrawEmera: emera.EmeraDrawHandler,
...
class DailyAssistantOneKey(RequestHandlerTask):
	...
	def run(self):
		...
		# 今日首次使用日常助手，累计天数+1（用于勋章统计）
		if not self.game.dailyRecord.daily_assistant_used:
			self.game.dailyRecord.daily_assistant_used = 1
			self.game.role.daily_assistant_in_days = (self.game.role.daily_assistant_in_days or 0) + 1
...
class DailyAssistantSet(RequestHandlerTask):
	...
	def run(self):
		...
		elif csvID == DailyAssistantDefs.DrawGem:
			# 符石金币10连抽开关
			if value not in (0, 1):
				raise ClientError('drawGem value error')
			dailyAssistant["draw_gem_gold10"] = value
