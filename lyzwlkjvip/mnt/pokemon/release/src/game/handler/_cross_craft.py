...
from framework.service.helper import game2pvp, game2crosscraft
...
class CrossCraftPlayRecordGet(RequestHandlerTask):
	...
	def run(self):
		...
		# crossKey 可能是 game.cn.1，需要转换为 crosscraft.cn.1
		crossCraftKey = game2crosscraft(crossKey)
		rpc = ObjectCrossCraftGameGlobal.cross_client(self.game.role.areaKey, cross_key=crossCraftKey)
		if rpc is None:
			raise ClientError(ErrDefs.crossCraftPlayNotExisted)
		model = yield rpc.call_async('GetCrossCraftPlayRecord', recordID)
		if not model:
			raise ClientError(ErrDefs.crossCraftPlayNotExisted)
		self.write({'model': {'cross_craft_playrecords': {originalRecordID: model}}})
