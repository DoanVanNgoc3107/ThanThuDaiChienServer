...
class HuoDongSaoDang(RequestHandlerTask):
	...
	def run(self):
		...
		# 契约副本扫荡检查：必须先通关（检查 huodongs_gate 中的通关记录）
		if sceneType == MapDefs.TypeContract:
			star = self.game.role.getHuoDongGateStar(huodongID, gateID)
			if star == 0:
				raise ClientError('contract gate not passed')
	
		multiples = 1
		relicBuffType = 0  # 遗迹祝福类型: 1=金币, 2=经验, 3=礼物, 4=碎片
		if sceneType == MapDefs.TypeGold:
			multiples += self.game.trainer.huodongTypeGoldDropRate
			multiples += self.game.privilege.huodongGoldDropRate
		...
		# 遗迹祝福 - 日常副本产出增加 (type 1-4)
		if relicBuffType > 0 and self.game.town:
			from game.object.game.town_lottery import ObjectTownRelicBuff
			buffCfg = ObjectTownRelicBuff.consumeBuff(self.game, relicBuffType)
			if buffCfg:
				param = getattr(buffCfg, 'param', 0)
				if param > 0:
					multiples += param  # param 是倍数加成，如 0.5 表示额外50%
					logger.info('[RelicBuff] huodong: type=%s triggered, param=%s multiples=%s', relicBuffType, param, multiples)
			else:
				logger.info('[RelicBuff] huodong: type=%s no buff available', relicBuffType)
		# 双倍掉落
		yyID = ObjectYYHuoDongFactory.getDoubleDropOpenID(YYHuoDongDefs.DoubleDropGate, gateID)
		if yyID:
			multiples += 1
