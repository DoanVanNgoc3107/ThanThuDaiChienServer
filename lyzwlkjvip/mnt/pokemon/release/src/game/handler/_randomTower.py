...
class RandomTowerEnd(RequestHandlerTask):
	...
	def run(self):
		...
		if randomTower.refresh():
			...
			if result == 'win':
				# 检查是否是困难敌人（monsterType=3），用于勋章统计
				boardID = randomTower.room_info.get('board_id')
				if boardID and csv.random_tower.board[boardID].monsterType == 3:
					self.game.role.random_tower_hard_beat_count = (self.game.role.random_tower_hard_beat_count or 0) + 1
				# 进入下一个房间
				randomTower.nextRoom()
				yield ObjectRankGlobal.onKeyInfoChange(self.game, 'random_tower')
...
class RandomTowerPass(RequestHandlerTask):
	...
	def run(self):
		...
		# 困难敌人（monsterType=3）计数，用于勋章统计
		if cfgBoard.monsterType == 3:
			self.game.role.random_tower_hard_beat_count = (self.game.role.random_tower_hard_beat_count or 0) + 1
