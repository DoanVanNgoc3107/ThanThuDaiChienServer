...
class YYGetAwardOneKey(RequestHandlerTask):
	...
	def run(self):
		...
		if hasattr(YYHuoDongDefs, 'Refloat'):
			allowed_types.append(YYHuoDongDefs.Refloat)  # 惊喜捞一捞
		if hasattr(YYHuoDongDefs, 'RoundFlop'):
			allowed_types.append(YYHuoDongDefs.RoundFlop)  # 惊喜连连
		if csv.yunying.yyhuodong[yyID].type not in allowed_types:
...
class YYLimitBoxGet(RequestHandlerTask):
	...
	def run(self):
		...
		# 使用huodongID区分不同活动的排行榜
		cfg = csv.yunying.yyhuodong[yyID]
		yybox = 'yybox_%s' % cfg.huodongID
...
class YYWordBossStart(RequestHandlerTask):
	...
	def run(self):
		...
		if aidCardIDs is not None:
			...
			if self.game.role.huodong_aid_cards is None:
				self.game.role.huodong_aid_cards = {}
			self.game.role.huodong_aid_cards[WorldBossHuodongID] = aid_dict
		
		# 保存天气/兵种
		if extra is not None:
			self.game.role.deployHuodongExtra(WorldBossHuodongID, extra.get('weather', 0), extra.get('arms', []))
		
		# 战斗数据
		self.game.battle = ObjectWorldBossBattle(self.game)
		ret = self.game.battle.begin(gateID, battleCardIDs, freeCount)
		ret["world_boss_battle"]["extra"] = self.game.role.getHuodongExtra(WorldBossHuodongID)
		
		# 添加助战数据到战斗模型（字典格式）
		aid_cards_dict = self.game.role.huodong_aid_cards.get(WorldBossHuodongID, {}) if self.game.role.huodong_aid_cards else {}
		# 兼容老数据（数组格式）
		if isinstance(aid_cards_dict, list):
			aid_cards_dict = {i+1: v for i, v in enumerate(aid_cards_dict) if v is not None}
		ret["world_boss_battle"]["aid_cards"] = aid_cards_dict
...
class YYWordBossEnd(RequestHandlerTask):
	...
	def run(self):
		...
		# 更新周最大伤害
		if damage > self.game.weeklyRecord.world_boss_damage_max:
			self.game.weeklyRecord.world_boss_damage_max = damage
...
# 世界boss扫荡
class YYWorldBossSaoDang(RequestHandlerTask):
	url = r'/game/yy/world/boss/saodang'
	...
	def run(self):
		...
		yycfg = csv.yunying.yyhuodong[yyID]
		freeCount = yycfg.paramMap['freeCount']
		if self.game.dailyRecord.boss_gate >= freeCount + self.game.dailyRecord.boss_gate_buy:
			raise ClientError(ErrDefs.yyWorldBossCountLimit)

		damage = int(self.game.weeklyRecord.world_boss_damage_max or 0)
		if damage <= 0:
			raise ClientError('damage max error')
		...
		# 按最高伤害档位发奖
		worldBossCfg = hdCls.getWorldBossCfg(yycfg.huodongID)
		eff = hdCls.getDamageAward(self.game, worldBossCfg.huodongID, damage)
		award = {}
		if eff:
			yield effectAutoGain(eff, self.game, self.dbcGame, src="world_boss_saodang", yy_id=yyID)
			award = eff.result

		self.write({'view': {'award': award}})
...
class YYHuodongReunionBindJoin(RequestHandlerTask):
	...
	def run(self):
		...
		try:
			...
			# 勋章计数：重聚活动绑定次数 (targetType=14, medalID=1111)
			self.game.medal.incrementMedalCounter(1111)
		finally:
...
class BraveChallengePrepareStart(RequestHandlerTask):
	...
	def run(self):
		...
		if data and data.get("add"):
			yyObj = ObjectYYHuoDongFactory.getConfig(yyID)
			for cardID in data["add"]:
...
# 惊喜捞一捞 抽奖
class YYRefloatDraw(RequestHandlerTask):
	url = r'/game/yy/refloat/draw'

	@coroutine
	def run(self):
		yyID = self.input.get('yyID', None)
		drawType = self.input.get('drawType', None)  # 'refloat_rmb1' 或 'refloat_rmb10'

		if yyID is None or drawType is None:
			raise ClientError('param miss')
		...
		eff = hdCls.getEffect(yyID, drawType, self.game)
		if eff:
			yield effectAutoGain(eff, self.game, self.dbcGame, src='yy_refloat_%s' % drawType, yy_id=yyID)

		self.write({'view': eff.result if eff else {}})


# 启示之塔 - 选择奖品
class YYRoundDrawChoose(RequestHandlerTask):
	url = r'/game/yy/round_draw/choose'

	@coroutine
	def run(self):
		yyID = self.input.get('yyID', None)
		posID = self.input.get('flag', None)  # 前端发送的是 flag
		csvID = self.input.get('csvID', None)

		if any([x is None for x in (yyID, posID, csvID)]):
			raise ClientError('param miss')
		...
		hdCls.choose(yyID, self.game, posID, csvID)

		self.write({'view': {}})


# 启示之塔 - 抽奖
class YYRoundDrawDraw(RequestHandlerTask):
	url = r'/game/yy/round_draw/draw'
	...
	def run(self):
		...
		eff, csvID = hdCls.draw(yyID, self.game)
		if eff:
			yield effectAutoGain(eff, self.game, self.dbcGame, src='yy_round_draw', yy_id=yyID)

		self.write({'view': {'csvID': csvID}})


# 启示之塔 - 进入下一轮
class YYRoundDrawNext(RequestHandlerTask):
	url = r'/game/yy/round_draw/next'

	@coroutine
	def run(self):
		yyID = self.input.get('yyID', None)
		confirm = self.input.get('confirm', True)
		...
		hdCls.nextRound(yyID, self.game, confirm)

		self.write({'view': {}})


# 轮次翻牌 - 选择大奖
class YYRoundFlopGrandPrizeSwitch(RequestHandlerTask):
	url = r'/game/yy/round_flop/grand_prize/switch'
	...
	def run(self):
		...
		hdCls.switchGrandPrize(yyID, csvID, self.game)

		self.write({'view': {}})


# 轮次翻牌 - 翻牌获取奖励
class YYRoundFlopAwardGet(RequestHandlerTask):
	url = r'/game/yy/round_flop/award/get'
	...
	def run(self):
		...
		if yyID is None or pos is None:
			raise ClientError('param miss')
		...
		ret = hdCls.getFlipEffect(yyID, pos, self.game)
		if ret:
			eff, openResult = ret
			yield effectAutoGain(eff, self.game, self.dbcGame, src='yy_round_flop', yy_id=yyID)
			view = eff.result
