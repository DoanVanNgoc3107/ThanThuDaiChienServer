...
# 拟态对战排行榜奖励邮件由跨服服务 crossmimicry 处理
# 参见 ObjectCrossMimicryGameGlobal.onRankAward


@coroutine
def _updateMedalCounterAsync(dbc, getOnlineByRoleID, roleID, medalID):
	"""
	异步更新勋章计数（内部函数）
	"""
	try:
		# 检查玩家是否在线
		game, safeGuard = getOnlineByRoleID(roleID)
		with safeGuard:
			if game:
				# 在线玩家，直接更新
				game.medal.incrementMedalCounter(medalID, 1)
				logger.info('updateMedalCounterForRank online: roleID=%s medalID=%s', 
					binascii.hexlify(roleID), medalID)
			else:
				# 离线玩家，通过数据库更新
				ret = yield dbc.call_async('DBMultipleReadKeys', 'Role', [roleID], ['medal_counter'])
				if not ret['ret'] or not ret['models']:
					logger.warning('updateMedalCounterForRank: failed to read medal_counter for roleID=%s', binascii.hexlify(roleID))
					return
				
				medalCounter = ret['models'][0].get('medal_counter') or {}
				medalCounter[medalID] = medalCounter.get(medalID, 0) + 1
				
				updateRet = yield dbc.call_async('DBUpdate', 'Role', roleID, {'medal_counter': medalCounter}, False)
				if not updateRet['ret']:
					logger.warning('updateMedalCounterForRank: failed to update medal_counter for roleID=%s', binascii.hexlify(roleID))
					return
				
				logger.info('updateMedalCounterForRank offline: roleID=%s medalID=%s counter=%s', 
					binascii.hexlify(roleID), medalID, medalCounter.get(medalID, 0))
	except Exception as e:
		logger.exception('updateMedalCounterForRank error: roleID=%s medalID=%s error=%s', 
			binascii.hexlify(roleID), medalID, str(e))


def updateMedalCounterForRank(dbc, getOnlineByRoleID, roleID, medalID, targetRank, playerRank):
	"""
	排名结算时更新勋章计数（异步执行，不阻塞）
	
	Args:
		dbc: 数据库连接
		getOnlineByRoleID: 获取在线玩家的函数
		roleID: 玩家ID
		medalID: 勋章ID
		targetRank: 目标排名（如前3名则为3）
		playerRank: 玩家实际排名
	"""
	from tornado.ioloop import IOLoop
	
	# 检查是否达到目标排名
	if playerRank <= 0 or playerRank > targetRank:
		return
	
	# 异步执行勋章更新
	IOLoop.current().spawn_callback(_updateMedalCounterAsync, dbc, getOnlineByRoleID, roleID, medalID)