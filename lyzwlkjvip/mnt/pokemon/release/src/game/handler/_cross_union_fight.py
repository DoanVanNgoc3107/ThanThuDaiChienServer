...
from framework.service.helper import game2crossunionfight
...
class CrossUnionFightPlayRecordGet(RequestHandlerTask):
	...
	def run(self):
		...
		if isinstance(playID, basestring) and len(playID) == 24:
			playID = string2objectid(playID)

		# crossKey 可能是 game.cn.1，需要转换为 crossunionfight.cn.1
		crossUnionFightKey = game2crossunionfight(crossKey)
		role = self.game.role
		rpc = ObjectCrossUnionFightGameGlobal.cross_client(role.areaKey, cross_key=crossUnionFightKey)
		if rpc is None:
