...
class Session(object):
	...
	def _onNewDayInClock5(cls):
		...
		# 拟态对战排行榜结算由跨服服务 crossmimicry 处理
		# CrossMimicryEvent("rank_award", ...) -> ObjectCrossMimicryGameGlobal.onRankAward
	...
	def _onYYLimitBoxAward(cls, yyID):
		...
		logger.info('_onYYLimitBoxAward yyID=%s', yyID)
		...
		rankKey = 'Rank_yybox_%s' % huodongID

		# 兼容合服前后的逻辑
		aliasList = cls.server.alias if cls.server.alias else ['']
		for gameKey in aliasList:
			rankRoles = yield cls.server.dbcGame.call_async('DBSummaryRange', rankKey, 1, maxRank, gameKey)
	...
	def _onYYLimitBoxEnd(cls, yyID):
		from game.object.game.rank import ObjectRankGlobal
		# 使用huodongID清除特定活动的排行榜
		huodongID = csv.yunying.yyhuodong[yyID].huodongID
		logger.info('_onYYLimitBoxEnd yyID=%s huodongID=%s', yyID, huodongID)

		yield ObjectRankGlobal.yyboxRefresh(huodongID)
