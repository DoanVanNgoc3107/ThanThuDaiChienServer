...
class Server(TCPServer):
	...
	def __init__(self, name):
		...
		# set VIP Max by csv (支持更高VIP等级，例如vip20)
		globaldata.VIPLevelMax = max(csv.vip.keys()) - 1 if csv.vip else 0
		...
		# init cross supremacy
		self.initCrossSupremacy()
		logger.info('initCrossSupremacy OK')

		# init cross fishing
		self.initCrossFishing()
		logger.info('initCrossFishing OK')

		# [拟态对战] init cross mimicry
		self.initCrossMimicry()
		logger.info('initCrossMimicry OK')

		# [跨服卡牌对决] init cross auto chess
		self.initCrossAutoChess()
		logger.info('initCrossAutoChess OK')

		# [家园派对] init cross town party
		self.initCrossTownParty()
		logger.info('initCrossTownParty OK')
	...
	def initShop(self):
		from game.object.game.shop import ObjectUnionShop, ObjectExplorerShop, ObjectFragShop, ObjectRandomTowerShop, ObjectEquipShop, ObjectFishingShop, ObjectTotemShop
		...
		# ObjectTotemShop
		data = self.dbcGame.call('DBReadBy', 'TotemShop', {'discard_flag': True})
		if not data['ret']:
			raise ServerError('db read discarded totem shops error')
		ids = [d['id'] for d in data['models']]
		ObjectTotemShop.initFree(ids)
	...
	def initSociety(self):
		...
		oneMonth = 24*3600*30
		now = nowtime_t()
		data = self.dbcGame.call('DBReadRangeBy', 'Role', {
			'last_time': (now-oneMonth, now+oneMonth),
			'level': (2, 99999),
			# 转区旧角色会将 account_id 标记为 orphan_ 前缀，绕过避免 ObjectId 解析异常
			'account_id': {'$not': {'$regex': '^orphan_'}},
		}, 100)
		if not data['ret']:
	...
	def initCrossSupremacy(self):
		from game.object.game.cross_supremacy import ObjectCrossSupremacyGameGlobal

		data = self.dbcGame.call('DBReadAll', 'CrossSupremacyGameGlobal')
		newData = {}
		if not any([model for model in data['models'] if model['key'] == self.key]):
			newData = self.dbcGame.call('DBReadsert', 'CrossSupremacyGameGlobal', {'key': self.key}, False)
			if not newData['ret']:
				raise ServerError('db readsert cross supremacy global error')
		models = data.get('models', []) + ([newData.get('model')] if newData.get('model') else [])

		self.dbcGame.call('DBCommit', False, True)

		for model in models:
			globalObj = ObjectCrossSupremacyGameGlobal(self.dbcGame)
			globalObj.set(model)

			if globalObj.cross_key:
				data = self.container.getserviceOrCreate(globalObj.cross_key).call('CrossSupremacyJoin', globalObj.key)
				globalObj.init(self, data)
			else:
				globalObj.init(self, {})

			logger.info('%s Cross Supremacy key: %s, Now Status %s', globalObj.key, globalObj.cross_key, globalObj.round)
	...
	# [拟态对战] 初始化跨服拟态对战
	def initCrossMimicry(self):
		from game.object.game.cross_mimicry import ObjectCrossMimicryGameGlobal

		data = self.dbcGame.call('DBReadAll', 'CrossMimicryGameGlobal')
		newData = {}
		if not any([model for model in data['models'] if model['key'] == self.key]):
			newData = self.dbcGame.call('DBReadsert', 'CrossMimicryGameGlobal', {'key': self.key}, False)
			if not newData['ret']:
				raise ServerError('db readsert cross mimicry global error')
		models = data.get('models', []) + ([newData.get('model')] if newData.get('model') else [])

		self.dbcGame.call('DBCommit', False, True)

		for model in models:
			globalObj = ObjectCrossMimicryGameGlobal(self.dbcGame)
			globalObj.set(model)

			if globalObj.cross_key:
				data = self.container.getserviceOrCreate(globalObj.cross_key).call('CrossMimicryJoin', globalObj.key)
				globalObj.init(self, data)
			else:
				globalObj.init(self, {})

			logger.info('%s Cross mimicry key: %s, Cross mimicry Now Status %s', globalObj.key, globalObj.cross_key, globalObj.round)

	# [跨服卡牌对决] 初始化跨服卡牌对决
	def initCrossAutoChess(self):
		from game.object.game.cross_auto_chess import ObjectCrossAutoChessGameGlobal

		data = self.dbcGame.call('DBReadAll', 'CrossAutoChessGameGlobal')
		newData = {}
		if not any([model for model in data['models'] if model['key'] == self.key]):
			newData = self.dbcGame.call('DBReadsert', 'CrossAutoChessGameGlobal', {'key': self.key}, False)
			if not newData['ret']:
				raise ServerError('db readsert cross auto chess global error')
		models = data.get('models', []) + ([newData.get('model')] if newData.get('model') else [])

		self.dbcGame.call('DBCommit', False, True)

		for model in models:
			globalObj = ObjectCrossAutoChessGameGlobal(self.dbcGame)
			globalObj.set(model)

			if globalObj.cross_key:
				data = self.container.getserviceOrCreate(globalObj.cross_key).call('CrossAutoChessJoin', globalObj.key)
				globalObj.init(self, data)
			else:
				globalObj.init(self, {})

			logger.info('%s Cross auto chess key: %s, Cross auto chess Now Status %s', globalObj.key, globalObj.cross_key, globalObj.round)

	# [家园派对] 初始化跨服派对
	def initCrossTownParty(self):
		from game.object.game.cross_town_party import ObjectCrossTownPartyGlobal

		data = self.dbcGame.call('DBReadAll', 'CrossTownPartyGlobal')
		newData = {}
		if not any([model for model in data['models'] if model['key'] == self.key]):
			newData = self.dbcGame.call('DBReadsert', 'CrossTownPartyGlobal', {'key': self.key}, False)
			if not newData['ret']:
				raise ServerError('db readsert cross town party global error')
		models = data.get('models', []) + ([newData.get('model')] if newData.get('model') else [])

		self.dbcGame.call('DBCommit', False, True)

		for model in models:
			globalObj = ObjectCrossTownPartyGlobal(self.dbcGame)
			globalObj.set(model)

			# 和拟态一样，由 match 服务分配跨服 key
			# 如果已有 cross_key（match 已分配），则连接跨服
			if globalObj.cross_key:
				try:
					data = self.container.getserviceOrCreate(globalObj.cross_key).call('CrossTownPartyJoin', globalObj.key)
					globalObj.init(self, data)
					logger.info('%s Cross town party connected: cross_key=%s round=%s', globalObj.key, globalObj.cross_key, globalObj.round)
				except Exception as e:
					logger.warning('%s Cross town party connect failed: %s', globalObj.key, e)
					globalObj.init(self, {})
			else:
				# 等待 match 服务分配跨服
				globalObj.init(self, {})
				logger.info('%s Cross town party waiting for match, round=%s', globalObj.key, globalObj.round)
